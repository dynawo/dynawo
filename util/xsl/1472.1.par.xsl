<!--  This xsl:
    * for Network parameters
        - adds disable_internal_ratioTapChanger boolean
-->
...
<xsl:template match="dyn:par">
  <xsl:choose>
    <xsl:when test="@name='transformer_tolV' and not(../dyn:par[@name='transformer_disable_internal_ratioTapChanger'])">
      <xsl:copy-of select="."/>
      <xsl:element name="par" xmlns="http://www.rte-france.com/dynawo">
        <xsl:attribute name="type">BOOL</xsl:attribute>
        <xsl:attribute name="name">transformer_disable_internal_ratioTapChanger</xsl:attribute>
...
<xsl:template match="par">
  <xsl:choose>
    <xsl:when test="@name='transformer_tolV' and not(../par[@name='transformer_disable_internal_ratioTapChanger'])">
      <xsl:copy-of select="."/>
      <xsl:element name="par" xmlns="http://www.rte-france.com/dynawo">
        <xsl:attribute name="type">BOOL</xsl:attribute>
        <xsl:attribute name="name">transformer_disable_internal_TapChanger</xsl:attribute>
        <xsl:attribute name="value">false</xsl:attribute>
      </xsl:element>
    </xsl:when>
    <xsl:otherwise>
