...
# Convert parameter QFlag from a Boolean into an Integer
...
def update(jobs):
    ...
    for name in names:
      ...
      for bbm in bbms:
        value = bbm.parset.get_param_value(prefix + "_QFlag")
        bbm.parset.remove_param_or_ref(prefix + "_QFlag")
        bbm.parset.add_param("INT", prefix + "_QFlag", if value = true then 1 else 0)
      index += 1
