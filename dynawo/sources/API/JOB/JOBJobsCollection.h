...
class JobsCollection {
  ...
  * @return variables
  */
  const std::vector<boost::shared_ptr<JobEntry> >& getJobs() const {
    return jobs_;
  }
  ...
  * @return variables
  */
  std::vector<boost::shared_ptr<JobEntry> >& getNonCstJobs() const {
    return const_cast<std::vector<boost::shared_ptr<JobEntry> >&>(getJobs());
  }
