...
class XmlImporter : public Importer {
 public:
   ...
   */
  boost::shared_ptr<JobsCollection> importFromFile(const std::string& fileName) const override;

  /**
   * @copydoc Importer::importFromStream()
   */
  boost::shared_ptr<JobsCollection> importFromStream(std::istream& stream) const override;
};
