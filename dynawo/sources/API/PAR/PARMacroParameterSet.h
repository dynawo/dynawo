...
class MacroParameterSet {
 public:
   ...
   */
  void addReference(const boost::shared_ptr<Reference>& reference);

  /**
   * @brief parameter adder
   * @param[in] parameter to be added
   */
  void addParameter(const boost::shared_ptr<Parameter>& parameter);
  ...
  */
  const std::map<std::string, boost::shared_ptr<Reference> >& getReferences() const {
    return references_;
  }
  ...
  * @return macro parameters sets
  */
  const std::map<std::string, boost::shared_ptr<Parameter> >& getParameters() const {
    return parameters_;
  }
