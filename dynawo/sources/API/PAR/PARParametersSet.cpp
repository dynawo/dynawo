...
void
ParametersSet::extend(const shared_ptr<ParametersSet>& parametersSet) {
...
shared_ptr<ParametersSet>
ParametersSet::addReference(const boost::shared_ptr<Reference>& ref) {
...
const unordered_map<std::string, boost::shared_ptr<Reference> >&
ParametersSet::getReferences() {
  return references_;
}

const std::map<std::string, boost::shared_ptr<MacroParSet> >&
