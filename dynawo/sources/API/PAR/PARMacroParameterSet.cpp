...
void
MacroParameterSet::addReference(const boost::shared_ptr<Reference>& reference) {
  std::string name = reference->getName();
  const std::pair<std::map<std::string, boost::shared_ptr<Reference> >::iterator, bool> ret = references_.emplace(name, reference);
  if (!ret.second)
    throw DYNError(DYN::Error::API, ReferenceAlreadySetInMacroParameterSet, reference->getName(), getId());
}

void
MacroParameterSet::addParameter(const boost::shared_ptr<Parameter>& parameter) {
  std::string name = parameter->getName();
  const std::pair<std::map<std::string, boost::shared_ptr<Parameter> >::iterator, bool> ret = parameters_.emplace(name, parameter);
  if (!ret.second)
