...
TEST(APIPARTest, ParametersSetCreate) {
  ...
  // Test size
  int nbParameters = parametersSet->getParameters().size();
...
TEST(APIPARTest, ParametersSetAddReference) {
  ...
  const std::unordered_map<std::string, boost::shared_ptr<Reference> >& references = parametersSet2->getReferences();
  const int nbRefs = 3;
...
TEST(APIPARTest, ParametersSetGetReferences) {
  ...
  // Test size
  int nbReferences = parametersSet->getReferences().size();
