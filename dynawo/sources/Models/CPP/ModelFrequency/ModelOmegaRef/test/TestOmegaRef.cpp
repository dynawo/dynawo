...
TEST(ModelsModelOmegaRef, ModelOmegaRefDefineMethods) {
  ...
  ASSERT_EQ(variableOmegaRef->getName(), "omegaRef_0_");
  ...
  ASSERT_EQ(elementOmegaRef.getTypeElement(), Element::TERMINAL);
  ASSERT_EQ(elementOmegaRef.name(), elementOmegaRef.id());
  ASSERT_EQ(elementOmegaRef.name(), "omegaRef_0_");
  ASSERT_EQ(mapElements["omegaRef_0_"], 0);
  Element elementOmegaGrp = elements[10];
  ASSERT_EQ(elementOmegaGrp.getTypeElement(), Element::TERMINAL);
  ASSERT_EQ(elementOmegaGrp.name(), elementOmegaGrp.id());
  ASSERT_EQ(elementOmegaGrp.name(), "omega_grp_0_");
  ASSERT_EQ(mapElements["omega_grp_0_"], 10);
  Element elementNumCC = elements[11];
  ASSERT_EQ(elementNumCC.getTypeElement(), Element::TERMINAL);
  ASSERT_EQ(elementNumCC.name(), elementNumCC.id());
  ASSERT_EQ(elementNumCC.name(), "numcc_node_0_");
  ASSERT_EQ(mapElements["numcc_node_0_"], 11);
  Element elementRunningGrp = elements[12];
  ASSERT_EQ(elementRunningGrp.getTypeElement(), Element::TERMINAL);
  ASSERT_EQ(elementRunningGrp.name(), elementRunningGrp.id());
  ASSERT_EQ(elementRunningGrp.name(), "running_grp_0_");
  ASSERT_EQ(mapElements["running_grp_0_"], 12);
  Element elementOmegaRefGrp = elements[13];
  ASSERT_EQ(elementOmegaRefGrp.getTypeElement(), Element::TERMINAL);
  ASSERT_EQ(elementOmegaRefGrp.name(), elementOmegaRefGrp.id());
  ASSERT_EQ(elementOmegaRefGrp.name(), "omegaRef_grp_0_");
  ASSERT_EQ(mapElements["omegaRef_grp_0_"], 13);
}
