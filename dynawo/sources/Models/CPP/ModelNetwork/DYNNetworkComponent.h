...
class NetworkComponent {  ///< Base class for network component models
 public:
   ...
   */
  typedef enum { NO_CHANGE = 0, STATE_CHANGE = 1, TOPO_CHANGE = 2 } StateChange_t;
 ...
 /**
   ...
   */
  virtual int sizeCalculatedVar() const {
   ...
   */
  virtual unsigned int getOffsetCalculatedVar() const {
   ...
   */
  virtual void setOffsetCalculatedVar(unsigned int offset) {
 ...
 protected:
   ...
   */
  template<typename T>
  T getParameterDynamic(const boost::unordered_map<std::string, ParameterModeler>& params, const std::string& id, const std::vector<std::string>& ids) const;
   ...
   */
  template<typename T>
  T getParameterDynamicNoThrow(const boost::unordered_map<std::string, ParameterModeler>& params, const std::string& id, bool& foundParam,
 ...
 private:
   ...
   */
  template<typename T>
  void findParameterDynamicNoThrow(const boost::unordered_map<std::string, ParameterModeler>& params, const std::string& id, bool& foundParam,
                                   const std::vector<std::string>& ids, T& value) const;
