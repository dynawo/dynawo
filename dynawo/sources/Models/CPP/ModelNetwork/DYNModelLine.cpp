...
ModelLine::evalJt(SparseMatrix& jt, const double& cj, const int& rowOffset) {
  ...
  Timer timer3("ModelNetwork::ModelLine::evalJt");
#endif
  if (!isDynamic_ || network_->isInitModel())
    return;
  ...
  if (isDynamic_ && getConnectionState() == CLOSED) {
    switch (knownBus_) {
      case BUS1_BUS2: {
        ...
        int ur1YNum = modelBus1.urYNum();
        int ui1YNum = modelBus1.uiYNum();
        int ur2YNum = modelBus2.urYNum();
        int ui2YNum = modelBus2.uiYNum();
