...
class ModelBus : public NetworkComponent {
 ...
 protected :
  /**
   * @brief constructor from ID and static data, for inheriting classes
   * @param id the component ID in ModelNetwork
   * @param bus the bus description from static data
   */
  ModelBus(const std::string & id, const std::shared_ptr<BusInterface> & bus);
