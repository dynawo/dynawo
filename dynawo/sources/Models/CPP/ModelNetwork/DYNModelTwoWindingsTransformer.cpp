...
modelType_("TwoWindingsTransformer") {
  ...
  const shared_ptr<PhaseTapChangerInterface> phaseTapChanger = tfo->getPhaseTapChanger();
  const shared_ptr<RatioTapChangerInterface> ratioTapChanger = tfo->getRatioTapChanger();
  ...
  if (phaseTapChanger) {
    ...
    const vector<shared_ptr<StepInterface> > steps = phaseTapChanger->getSteps();
    ...
    if (tfo->getRatioTapChanger()->hasLoadTapChangingCapabilities() && !tfo->getRatioTapChanger()->getTerminalRefSide().empty()) {
      ...
      const vector<shared_ptr<StepInterface> > steps = ratioTapChanger->getSteps();
      for (const auto& step : steps) {
  ...
  const vector<shared_ptr<CurrentLimitInterface> > cLimit1 = tfo->getCurrentLimitInterfaces1();
  if (!cLimit1.empty()) {
  ...
  const vector<shared_ptr<CurrentLimitInterface> > cLimit2 = tfo->getCurrentLimitInterfaces2();
  if (!cLimit2.empty()) {
