...
TEST(ModelsModelNetwork, ModelNetworkHvdcLinkCalculatedVariables) {
  ...
  yI[0] = 0.35;
  yI[1] = 0.02;
  ASSERT_DOUBLE_EQUALS_DYNAWO(hvdc->evalCalculatedVarI(ModelHvdcLink::p1Num_, &yI[0], &yp[0]), calculatedVars[ModelHvdcLink::p1Num_]);
  ASSERT_DOUBLE_EQUALS_DYNAWO(hvdc->evalCalculatedVarI(ModelHvdcLink::q1Num_, &yI[0], &yp[0]), calculatedVars[ModelHvdcLink::q1Num_]);
  yI[0] = 5;
  yI[1] = 2.5;
  ASSERT_DOUBLE_EQUALS_DYNAWO(hvdc->evalCalculatedVarI(ModelHvdcLink::p2Num_, &yI[0], &yp[0]), calculatedVars[ModelHvdcLink::p2Num_]);
  ASSERT_DOUBLE_EQUALS_DYNAWO(hvdc->evalCalculatedVarI(ModelHvdcLink::q2Num_, &yI[0], &yp[0]), calculatedVars[ModelHvdcLink::q2Num_]);
  ASSERT_THROW_DYNAWO(hvdc->evalCalculatedVarI(42, &yI[0], &yp[0]), Error::MODELER, KeyError_t::UndefCalculatedVarI);
  ...
  yI[0] = 0.35;
  yI[1] = 0.02;
  ASSERT_THROW_DYNAWO(hvdc->evalJCalculatedVarI(42, &yI[0], &yp[0], res), Error::MODELER, KeyError_t::UndefJCalculatedVarI);
  ASSERT_NO_THROW(hvdc->evalJCalculatedVarI(ModelHvdcLink::p1Num_, &yI[0], &yp[0], res));
  ASSERT_DOUBLE_EQUALS_DYNAWO(res[0], 7.8062556418956319e-18);
  ASSERT_DOUBLE_EQUALS_DYNAWO(res[1], 8.6736173798840355e-19);
  std::fill(res.begin(), res.end(), 0);
  ASSERT_NO_THROW(hvdc->evalJCalculatedVarI(ModelHvdcLink::q1Num_, &yI[0], &yp[0], res));
  ASSERT_DOUBLE_EQUALS_DYNAWO(res[0], 1.3010426069826053e-17);
  ASSERT_DOUBLE_EQUALS_DYNAWO(res[1], 1.3877787807814457e-17);
  std::fill(res.begin(), res.end(), 0);
  yI[0] = 5;
  yI[1] = 2.5;
  ASSERT_NO_THROW(hvdc->evalJCalculatedVarI(ModelHvdcLink::p2Num_, &yI[0], &yp[0], res));
  ASSERT_DOUBLE_EQUALS_DYNAWO(res[0], 8.4025668367626594e-19);
  ASSERT_DOUBLE_EQUALS_DYNAWO(res[1], 4.3368086899420177e-19);
  std::fill(res.begin(), res.end(), 0);
  ASSERT_NO_THROW(hvdc->evalJCalculatedVarI(ModelHvdcLink::q2Num_, &yI[0], &yp[0], res));
  ...
  ASSERT_THROW_DYNAWO(hvdc->getDefJCalculatedVarI(42, numVars), Error::MODELER, KeyError_t::UndefJCalculatedVarI);
  ASSERT_NO_THROW(hvdc->getDefJCalculatedVarI(ModelHvdcLink::p1Num_, numVars));
  ASSERT_EQ(numVars.size(), 2);
  for (size_t i = 0; i < numVars.size(); ++i) {
    ASSERT_EQ(numVars[i], i);
  }
  numVars.clear();
  ASSERT_NO_THROW(hvdc->getDefJCalculatedVarI(ModelHvdcLink::q1Num_, numVars));
  ASSERT_EQ(numVars.size(), 2);
  for (size_t i = 0; i < numVars.size(); ++i) {
    ASSERT_EQ(numVars[i], i);
  }
  numVars.clear();
  ASSERT_NO_THROW(hvdc->getDefJCalculatedVarI(ModelHvdcLink::p2Num_, numVars));
  ASSERT_EQ(numVars.size(), 2);
  for (size_t i = 0; i < numVars.size(); ++i) {
    ASSERT_EQ(numVars[i], i + 2);
  }
  numVars.clear();
  ASSERT_NO_THROW(hvdc->getDefJCalculatedVarI(ModelHvdcLink::q2Num_, numVars));
  ASSERT_EQ(numVars.size(), 2);
  for (size_t i = 0; i < numVars.size(); ++i) {
    ASSERT_EQ(numVars[i], i + 2);
  }
