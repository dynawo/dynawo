...
TEST(ModelsModelNetwork, ModelNetworkLineDiscreteVariables) {
  ...
  z[0] = OPEN;
  z[1] = 0.;
  ASSERT_EQ(dl->evalZ(0., deactivateZeroCrossingFunctions), NetworkComponent::TOPO_CHANGE);
  ASSERT_EQ(dl->getConnectionState(), OPEN);
  ASSERT_EQ(z[0], OPEN);
  ASSERT_DOUBLE_EQUALS_DYNAWO(dl->getCurrentLimitsDesactivate(), 0.);
  ASSERT_DOUBLE_EQUALS_DYNAWO(z[1], dl->getCurrentLimitsDesactivate());
  ...
  dl->setConnectionState(OPEN);
  ASSERT_EQ(dl->evalZ(0., deactivateZeroCrossingFunctions), NetworkComponent::NO_CHANGE);
  ...
  ASSERT_THROW_DYNAWO(dl->evalZ(0., deactivateZeroCrossingFunctions), Error::MODELER, KeyError_t::NoThirdSide);

  dl->setConnectionState(UNDEFINED_STATE);
  ASSERT_THROW_DYNAWO(dl->evalZ(0., deactivateZeroCrossingFunctions), Error::MODELER, KeyError_t::UnsupportedComponentState);
  ...
  z[0] = UNDEFINED_STATE;
  dl->setConnectionState(CLOSED);
  ASSERT_THROW_DYNAWO(dl->evalZ(0., deactivateZeroCrossingFunctions), Error::MODELER, KeyError_t::UndefinedComponentState);
  z[0] = CLOSED_3;
  ASSERT_THROW_DYNAWO(dl->evalZ(0., deactivateZeroCrossingFunctions), Error::MODELER, KeyError_t::NoThirdSide);
  z[0] = CLOSED;
...
TEST(ModelsModelNetwork, ModelNetworkDynamicLine) {
  ...
  z3[0] = CLOSED_2;
  ASSERT_THROW_DYNAWO(dl3->evalZ(0, deactivateZeroCrossingFunctions), Error::MODELER, KeyError_t::DynamicLineStatusNotSupported);
  z3[0] = CLOSED_1;
  ASSERT_THROW_DYNAWO(dl3->evalZ(0, deactivateZeroCrossingFunctions), Error::MODELER, KeyError_t::DynamicLineStatusNotSupported);
  z3[0] = CLOSED;
  ASSERT_NO_THROW(dl3->evalZ(0, deactivateZeroCrossingFunctions));
  delete[] zConnected;
  delete[] zConnected3;
}
