...
class NetworkBridge : public NetworkComponent {
 public:
  ...
  NetworkComponent::StateChange_t evalState(double) override;
 ...
 private:
  ...
  State lastStateEvalZ_ = UNDEFINED_STATE;
  State lastStateEvalState_ = UNDEFINED_STATE;
  std::string stateVarPrefix_;

  // unused virtual methods from NetworkComponent from here on
 public :
  /** @brief unused */
