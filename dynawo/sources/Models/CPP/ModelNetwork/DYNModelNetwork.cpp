...
ModelNetwork::initializeFromData(const shared_ptr<DataInterface>& data) {
  ...
  for (const auto& voltageLevel : network->getVoltageLevels()) {
    ...
    shared_ptr<ModelVoltageLevel> modelVoltageLevel(new ModelVoltageLevel(voltageLevel));
    shared_ptr<ModelVoltageLevel> modelVoltageLevelInit(new ModelVoltageLevel(voltageLevel));
    // Add to containers
    ...
    for (const auto& bus : voltageLevel->getBuses()) {
      string id = bus->getID();
      shared_ptr<ModelBus> modelBus(new ModelBus(bus, voltageLevel->isNodeBreakerTopology()));
    ...
    for (const auto& aSwitch : voltageLevel->getSwitches()) {
      ...
      shared_ptr<ModelSwitch> modelSwitch(new ModelSwitch(aSwitch));

      modelSwitch->setNetwork(this);
      if (aSwitch->getBusInterface1()) {
        shared_ptr<ModelBus> modelBus1 = modelBusById[aSwitch->getBusInterface1()->getID()];
        modelSwitch->setModelBus1(modelBus1);
      }
      if (aSwitch->getBusInterface2()) {
        shared_ptr<ModelBus> modelBus2 = modelBusById[aSwitch->getBusInterface2()->getID()];
    ...
    for (const auto& load : voltageLevel->getLoads()) {
      ...
      shared_ptr<ModelBus> modelBus = modelBusById[load->getBusInterface()->getID()];
      shared_ptr<ModelLoad> modelLoad(new ModelLoad(*load, *modelBus));
    ...
    for (const auto& generator : voltageLevel->getGenerators()) {
      ...
      shared_ptr<ModelGenerator> modelGenerator(new ModelGenerator(generator));
      modelGenerator->setNetwork(this);
      shared_ptr<ModelBus> modelBus = modelBusById[generator->getBusInterface()->getID()];
    ...
    for (const auto& shunt : voltageLevel->getShuntCompensators()) {
      ...
      shared_ptr<ModelShuntCompensator> modelShuntCompensator(new ModelShuntCompensator(shunt));
      modelShuntCompensator->setNetwork(this);
      shared_ptr<ModelBus> modelBus = modelBusById[shunt->getBusInterface()->getID()];
    ...
    for (const auto& sVarC : voltageLevel->getStaticVarCompensators()) {
      ...
      shared_ptr<ModelStaticVarCompensator> modelStaticVarCompensator(new ModelStaticVarCompensator(sVarC));
      modelStaticVarCompensator->setNetwork(this);
      shared_ptr<ModelBus> modelBus = modelBusById[sVarC->getBusInterface()->getID()];
    ...
    for (const auto& danglingLine : voltageLevel->getDanglingLines()) {
      ...
      shared_ptr<ModelDanglingLine> modelDanglingLine(new ModelDanglingLine(danglingLine));
      modelDanglingLine->setNetwork(this);
      shared_ptr<ModelBus> modelBus = modelBusById[danglingLine->getBusInterface()->getID()];
  ...
  for (const auto& line : network->getLines()) {
    ...
    shared_ptr<ModelLine> modelLine(new ModelLine(line));
    modelLine->setNetwork(this);
    if (line->getBusInterface1()) {
      shared_ptr<ModelBus> modelBus1 = modelBusById[line->getBusInterface1()->getID()];
      modelLine->setModelBus1(modelBus1);
    }
    if (line->getBusInterface2()) {
      shared_ptr<ModelBus> modelBus2 = modelBusById[line->getBusInterface2()->getID()];
  ...
  for (const auto& twoWTfo : network->getTwoWTransformers()) {
    ...
    shared_ptr<ModelTwoWindingsTransformer> modelTwoWindingsTransformer(new ModelTwoWindingsTransformer(twoWTfo));
    modelsTfo.push_back(modelTwoWindingsTransformer);
    modelTwoWindingsTransformer->setNetwork(this);
    if (twoWTfo->getBusInterface1()) {
      shared_ptr<ModelBus> modelBus1 = modelBusById[twoWTfo->getBusInterface1()->getID()];
      modelTwoWindingsTransformer->setModelBus1(modelBus1);
    }
    if (twoWTfo->getBusInterface2()) {
      shared_ptr<ModelBus> modelBus2 = modelBusById[twoWTfo->getBusInterface2()->getID()];
  ...
  for (const auto& threeWTfo : network->getThreeWTransformers()) {
    ...
    shared_ptr<ModelThreeWindingsTransformer> modelThreeWindingsTransformer(new ModelThreeWindingsTransformer(threeWTfo));
    modelThreeWindingsTransformer->setNetwork(this);
    if (threeWTfo->getBusInterface1()) {
      shared_ptr<ModelBus> modelBus1 = modelBusById[threeWTfo->getBusInterface1()->getID()];
      modelThreeWindingsTransformer->setModelBus1(modelBus1);
    }
    if (threeWTfo->getBusInterface2()) {
      shared_ptr<ModelBus> modelBus2 = modelBusById[threeWTfo->getBusInterface2()->getID()];
      modelThreeWindingsTransformer->setModelBus2(modelBus2);
    }
    if (threeWTfo->getBusInterface3()) {
      shared_ptr<ModelBus> modelBus3 = modelBusById[threeWTfo->getBusInterface3()->getID()];
  ...
  for (const auto&  hvdcLine : network->getHvdcLines()) {
    ...
    const shared_ptr<ConverterInterface>& conv1 = hvdcLine->getConverter1();
    const shared_ptr<ConverterInterface>& conv2 = hvdcLine->getConverter2();
    ...
    shared_ptr<ModelHvdcLink> modelHvdcLink(new ModelHvdcLink(hvdcLine));
...
ModelNetwork::getIndexesOfVariablesUsedForCalculatedVarI(const unsigned iCalculatedVar, std::vector<int>& indexes) const {
  ...
  const shared_ptr<NetworkComponent>& comp = isInitModel_ ?  initComponents_[index] : components_[index];
...
ModelNetwork::initParams() {
  ...
  const shared_ptr<ParametersSet> networkModelLocalInitParameters =
      shared_ptr<ParametersSet>(new ParametersSet("networkModelLocalInitParameters"));
...
void
ModelNetwork::defineVariables(vector<shared_ptr<Variable> >& variables) {
