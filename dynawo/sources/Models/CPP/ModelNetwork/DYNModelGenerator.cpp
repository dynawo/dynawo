...
ModelGenerator::evalDerivatives(const double /*cj*/) {
  ...
  if (!network_->isInitModel() && isConnected() && modelBus.getSwitchOff()) {
    ...
    if (isVoltageDependant_) {
      ...
      modelBus_->derivatives()->addDerivative(IR_DERIVATIVE, urYNum, ir_dUrValue);
      modelBus_->derivatives()->addDerivative(IR_DERIVATIVE, uiYNum, ir_dUiValue);
      modelBus_->derivatives()->addDerivative(II_DERIVATIVE, urYNum, ii_dUrValue);
      modelBus_->derivatives()->addDerivative(II_DERIVATIVE, uiYNum, ii_dUiValue);
    } else {
      const double Pc = PcPu();
      const double Qc = QcPu();
