...
TEST(ModelsModelAreaShedding, ModelAreaSheddingDefineMethods) {
  ...
  ASSERT_EQ(variableAreaShedding->getName(), "deltaP_load_0_");
  ...
  ASSERT_EQ(variableAreaShedding->getName(), "deltaQ_load_0_");
  ASSERT_EQ(variableAreaShedding->getType(), CONTINUOUS);
  ASSERT_EQ(variableAreaShedding->getNegated(), false);
  ASSERT_EQ(variableAreaShedding->isState(), true);
  ASSERT_EQ(variableAreaShedding->isAlias(), false);
  variableAreaShedding = variables[2];
  ASSERT_EQ(variableAreaShedding->getName(), "deltaP_load_1_");
  ASSERT_EQ(variableAreaShedding->getType(), CONTINUOUS);
  ASSERT_EQ(variableAreaShedding->getNegated(), false);
  ASSERT_EQ(variableAreaShedding->isState(), true);
  ASSERT_EQ(variableAreaShedding->isAlias(), false);
  variableAreaShedding = variables[3];
  ASSERT_EQ(variableAreaShedding->getName(), "deltaQ_load_1_");
  ASSERT_EQ(variableAreaShedding->getType(), CONTINUOUS);
  ASSERT_EQ(variableAreaShedding->getNegated(), false);
  ASSERT_EQ(variableAreaShedding->isState(), true);
  ASSERT_EQ(variableAreaShedding->isAlias(), false);
  ...
  ASSERT_EQ(element.name(), "deltaP_load_0_");
  ASSERT_EQ(element.subElementsNum().size(), 0);
  ASSERT_EQ(mapElements["deltaP_load_0_"], 0);
  element = elements[1];
  ASSERT_EQ(element.getTypeElement(), Element::TERMINAL);
  ASSERT_EQ(element.name(), element.id());
  ASSERT_EQ(element.id(), "deltaQ_load_0_");
  ASSERT_EQ(element.subElementsNum().size(), 0);
  ASSERT_EQ(mapElements["deltaQ_load_0_"], 1);
  element = elements[2];
  ASSERT_EQ(element.getTypeElement(), Element::TERMINAL);
  ASSERT_EQ(element.name(), element.id());
  ASSERT_EQ(element.name(), "deltaP_load_1_");
  ASSERT_EQ(element.subElementsNum().size(), 0);
  ASSERT_EQ(mapElements["deltaP_load_1_"], 2);
  element = elements[3];
  ASSERT_EQ(element.getTypeElement(), Element::TERMINAL);
  ASSERT_EQ(element.name(), element.id());
  ASSERT_EQ(element.id(), "deltaQ_load_1_");
  ASSERT_EQ(element.subElementsNum().size(), 0);
  ASSERT_EQ(mapElements["deltaQ_load_1_"], 3);
}
