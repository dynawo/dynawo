...
TEST(ModelsModelVariationArea, ModelVariationAreaDefineMethods) {
  ...
  ASSERT_EQ(variableVariationArea->getName(), "DeltaPc_load_0_");
  ...
  ASSERT_EQ(variableVariationArea->getName(), "DeltaQc_load_0_");
  ASSERT_EQ(variableVariationArea->getType(), CONTINUOUS);
  ASSERT_EQ(variableVariationArea->getNegated(), false);
  ASSERT_EQ(variableVariationArea->isState(), true);
  ASSERT_EQ(variableVariationArea->isAlias(), false);
  variableVariationArea = variables[2];
  ASSERT_EQ(variableVariationArea->getName(), "DeltaPc_load_1_");
  ASSERT_EQ(variableVariationArea->getType(), CONTINUOUS);
  ASSERT_EQ(variableVariationArea->getNegated(), false);
  ASSERT_EQ(variableVariationArea->isState(), true);
  ASSERT_EQ(variableVariationArea->isAlias(), false);
  variableVariationArea = variables[3];
  ASSERT_EQ(variableVariationArea->getName(), "DeltaQc_load_1_");
  ASSERT_EQ(variableVariationArea->getType(), CONTINUOUS);
  ASSERT_EQ(variableVariationArea->getNegated(), false);
  ASSERT_EQ(variableVariationArea->isState(), true);
  ASSERT_EQ(variableVariationArea->isAlias(), false);
  ...
  ASSERT_EQ(element.name(), "DeltaPc_load_0_");
  ASSERT_EQ(element.subElementsNum().size(), 0);
  ASSERT_EQ(mapElements["DeltaPc_load_0_"], 0);
  element = elements[1];
  ASSERT_EQ(element.getTypeElement(), Element::TERMINAL);
  ASSERT_EQ(element.name(), element.id());
  ASSERT_EQ(element.id(), "DeltaQc_load_0_");
  ASSERT_EQ(element.subElementsNum().size(), 0);
  ASSERT_EQ(mapElements["DeltaQc_load_0_"], 1);
  element = elements[2];
  ASSERT_EQ(element.getTypeElement(), Element::TERMINAL);
  ASSERT_EQ(element.name(), element.id());
  ASSERT_EQ(element.name(), "DeltaPc_load_1_");
  ASSERT_EQ(element.subElementsNum().size(), 0);
  ASSERT_EQ(mapElements["DeltaPc_load_1_"], 2);
  element = elements[3];
  ASSERT_EQ(element.getTypeElement(), Element::TERMINAL);
  ASSERT_EQ(element.name(), element.id());
  ASSERT_EQ(element.id(), "DeltaQc_load_1_");
  ASSERT_EQ(element.subElementsNum().size(), 0);
  ASSERT_EQ(mapElements["DeltaQc_load_1_"], 3);
}
