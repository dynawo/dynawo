...
model SurgeVoltage
 ...
 parameter Real Vm (unit="kV")=-3060"maximum voltage of the source";
 parameter Real alpha (unit="1/S")=-14000 "Alpha coefficient";
 parameter Real beta  (unit="1/S")=-4166666 "Beta coefficient";
 parameter Modelica.SIunits.Time Tstart=0  ;
 parameter Modelica.SIunits.Time Tstop =10 ;
equation
if (time>=Tstart and time<=Tstop) then
 v=Vm*1e3*(exp(alpha*(time-Tstart))-exp(beta*(time-Tstart)));
else
 v=0;
end if;
  annotation (
    defaultComponentName = "Vsurge", Icon(coordinateSystem(initialScale = 0.1), graphics={Line(points = {{-90, 0}, {90, 0}}, color = {0, 0, 255}), Line(points = {{-70, 30}, {-70, 10}}, color = {0, 0, 255}), Line(points = {{60, 20}, {80, 20}}, color = {0, 0, 255}), Ellipse(origin = {-2, 0}, lineColor = {0, 0, 255}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{-50, 50}, {50, -50}}, endAngle = 360), Line(points = {{-80, 20}, {-60, 20}}, color = {0, 0, 255}), Line(origin = {0.379747, 0}, points = {{-76, -59}, {-73.2, -44.3}, {-70.3, -31.1}, {-66.8, -16.6}, {-63.3, -4}, {-59.7, 6.92}, {-55.5, 18.18}, {-51.3, 27.7}, {-46.3, 37}, {-40.6, 45.5}, {-34.3, 53.1}, {-27.2, 59.6}, {-18.7, 65.3}, {-8.1, 70.2}, {-6, 71}, {-3.88, 58.5}, {-1.05, 43.7}, {1.78, 30.8}, {4.606, 19.45}, {8.14, 7.3}, {11.68, -3}, {15.9, -13.2}, {20.2, -21.6}, {25.1, -29.5}, {30.8, -36.4}, {37.1, -42.3}, {44.9, -47.5}, {54.8, -51.8}, {64, -54.4}}, color = {192, 192, 192}), Text(origin = {-21, 102}, lineColor = {0, 0, 255}, extent = {{-55, 26}, {87, -44}}, textString = "%name")}),
    ...
    Documentation(revisions= "<html><head></head><body><ul style=\"font-size: 12px;\"><li><em>2020-09-22 &nbsp;&nbsp;</em>&nbsp;by Alireza Masoom initially implemented</li></ul></body></html>", info= "<html><head></head><body><p>The voltage source equation is given by:</p><p>&nbsp;V(t)=Vm*{exp(alpha*time)-exp(beta*time)}</p>
</body></html>"));
end SurgeVoltage;
