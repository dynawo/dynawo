...
model CosineVoltage "V=Vm cos (wt +theta)"
 extends OpenEMTP.Interfaces.OnePort;
 parameter Real Vm( unit="kV RMS")= 230 "Magnitude of voltage" annotation(HideResult=true);
 parameter SI.Frequency f=60 "Frequency" annotation(HideResult=true);
 parameter SI.Angle Phase= 0
                            "phase angle" annotation(HideResult=true);
 parameter SI.Time StartTime=0 "start time,if t < t_start, the source is shorted" annotation(HideResult=true);
 parameter SI.Time StopTime= 1 "stop time, if t > t_stop, the source is shorted" annotation(HideResult=true);
equation
 if time<StartTime or time>StopTime then v=0; else
  v=Vm*1000*sqrt(2)*cos(2*C.pi*f*time+Phase);
 end if;
