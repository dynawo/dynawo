...
model BaseWPP "Base model for Wind Power Plants from IEC 61400-27-1 standard"
  ...
  parameter Types.CurrentComponent PPcc0Pu = 0 "Initial active power at the external bus controlled by the PPC (used when PPCLocal = false) in pu (base UNom, SnRef) (receptor convention) (only if the PCS is defined outside of the model)" annotation(
    Dialog(tab = "Operating point", enable = not PPCLocal));
  parameter Types.ReactivePowerPu Q0Pu "Initial reactive power at grid terminal in pu (base SnRef) (receptor convention)" annotation(
    Dialog(tab = "Operating point"));
  parameter Types.CurrentComponent QPcc0Pu = 0 "Initial reactive power at the external bus controlled by the PPC (used when PPCLocal = false) in pu (base UNom, SnRef) (receptor convention) (only if the PCS is defined outside of the model)" annotation(
    Dialog(tab = "Operating point", enable = not PPCLocal));
  ...
  final parameter Types.ComplexCurrentPu iControl0Pu = if PPCLocal then i0Pu else Modelica.ComplexMath.conj(Complex(PPcc0Pu, QPcc0Pu) / Modelica.ComplexMath.fromPolar(UPcc0Pu, UPccPhase0)) "Initial complex current to be controlled by the PPC coming either from the external bus or from the model's output terminal in pu (base UNom, SnRef) (receptor convention)";
  final parameter Types.ComplexCurrentPu iGs0Pu = Complex(GesPu, BesPu) * (uWt0Pu - Complex(ResPu, XesPu) * iWt0Pu * SystemBase.SnRef / SNom) - iWt0Pu * SystemBase.SnRef / SNom "Complex current at converter output in pu (base SNom) (generator convention)";
  final parameter Types.PerUnit Ip0Pu = cos(UPhaseWt0) * iGs0Pu.re + sin(UPhaseWt0) * iGs0Pu.im "Initial active current component at converter terminal in pu (base UNom, SNom) (generator convention)";
  final parameter Types.PerUnit IpMax0Pu = Modelica.Math.Vectors.interpolate(TableIpMaxUwt[:, 1], TableIpMaxUwt[:, 2], UWt0Pu) "Initial maximum active current at converter terminal in pu (base UNom, SNom) (generator convention)";
  final parameter Types.PerUnit Iq0Pu = cos(UPhaseWt0) * iGs0Pu.im - sin(UPhaseWt0) * iGs0Pu.re "Initial reactive current component at converter terminal in pu (base UNom, SNom) (generator convention)";
  final parameter Types.PerUnit IqMax0Pu = min(Modelica.Math.Vectors.interpolate(TableIqMaxUwt[:, 1], TableIqMaxUwt[:, 2], UWt0Pu), max(0, IMaxPu ^ 2 - min(IpMax0Pu, -PWt0Pu * SystemBase.SnRef / (SNom * UWt0Pu)) ^ 2) ^ 0.5) "Initial maximum reactive current at converter terminal in pu (base UNom, SNom) (generator convention)";
  final parameter Types.PerUnit IqMin0Pu = max(-IqMax0Pu, Kpqu * (UWt0Pu - UpquMaxPu)) "Initial minimum reactive current at converter terminal in pu (base UNom, SNom) (generator convention)";
  ...
  final parameter Types.ActivePowerPu PWt0Pu = Modelica.ComplexMath.real(uWt0Pu * Modelica.ComplexMath.conj(iWt0Pu));
  ...
  final parameter Types.ReactivePowerPu QMax0Pu = if QlConst then QMaxPu else min(Modelica.Math.Vectors.interpolate(TableQMaxUwtcFilt[:, 1], TableQMaxUwtcFilt[:, 2], UWt0Pu), Modelica.Math.Vectors.interpolate(TableQMaxPwtcFilt[:, 1], TableQMaxPwtcFilt[:, 2], -PWt0Pu * SystemBase.SnRef / SNom)) "Initial maximum reactive power at grid terminal in pu (base SNom) (generator convention)";
  final parameter Types.ReactivePowerPu QMin0Pu = if QlConst then QMinPu else max(Modelica.Math.Vectors.interpolate(TableQMinUwtcFilt[:, 1], TableQMinUwtcFilt[:, 2], UWt0Pu), Modelica.Math.Vectors.interpolate(TableQMinPwtcFilt[:, 1], TableQMinPwtcFilt[:, 2], -PWt0Pu * SystemBase.SnRef / SNom)) "Initial minimum reactive power at grid terminal in pu (base SNom) (generator convention)";
  final parameter Types.ReactivePowerPu QWt0Pu = Modelica.ComplexMath.imag(uWt0Pu * Modelica.ComplexMath.conj(iWt0Pu));
  ...
  final parameter Types.Angle UPhaseWt0 = Modelica.ComplexMath.arg(uWt0Pu);
  final parameter Types.VoltageModulePu UWt0Pu = Modelica.ComplexMath.'abs'(uWt0Pu);
  ...
  final parameter Types.VoltageModulePu UWt0DroppedPu = (((UWt0Pu + RDropPu * PWt0Pu + XDropPu * QWt0Pu) ^ 2 + (-XDropPu * PWt0Pu + RDropPu * QWt0Pu) ^ 2) ^ 0.5) * SystemBase.SnRef / (SNom * UWt0Pu) "Initial voltage magnitude controlled by the WT in pu (base UNom)";
  final parameter Types.PerUnit XWT0Pu = if MqG == 0 then UWt0DroppedPu - URef0Pu else -Iq0Pu * UWt0Pu "Initial reactive power or voltage reference at grid WT terminal in pu (base SNom or UNom) (generator convention)";
