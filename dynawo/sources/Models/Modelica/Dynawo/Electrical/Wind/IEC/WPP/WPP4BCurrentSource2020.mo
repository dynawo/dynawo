...
model WPP4BCurrentSource2020 "Wind Power Plant Type 4B model from IEC 61400-27-1:2020 standard : WT4B, communication modules"
  ...
  final parameter Types.VoltageModulePu UWpp0DroppedPu = ((U0Pu + RwpDropPu*P0Pu*SystemBase.SnRef/(SNom*U0Pu) + XwpDropPu*Q0Pu*SystemBase.SnRef/(SNom*U0Pu))^2 + (-XwpDropPu*P0Pu*SystemBase.SnRef/(SNom*U0Pu) + RwpDropPu*Q0Pu*SystemBase.SnRef/(SNom*U0Pu))^2)^0.5 "Initial voltage module at the output of the voltage drop block (This voltage is the one that is controlled in WPPQControl)";
  final parameter Types.PerUnit X0Pu = if MwpqMode == 0 then -Q0Pu*SystemBase.SnRef/SNom else if MwpqMode == 1 then Q0Pu/P0Pu else if MwpqMode == 2 then UWpp0DroppedPu + UErr0 else if MwpqMode == 3 then UWpp0DroppedPu - Kwpqu*Q0Pu*SystemBase.SnRef/SNom else 0;
...
equation
  ...
  connect(wT4BCurrentSource.terminal, PCS.terminal1) annotation(
    Line(points = {{42, 40}, {50, 40}, {50, 20}, {58, 20}}, color = {0, 0, 255}));
