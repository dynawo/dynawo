...
model WT3CurrentSource1 "WECC Wind Turbine model without plant controller and with a current source as interface with the grid and the mechanical controllers"
  ...
  Modelica.Blocks.Interfaces.RealInput omegaRefPu(start = SystemBase.omegaRef0Pu) "Frequency reference in pu (base omegaNom)" annotation(
    Placement(transformation(origin = {-190, 58}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {0, 110}, extent = {{-10, 10}, {10, -10}}, rotation = -90)));
  Modelica.Blocks.Interfaces.RealInput PConvRefPu(start = PConv0Pu) "Active power setpoint at converter terminal in pu (generator convention) (base SNom)" annotation(
    Placement(transformation(origin = {-190, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-110, 60}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealInput PFaRef(start = acos(PF0)) "Power factor angle reference in rad" annotation(
    Placement(transformation(origin = {-79, 120}, extent = {{-10, -10}, {10, 10}}, rotation = -90), iconTransformation(origin = {-110, -60}, extent = {{-10, -10}, {10, 10}})));
  ...
  Dynawo.Electrical.Controls.WECC.Mechanical.WTGQa wecc_wtgq(
    ...
    P4 = P4,
    Spd4 = Spd4,
    TorqueControlFileName = TorqueControlFileName,
    TorqueControlTableName = TorqueControlTableName,
    TorqueControlTableOnFile = TorqueControlTableOnFile,
    TFlag = TFlag,
    PConv0Pu = PConv0Pu,
    omegaRefWTGQPu0 = omegaRefWTGQPu0) annotation(
  ...
  Dynawo.Electrical.Controls.WECC.REEC.REECa wecc_reec(
    ...
    VDLIq41 = VDLIq41,
    VDLIq42 = VDLIq42,
    VDLFileName = VDLFileName,
    VDLIpTableName = VDLIpTableName,
    VDLIqTableName = VDLIqTableName,
    VDLTablesOnFile = VDLTablesOnFile,
    VDipPu = VDipPu,
    VFlag = VFlag,
    VMaxPu = VMaxPu,
    VMinPu = VMinPu,
...
equation
  ...
  connect(wecc_wtga.PmPu, wecc_wtgt.PmPu) annotation(
    Line(points = {{21, -46}, {52, -46}}, color = {0, 0, 127}));
  connect(wecc_wtgp.theta, wecc_wtga.theta) annotation(
    Line(points = {{-28, -46}, {-1, -46}}, color = {0, 0, 127}));
  ...
  connect(PmRefPu, wecc_wtga.PmRefPu) annotation(
    Line(points = {{15, -110}, {15, -57}}, color = {0, 0, 127}));
  connect(wecc_wtgq.PRefPu, wecc_wtgp.PRefPu) annotation(
    Line(points = {{-78, -52}, {-70, -52}, {-70, -66}, {-45, -66}, {-45, -57}}, color = {0, 0, 127}));
  ...
  connect(LvMeasurements.PPu, wecc_wtgq.PePu) annotation(
  ...
  connect(wecc_reec.frtOn, wecc_wtgq.freeze) annotation(
    Line(points = {{-68, 0}, {-64, 0}, {-64, -30}, {-87, -30}, {-87, -34}}, color = {255, 0, 255}));
  connect(PConvRefPu, wecc_wtgq.PRef0Pu) annotation(
    Line(points = {{-190, 20}, {-160, 20}, {-160, 6}, {-104, 6}, {-104, -40}, {-100, -40}}, color = {0, 0, 127}));

  annotation(
    preferredView = "diagram",
    Documentation(info = "<html><head></head><body><p style=\"font-family: 'MS Shell Dlg 2'; font-size: 12px;\">This block contains the generic WECC WT model according to (in case page cannot be found, copy link in browser):&nbsp;<br><a href=\"https://www.wecc.org/Reliability/WECC-Second-Generation-Wind-Turbine-Models-012314.pdf\">https://www.wecc.org/Reliability/WECC-Second-Generation-Wind-Turbine-Models-012314.pdf</a></p><p style=\"font-family: 'MS Shell Dlg 2'; font-size: 12px;\">The overall model is structured as follows:</p><ul style=\"font-family: 'MS Shell Dlg 2'; font-size: 12px;\"><li>Main model: WECC_Wind with terminal connection and measurement inputs for P/Q/U/I.&nbsp;</li><li>Electrical inverter control.</li><li>Generator control.&nbsp;</li><li>Injector (id,iq).</li><li>Torque control.</li><li>Pitch angle control.</li><li>Aero-Dynamic model.</li><li>Drive-train.</li></ul></body></html>"),
    Icon(graphics = {Text(origin = {-26, 11}, extent = {{-48, 27}, {98, -53}}, textString = "WECC WT 3 1"), Rectangle(extent = {{-100, 100}, {100, -100}})}, coordinateSystem(extent = {{-100, -100}, {100, 100}})),
