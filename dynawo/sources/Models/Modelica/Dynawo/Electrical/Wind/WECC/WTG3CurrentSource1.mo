...
model WTG3CurrentSource1 "WECC Wind Turbine model with a current source as interface with the grid and the mechanical controllers"
  ...
  Electrical.Controls.WECC.Mechanical.WTGPa wecc_wtgp(kiw = kiw, kpw = kpw, kic = kic, kpc = kpc, kcc = kcc, tTheta = tTheta, thetaMax = thetaMax, thetaMin = thetaMin, thetaRMax = thetaRMax, thetaRMin = thetaRMin, PInj0Pu = PInj0Pu, theta0 = theta0) annotation(
    Placement(transformation(origin = {12, -56}, extent = {{-10, -10}, {10, 10}})));
  Electrical.Controls.WECC.Mechanical.WTGAa wecc_wtga(theta0 = theta0, Ka = Ka, Pm0Pu = Pm0Pu) annotation(
    Placement(transformation(origin = {52, -56}, extent = {{-10, -10}, {10, 10}})));
  Electrical.Controls.WECC.Mechanical.WTGQa wecc_wtgq(kip = kip, kpp = kpp, tp = tp, tOmegaRef = tOmegaRef, tEMax = tEMax, tEMin = tEMin, p1 = p1, spd1 = spd1, p2 = p2, spd2 = spd2, p3 = p3, spd3 = spd3, p4 = p4, spd4 = spd4, tFlag = tFlag, PInj0Pu = PInj0Pu) annotation(
    Placement(transformation(origin = {-38, -56}, extent = {{-10, 10}, {10, -10}})));
  Electrical.Controls.WECC.Mechanical.WTGTa wecc_wtgt(Ht = Ht, Hg = Hg, Dshaft = Dshaft, Kshaft = Kshaft, PInj0Pu = PInj0Pu, Pm0Pu = Pm0Pu) annotation(
