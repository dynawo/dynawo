...
model BasePCS "Base model of the Power Collection System to be extended in the WECC models of power plants"
  ...
  Dynawo.Electrical.Controls.WECC.Utilities.Measurements HvMeasurements(SNom = SNom) annotation(
  ...
  Dynawo.Electrical.Transformers.TransformersFixedTap.TransformerFixedRatio HvTfo(BPu = BPcsPu*SNom/SystemBase.SnRef, GPu = GPcsPu*SNom/SystemBase.SnRef, RPu = RPcsPu*SystemBase.SnRef/SNom, XPu = XPcsPu*SystemBase.SnRef/SNom, rTfoPu = rTfoPu) annotation(
...
equation
  ...
  connect(HvMeasurements.terminal2, terminal) annotation(
    Line(points = {{120, 0}, {140, 0}}, color = {0, 0, 255}));
  connect(HvTfo.terminal1, HvMeasurements.terminal1) annotation(
