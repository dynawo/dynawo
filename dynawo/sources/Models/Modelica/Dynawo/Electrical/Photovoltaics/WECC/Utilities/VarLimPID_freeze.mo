...
block VarLimPID_freeze "P, PI, PD, and PID controller with limited output (with adjustable limits), anti-windup compensation, setpoint weighting, optional feed-forward and optional freezing of the state"
  ...
  Integrator_setFreeze I(initType = if initType == InitPID.SteadyState then Init.SteadyState else if initType == InitPID.InitialState or initType == InitPID.DoNotUse_InitialIntegratorState then Init.InitialState else Init.NoInit,k = unitTime / Ti, use_freeze = true, y_start = xi_start)  annotation(
