...
model PlantControllerPI
  ...
  final parameter Types.ReactivePowerPu QMaxPu = QMax / SystemBase.SnRef "Maximum plant level reactive power in pu (base SnRef)";
  final parameter Types.ReactivePowerPu QMinPu = QMin / SystemBase.SnRef "Minimum plant level reactive power in pu (base SnRef)";

  // Input variables
  Modelica.Blocks.Interfaces.BooleanInput freeze(start = false) annotation(
    Placement(transformation(origin = {100, 110}, extent = {{-10, -10}, {10, 10}}, rotation = -90), iconTransformation(origin = {-110, -40}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Interfaces.RealInput QRegPu(start = QReg0Pu) annotation(
    Placement(transformation(origin = {-40, 110}, extent = {{-10, -10}, {10, 10}}, rotation = -90), iconTransformation(origin = {1, 109}, extent = {{10, 10}, {-10, -10}}, rotation = 90)));
  ...
  // Output variables
  Modelica.Blocks.Interfaces.RealOutput PInjPu(start = PInj0Pu) "Active power at terminal in pu (base SnRef) (generator convention)" annotation(
    Placement(transformation(origin = {170, -40}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {110, 60}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Interfaces.RealOutput QInjPu(start = QInj0Pu) "Reactive power at terminal in pu (base SnRef) (generator convention)" annotation(
    Placement(transformation(origin = {170, 40}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {110, -60}, extent = {{-10, -10}, {10, 10}})));
  ...
  Modelica.Blocks.Math.Gain gain1(k = PNom/SystemBase.SnRef)  annotation(
  ...
  final parameter Types.PerUnit URef0Pu = U0Pu + LambdaPu * QReg0Pu "Start value of voltage setpoint for plant level control, calculated depending on VcompFlag, in pu (base UNom)";
...
equation
  when QVErrLim.y >= QMaxPu and pre(qStatus) <> QStatus.AbsorptionMax then
    qStatus = QStatus.AbsorptionMax;
    Timeline.logEvent1(TimelineKeys.GeneratorPVMaxQ);
  elsewhen QVErrLim.y <= QMinPu and pre(qStatus) <> QStatus.GenerationMax then
    qStatus = QStatus.GenerationMax;
    Timeline.logEvent1(TimelineKeys.GeneratorPVMinQ);
  elsewhen (QVErrLim.y < QMaxPu and pre(qStatus) == QStatus.AbsorptionMax) or (QVErrLim.y > QMinPu and pre(qStatus) == QStatus.GenerationMax) then
  ...
  connect(reactiveLimits.y, QVCtrlErr.u2) annotation(
    Line(points = {{-59, 40}, {-41, 40}, {-41, 34}, {-23, 34}}, color = {0, 0, 127}));
  ...
  connect(add3.y, limiter.u) annotation(
    Line(points = {{62, -40}, {98, -40}}, color = {0, 0, 127}));
  connect(limiter.y, PInjPu) annotation(
    Line(points = {{122, -40}, {170, -40}}, color = {0, 0, 127}));
  connect(QRegPu, baseQReg.u) annotation(
    Line(points = {{-40, 110}, {-40, 82}}, color = {0, 0, 127}));
  connect(baseQReg.y, QVCtrlErr.u1) annotation(
    Line(points = {{-40, 60}, {-40, 46}, {-22, 46}}, color = {0, 0, 127}));
