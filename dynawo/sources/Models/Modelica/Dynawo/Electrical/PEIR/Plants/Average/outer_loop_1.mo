...
model outer_loop_1
  ...
  Modelica.Blocks.Math.Division division annotation(
    Placement(transformation(origin = {29, 71}, extent = {{-21, -21}, {21, 21}})));
  Modelica.Blocks.Math.Division division1 annotation(
    Placement(transformation(origin = {14, 4}, extent = {{-14, -14}, {14, 14}})));
equation
...
// ── Outputs ──────────────────────────────────────────────────
  ...
  connect(division.u1, sum_node_p.y) annotation(
    Line(points = {{4, 84}, {-20, 84}, {-20, 82}}, color = {0, 0, 127}));
  connect(division.y, current_limiter_outer_loop.id_raw) annotation(
    Line(points = {{52, 72}, {86, 72}, {86, 62}}, color = {0, 0, 127}));
  connect(division.u2, V_meas) annotation(
    Line(points = {{4, 58}, {-220, 58}, {-220, -60}}, color = {0, 0, 127}));
  connect(sum_node_q.y, division1.u1) annotation(
    Line(points = {{-20, 18}, {-3, 18}, {-3, 12}}, color = {0, 0, 127}));
  connect(division1.u2, V_meas) annotation(
    Line(points = {{-3, -4}, {-220, -4}, {-220, -60}}, color = {0, 0, 127}));
  connect(division1.y, current_limiter_outer_loop.iq_raw) annotation(
    Line(points = {{30, 4}, {86, 4}, {86, 44}}, color = {0, 0, 127}));
  annotation(
