...
model current_loop
  ...
  // =============================================================================
  // ── Inputs ───────────────────────────────────────────────────
  Modelica.Blocks.Interfaces.RealInput i_d_ref(start = id_ref_0) "D-axis current reference from outer loop (pu)" annotation(
    Placement(transformation(origin = {-211, 99}, extent = {{-11, -11}, {11, 11}}), iconTransformation(origin = {-110, 34}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Interfaces.RealInput i_d_meas(start = id_meas_0) "D-axis measured current (pu)" annotation(
    Placement(transformation(origin = {-211, 79}, extent = {{-9, -9}, {9, 9}}), iconTransformation(origin = {-110, 72}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Interfaces.RealInput i_q_ref(start = -iq_ref_0) "Q-axis current reference from outer loop (pu)" annotation(
    Placement(transformation(origin = {-207, -61}, extent = {{-9, -9}, {9, 9}}), iconTransformation(origin = {-110, -20}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Interfaces.RealInput i_q_meas(start = iq_meas_0) "Q-axis measured current (pu)" annotation(
    Placement(transformation(origin = {-206, -80}, extent = {{-8, -8}, {8, 8}}), iconTransformation(origin = {-110, -58}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Interfaces.RealInput omega_pll(start = Omega0Pu) "Estimated grid frequency from PLL (pu) — used for cross-coupling decoupling" annotation(
    Placement(transformation(origin = {-208, 0}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {20, -110}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
  Modelica.Blocks.Interfaces.RealInput v_d(start = vd_0) "D-axis grid voltage feed-forward (pu) — active if voltagefeedforwardflag = 1" annotation(
    Placement(transformation(origin = {60, 212}, extent = {{-12, -12}, {12, 12}}, rotation = -90), iconTransformation(origin = {60, 110}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  Modelica.Blocks.Interfaces.RealInput v_q(start = vq_0) "Q-axis grid voltage feed-forward (pu) — active if voltagefeedforwardflag = 1" annotation(
    Placement(transformation(origin = {0, -212}, extent = {{-10, -10}, {10, 10}}, rotation = 90), iconTransformation(origin = {-40, 110}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  // ── Outputs ──────────────────────────────────────────────────
  Modelica.Blocks.Interfaces.RealOutput vm_d(start = vmd_0) "D-axis modulation voltage reference (pu)" annotation(
    Placement(transformation(origin = {210, 56}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {110, 32}, extent = {{-10, -10}, {10, 10}})));
  Modelica.Blocks.Interfaces.RealOutput vm_q(start = vmq_0) "Q-axis modulation voltage reference (pu)" annotation(
    Placement(transformation(origin = {210, -60}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {110, -50}, extent = {{-10, -10}, {10, 10}})));
...
// ── Connections
  ...
  annotation(
    ...
    Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}), graphics = {Rectangle(extent = {{-100, 100}, {100, -100}}, lineColor = {0, 0, 0}, fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid), Text(origin = {0, 0}, extent = {{-80, 20}, {80, -20}}, textString = "current loop")}),
