...
model GFLModel
  ...
  parameter Real L_g=LfPu+LPu "grid inductance (pu)";
  parameter Real   R_g=RfPu+RPu "grid resistance (pu)";
...
// ───────────────── Plant controller parameters ─────────────────
  ...
  parameter Real Theta0
    ...
    parameter Complex u0Pu_init =
  ComplexNumbers.Complex(UrPcc0Pu, UiPcc0Pu);

parameter Complex i0Pu_init =
  ComplexNumbers.Complex(IrPcc0Pu, IiPcc0Pu);

parameter Complex Z_g =
  ComplexNumbers.Complex(R_g, Omega0Pu*L_g);

parameter Complex uFilter0Pu_init =
  u0Pu_init + i0Pu_init*Z_g;

  // ── Sub-blocks ───────────────────────────────────────────────
  GFLControl control_GFL_ (u0Pu= u0Pu_init, i0Pu=i0Pu_init, uFilter0Pu=uFilter0Pu_init, P0Pu=P0_pcc, Q0Pu=Q0_pcc, Omega0Pu=Omega0Pu, k_p_d_current=k_p_d_current,k_i_d_current=k_i_d_current, k_p_q_current=k_p_q_current, k_i_q_current=k_i_q_current, L_g=L_g, R_g=R_g,  k_p_d_outer=k_p_d_outer,k_i_d_outer=k_i_d_outer, k_p_q_outer=k_p_q_outer, k_i_q_outer=k_i_q_outer, Imax=Imax, PQFlag=PQFlag, k_q_outer = k_q_outer, UboostStart = UboostStart, Kqv = Kqv, IqBoostMax = IqBoostMax, IqBoostMin = IqBoostMin, dPrefMaxPu = dPrefMaxPu, dQrefMaxPu = dQrefMaxPu, K_p_q_plant = K_p_q_plant, K_i_q_plant = K_i_q_plant, K_p_p_plant = K_p_p_plant, K_i_p_plant = K_i_p_plant, Lambda = Lambda, Kdroop_p = Kdroop_p, QMaxPu = QMaxPu, QMinPu = QMinPu, PMaxPu = PMaxPu, PMinPu = PMinPu, FEMaxPu = FEMaxPu, FEMinPu = FEMinPu, FDbd1Pu = FDbd1Pu, FDbd2Pu = FDbd2Pu, DbdPu = DbdPu, U0Pu = sqrt(UrPcc0Pu^2+UiPcc0Pu^2), Q0Pu_param = Q0_pcc, P0Pu_param = P0_pcc, K_p_pll = K_p_pll, K_i_pll = K_i_pll, OmegaMaxPu = OmegaMaxPu, OmegaMinPu = OmegaMinPu, Theta0 = Theta0) annotation(
  ...
  Dynawo.Electrical.Controls.PEIR.BaseControls.Average.VSC_with_pade_delay vsc_converter_delay_pade(
    tVSC      = tVSC,
    UdConv0Pu = UdConv0Pu,
    UqConv0Pu = UqConv0Pu) annotation(
  ...
  Dynawo.Connectors.ACPower terminal(
    ...
    Placement(transformation(origin = {184, 84}, extent = {{-10, -10}, {10, 10}}),
    iconTransformation(origin = {108, 2}, extent = {{-10, -10}, {10, 10}})));

  MeasurementBlock measurement_block1(
    k_filter = k_filter,
    T_filter = T_filter,
    U0_pcc   = sqrt(UrPcc0Pu^2+UiPcc0Pu^2),    // computed from UrPcc0Pu and UiPcc0Pu — not duplicated
    P0_pcc   = P0_pcc,
    Q0_pcc   = Q0_pcc,
    P0_conv  = P0_pcc+R_g*sqrt(IrPcc0Pu^2+IiPcc0Pu^2),
    Q0_conv  = Q0_conv+L_g*sqrt(IrPcc0Pu^2+IiPcc0Pu^2)) annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput u_ref_conv
    ...
    iconTransformation(origin = {-120, 10}, extent = {{-20, -20}, {20, 20}})));
  ...
  Modelica.Blocks.Interfaces.RealInput U_ref_pcc_pu
    ...
    iconTransformation(origin = {-120, 68}, extent = {{-20, -20}, {20, 20}})));
  ...
  Modelica.Blocks.Interfaces.RealInput omegaRefPu
    ...
    iconTransformation(origin = {-120, -48}, extent = {{-20, -20}, {20, 20}})));
  ...
  Modelica.Blocks.Interfaces.RealInput PRefu
    ...
    iconTransformation(origin = {-120, -90}, extent = {{-20, -20}, {20, 20}})));
