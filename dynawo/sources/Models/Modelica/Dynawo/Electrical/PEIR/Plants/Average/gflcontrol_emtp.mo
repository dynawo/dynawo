...
// =============================================================================
 ...
 parameter Real T_boost
  ...
  parameter Real Uq0Pu
    "Initial q-axis voltage at PCC in control dq frame (pu)";
      ...
      parameter Boolean k_direct_control_p  "true: bypass P PI, use i_d_ref = P_ref/V_meas | false: use PI regulator (default)";
  parameter Boolean k_direct_control_q  "true: bypass Q PI, use i_q_ref = Q_ref/V_meas | false: use PI regulator (default)";
   parameter Real U_filter_d_0 "Initial filter voltage magnitude (pu)";
   Modelica.Blocks.Interfaces.RealInput v_d (start=U_filter_d_0) annotation(
    Placement(transformation(origin = {-297, 57}, extent = {{-13, -13}, {13, 13}}), iconTransformation(origin = {109, -93}, extent = {{-9, -9}, {9, 9}}, rotation = 180)));
  // ────────────────────────────────────────────────────────────
  // Sub-blocks
  // ────────────────────────────────────────────────────────────
 ...
 Dynawo.Electrical.PEIR.Plants.Average.outer_loop_emtp outer_loop_GFL(k_p_d = k_p_d_outer, k_i_d = k_i_d_outer, k_p_q = k_p_q_outer, k_i_q = k_i_q_outer, Imax = Imax, y_start_outer_d = id_ref_0, y_start_outer_q = iq_ref_0, PQFlag = PQFlag, UboostHigh = UboostHigh, UboostLow = UboostLow, Kqv = Kqv, IqBoostMax = IqBoostMax, IqBoostMin = IqBoostMin, PInjPu0 = PInj0Pu, QInjPu0 = QInj0Pu, U_filter0 = U_LV0, i_d_ref_0 = id_ref_0, i_q_ref_0 = iq_ref_0, DyMax_pi_d = DyMax_pi_d, DyMax_pi_q = DyMax_pi_q, DuMax_idref = DuMax_idref, DuMin_idref = DuMin_idref, tS_idref = tS_idref, delay_time_plant = delay_time_plant, T_boost = T_boost, k_direct_control_p = k_direct_control_p, k_direct_control_q = k_direct_control_q) annotation(
...
// omegaPLL to plant controller (measured frequency for droop)
 ...
 connect(v_d, outer_loop_GFL.v_d) annotation(
    Line(points = {{-297, 57}, {-273, 57}, {-273, 56}, {-238, 56}}, color = {0, 0, 127}));
  annotation(
