...
model DynLine_switchOff "AC power line - Dynamic PI model"
...
parameter Types.PerUnit R_open = 1e8;
Types.PerUnit R_eff;
Real k(start = 1); // 1 = connected, 0 = disconnected
...
/*
  ...
  */
  ...
  R_eff = if running.value then RPu else R_open;


  LPu / SystemBase.omegaNom * der(terminal1.i.re) + R_eff * terminal1.i.re - omegaPu.value * LPu * terminal1.i.im = terminal1.V.re - terminal2.V.re;
  LPu / SystemBase.omegaNom * der(terminal1.i.im) + R_eff * terminal1.i.im + omegaPu.value * LPu * terminal1.i.re = terminal1.V.im - terminal2.V.im;
  terminal2.i = - terminal1.i;




  k = if running.value then 1 else 0;
