...
equation
  ...
  u0Pu = Complex(UsRe0Pu, UsIm0Pu) - Complex(ResPu, XesPu) * (-i0Pu * SystemBase.SnRef / SNom);
  Complex(IsRe0Pu, IsIm0Pu) = (-i0Pu * SystemBase.SnRef / SNom) + Complex(GesPu, BesPu) * Complex(UsRe0Pu, UsIm0Pu);
  u0Pu = Modelica.ComplexMath.fromPolar(U0Pu, UPhase0);
  connect(const1.y, combiTable1Ds.u) annotation(
    Line(points = {{-179, 60}, {-159, 60}, {-159, 80}, {-143, 80}}, color = {0, 0, 127}));
  connect(const1.y, combiTable1Ds1.u) annotation(
    Line(points = {{-179, 60}, {-159, 60}, {-159, 40}, {-143, 40}}, color = {0, 0, 127}));
  ...
  connect(division1.y, minMax.u[1]) annotation(
    Line(points = {{42, 20}, {60, 20}, {60, 80}, {70, 80}}, color = {0, 0, 127}));
  connect(iqLimitation.iQMaxPu, minMax.u[2]) annotation(
    Line(points = {{42, 80}, {70, 80}}, color = {0, 0, 127}));
  connect(realExpression.y, minMax.u[3]) annotation(
    Line(points = {{42, 96}, {60, 96}, {60, 80}, {70, 80}}, color = {0, 0, 127}));
  connect(minMax.yMin, switch11.u3) annotation(
    Line(points = {{92, 74}, {160, 74}, {160, 68}, {178, 68}}, color = {0, 0, 127}));
 connect(minMax.yMin, gain3.u) annotation(
    Line(points = {{92, 74}, {160, 74}, {160, 30}}, color = {0, 0, 127}));
  annotation(
    preferredView = "text",
