...
*/
  ...
  Modelica.Blocks.Interfaces.RealInput idPccPu(start = IdPcc0Pu) "d-axis current injected at the PCC in pu (base UNom, SNom) (generator convention)" annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput iqPccPu(start = IqPcc0Pu) "q-axis current injected at the PCC in pu (base UNom, SNom) (generator convention)" annotation(
  ...
  Types.PerUnit IConvPu(start = sqrt(IdConv0Pu ^ 2 + IqConv0Pu ^ 2)) "Module of the valve current (before filter) in pu (base UNom, SNom)";
...
equation
  ...
  if running then
    ...
    /* Converter Power in pu (base SNom) */
    ...
    /* Converter terminal voltage module in pu (base UNom) */
    UFilterPu = sqrt(udFilterPu ^ 2 + uqFilterPu ^ 2);

    /* Valve current module in pu (base UNom, SNom) */
