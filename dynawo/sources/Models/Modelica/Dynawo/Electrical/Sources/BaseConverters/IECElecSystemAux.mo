...
model IECElecSystemAux
  ...
  */
  ...
  Modelica.Blocks.Interfaces.RealInput iGsRePu(start = 0) "Real component of the current at the converter's terminals (generator system) in pu (Ubase, SNom) (generator convention)" annotation(
  ...
  Modelica.Blocks.Interfaces.RealOutput iWtRePu(start = 0) "Real component of the current at the wind turbine terminals in pu (Ubase, SNom) (generator convention)" annotation(
  ...
  Types.PerUnit iGsPu(start = sqrt(IGsIm0Pu^2)) "Module of the current at converter side in pu (base UNom, SNom)";

  /*Blocks*/
  Dynawo.Connectors.ACPower terminal(V(re(start = u0Pu.re), im(start = u0Pu.im)), i(re(start = 0), im(start = i0Pu.im))) "Connector used to connect the converter to the grid" annotation(
