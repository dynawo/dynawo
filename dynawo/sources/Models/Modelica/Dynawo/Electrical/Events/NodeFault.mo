...
model NodeFault "Node fault which lasts from tBegin to tEnd"
  ...
  */
  ...
  extends Dynawo.Electrical.Controls.Basics.SwitchOff.SwitchOffLogic(NbSwitchOffSignals = 1);
  ...
  parameter Boolean FaultOnNode = true "If true, fault on a node, if false, fault on a line";
...
equation
  ...
  when time >= tEnd and FaultOnNode then
    Timeline.logEvent1(TimelineKeys.NodeFaultEnd);
  elsewhen time >= tBegin and FaultOnNode then
    Timeline.logEvent1(TimelineKeys.NodeFaultBegin);
  end when;

  if nodeFault.value or switchOffSignal1.value then
