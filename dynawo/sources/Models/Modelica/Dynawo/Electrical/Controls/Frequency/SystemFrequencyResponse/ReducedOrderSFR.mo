...
model ReducedOrderSFR "Reduced order system frequency response model, based on the proposition by Anderson in 1990"
  ...
  Modelica.Blocks.Interfaces.RealInput PspPu(start = Psp0Pu) annotation(
    Placement(visible = true, transformation(origin = {-136, 30}, extent = {{-16, -16}, {16, 16}}, rotation = 0), iconTransformation(origin = {-120, 40}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealInput PePu(start = Pe0Pu) annotation(
    Placement(visible = true, transformation(origin = {24, 66}, extent = {{-14, -14}, {14, 14}}, rotation = -90), iconTransformation(origin = {-120, -40}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealOutput deltaFrequency(start = 0) annotation(
    Placement(visible = true, transformation(origin = {142, 20}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {110, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Math.Gain gain3(k = Dynawo.Electrical.SystemBase.fNom) annotation(
    Placement(visible = true, transformation(origin = {109, 19}, extent = {{-9, -9}, {9, 9}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealOutput deltaOmegaPu(start = 0) annotation(
    Placement(visible = true, transformation(origin = {142, -6}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {110, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));

  final parameter Types.ActivePowerPu Psp0Pu = Pe0Pu / Km;
...
equation
  ...
  connect(PspPu, add1.u1) annotation(
    Line(points = {{-136, 30}, {-104, 30}}, color = {0, 0, 127}));
  ...
  connect(PePu, add3.u1) annotation(
    Line(points = {{24, 66}, {24, 26}, {34, 26}}, color = {0, 0, 127}));
  ...
  connect(gain3.y, deltaFrequency) annotation(
    Line(points = {{118, 20}, {142, 20}}, color = {0, 0, 127}));
  connect(gain3.u, integrator.y) annotation(
    Line(points = {{98, 20}, {86, 20}}, color = {0, 0, 127}));
  ...
  connect(deltaOmegaPu, integrator.y) annotation(
    Line(points = {{142, -6}, {92, -6}, {92, 20}, {86, 20}}, color = {0, 0, 127}));
...
annotation(preferredView = "diagram",
  ...
  Icon(graphics = {Text(origin = {-33, 34}, extent = {{-59, 22}, {129, -88}}, textString = "ReducedOrderSFR"), Rectangle(extent = {{-100, 100}, {100, -100}})}));
