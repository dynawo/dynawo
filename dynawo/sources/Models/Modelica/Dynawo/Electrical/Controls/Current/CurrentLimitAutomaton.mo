...
model CurrentLimitAutomaton "Current Limit Automaton (CLA) monitoring one component"
  ...
  Dynawo.Connectors.ImPin IMonitored(value(start = 0)) "Monitored current (unit depending on IMax unit)";
...
equation
  when IMonitored.value > IMax and Running and pre(order.value) <> OrderToEmit and time > time0 then
  ...
  when tThresholdReached <> Constants.inf and tOrder == Constants.inf and der(IMonitored.value) < 0 and time > time0 then
  ...
  when time - tThresholdReached >= tLagBeforeActing and time > time0 then
