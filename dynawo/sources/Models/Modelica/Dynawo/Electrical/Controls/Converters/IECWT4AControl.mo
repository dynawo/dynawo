...
model IECWT4AControl "IEC Wind Turbine type 4A Control"
  ...
  Modelica.Blocks.Interfaces.RealInput PRefPu(start = -P0Pu * SystemBase.SnRef / SNom ) "Active power reference at the wind turbine terminals in pu (SNom) in generator convention" annotation(
    Placement(visible = true, transformation(origin = {-169, 134}, extent = {{-19, -19}, {19, 19}}, rotation = 0), iconTransformation(origin = {110, 53}, extent = {{10, -10}, {-10, 10}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealInput xRefPu(start = XWT0Pu) "Reactive power reference at the wind turbine terminals in pu (SNom) in generator convention" annotation(
  ...
  Modelica.Blocks.Interfaces.RealOutput ipCmdPu(start = -P0Pu * SystemBase.SnRef / (SNom * U0Pu)) "d-axis reference current at the generator system module (converter) terminal in pu (Ubase,SNom) in generator convention" annotation(
    Placement(visible = true, transformation(origin = {170, 130}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {-110, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 180)));
  ...
  Modelica.Blocks.Interfaces.RealOutput theta(start = UPhase0) "Phase shift between the converter's rotating frame and the grid rotating frame in radians" annotation(
    Placement(visible = true, transformation(origin = {170, -60.5}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {-40, 110}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
  Modelica.Blocks.Interfaces.BooleanOutput Focb(start=false) annotation(
    Placement(visible = true, transformation(origin = {169, 100}, extent = {{-19, -19}, {19, 19}}, rotation = 0), iconTransformation(origin = {40, 110}, extent = {{-10, -10}, {10, 10}}, rotation = 90)));
  ...
  Dynawo.Electrical.Controls.Converters.BaseControls.IECWT4APControl iECWT4APControl(DpMaxp4A = DpMaxp4A, DpRefMax4A = DpRefMax4A, DpRefMin4A = DpRefMin4A,IpMax0Pu = IpMax0Pu, MpUScale = MpUScale, P0Pu = P0Pu, SNom = SNom, TpOrdp4A = TpOrdp4A, TpWTRef4A = TpWTRef4A, U0Pu = U0Pu, UpDip = UpDip) annotation(
    Placement(visible = true, transformation(origin = {44.5, 114.5}, extent = {{-25.5, -25.5}, {25.5, 25.5}}, rotation = 0)));
  ...
  Dynawo.Electrical.Controls.Converters.BaseControls.GridProtectionSystem gridProtectionSystem(U0Pu = U0Pu, fOver = fOver, fUnder = fUnder, uOver = uOver, uUnder = uUnder)  annotation(
    Placement(visible = true, transformation(origin = {85, 55}, extent = {{-15, -15}, {15, 15}}, rotation = 0)));
equation
  connect(iECWT4ACurrentLimitation.iqMaxPu, iqMaxPu) annotation(
    Line(points = {{111, 0}, {140.5, 0}, {140.5, 20.5}, {169.5, 20.5}}, color = {0, 0, 127}));
  connect(iECWT4ACurrentLimitation.iqMinPu, iqMinPu) annotation(
    Line(points = {{111, -17}, {130, -17}, {130, -22}, {170, -22}}, color = {0, 0, 127}));
  connect(iECWT4ACurrentLimitation.ipMaxPu, ipMaxPu) annotation(
    Line(points = {{111, 17}, {130, 17}, {130, 59.5}, {170, 59.5}}, color = {0, 0, 127}));
  connect(iECWT4AMeasures.uWTCfiltPu, iECWT4ACurrentLimitation.uWTCfiltPu) annotation(
    Line(points = {{-66, 19}, {0, 19}, {0, 11}, {55, 11}}, color = {0, 0, 127}));
  connect(PRefPu, iECWT4APControl.pWTRefPu) annotation(
    Line(points = {{-169, 134}, {16, 134}}, color = {0, 0, 127}));
  connect(xRefPu, iECWT4AQControl.xWTRefPu) annotation(
  ...
  connect(iECWT4AQControl.Ffrt, iECWT4ACurrentLimitation.Ffrt) annotation(
    Line(points = {{39, -87}, {45, -87}, {45, -11}, {55, -11}}, color = {255, 127, 0}));
  ...
  connect(iECWT4AQLimitation.qWTMaxPu, iECWT4AQControl.qWTMaxPu) annotation(
    Line(points = {{-38, -114}, {-32, -114}, {-32, -107}, {-20, -107}}, color = {0, 0, 127}));
  connect(iECWT4AQLimitation.qWTMinPu, iECWT4AQControl.qWTMinPu) annotation(
    Line(points = {{-38, -130}, {-28, -130}, {-28, -115}, {-20, -115}}, color = {0, 0, 127}));
  connect(iECWT4AMeasures.uWTCPu, iECWT4APControl.uWTCPu) annotation(
    Line(points = {{-66, 37}, {-30, 37}, {-30, 121}, {16, 121}}, color = {0, 0, 127}));
  ...
  connect(iECWT4AMeasures.uWTCfiltPu, iECWT4APControl.uWTCfiltPu) annotation(
    Line(points = {{-66, 19}, {0, 19}, {0, 108}, {16, 108}}, color = {0, 0, 127}));
  connect(iECWT4AMeasures.omegaRefFiltPu, iECWT4ACurrentLimitation.OmegaPu) annotation(
    Line(points = {{-66, 9.5}, {-10, 9.5}, {-10, 0}, {55, 0}}, color = {0, 0, 127}));
  ...
  connect(iECWT4AQControl.iqCmdPu, iECWT4ACurrentLimitation.iqCmdPu) annotation(
    Line(points = {{39, -113}, {50, -113}, {50, -22}, {55, -22}}, color = {0, 0, 127}));
  ...
  connect(iECWT4AMeasures.uWTCPu, iECWT4APll.uWTCPu) annotation(
    Line(points = {{-66, 37}, {-30, 37}, {-30, -60}, {67, -60}}, color = {0, 0, 127}));
  connect(iECWT4AMeasures.uWTCfiltPu, iECWT4AQLimitation.uWTCfiltPu) annotation(
    Line(points = {{-66, 19}, {-50, 19}, {-50, -72}, {-90, -72}, {-90, -110}, {-82, -110}}, color = {0, 0, 127}));
  connect(iECWT4ACurrentLimitation.ipMaxPu, iECWT4APControl.ipMaxPu) annotation(
    Line(points = {{111, 17}, {130, 17}, {130, 35}, {10, 35}, {10, 95}, {16, 95}}, color = {0, 0, 127}));
  connect(iECWT4AMeasures.uWTCfiltPu, gridProtectionSystem.uWTPfiltPu) annotation(
    Line(points = {{-66, 19}, {-10, 19}, {-10, 61}, {68.5, 61}}, color = {0, 0, 127}));
  connect(iECWT4AMeasures.omegaRefFiltPu, gridProtectionSystem.omegaRefFiltPu) annotation(
    Line(points = {{-66, 9.5}, {-10, 9.5}, {-10, 49}, {68.5, 49}}, color = {0, 0, 127}));
  connect(iECWT4APControl.ipCmdPu, iECWT4ACurrentLimitation.ipCmdPu) annotation(
    Line(points = {{73, 114.5}, {89, 114.5}, {89, 80}, {30, 80}, {30, 23}, {55, 23}, {55, 22}}, color = {0, 0, 127}));
  connect(iECWT4APControl.ipCmdPu, ipCmdPu) annotation(
    Line(points = {{73, 114.5}, {89, 114.5}, {89, 130}, {170, 130}}, color = {0, 0, 127}));
  connect(gridProtectionSystem.Focb, Focb) annotation(
    Line(points = {{102, 55}, {115, 55}, {115, 100}, {169, 100}, {169, 100}}, color = {255, 0, 255}));

annotation(
    Diagram(coordinateSystem(grid = {1, 1}, extent = {{-150, -150}, {150, 150}})),
    preferredView = "diagram",
    Icon(coordinateSystem(grid = {1, 1}, initialScale = 0.1), graphics = {Rectangle(fillColor = {255, 255, 255}, fillPattern = FillPattern.Solid, extent = {{-100, 100}, {100, -100}}), Text(origin = {0, 30}, extent = {{-90, -30}, {90, 30}}, textString = "IEC WT4A"), Text(origin = {0, -30}, extent = {{-90, -30}, {90, 30}}, textString = "Control")}));
