...
model Comptodq "Computation of dq components from grid measurements in EPRI Grid Forming model"
  ...
  // Input variables
  Modelica.ComplexBlocks.Interfaces.ComplexInput iInjPu(re(start = -i0Pu.re), im(start = -i0Pu.im)) "Complex current in pu (base UNom, SnRef) (generator convention)" annotation(
  ...
  Modelica.Blocks.Interfaces.RealOutput iqConvPu(start = if OmegaFlag == 0 then - IqConv0Pu else IqConv0Pu) "Q-axis current of the converter in pu (base UNom, SNom) (generator convention)" annotation(
  ...
  Modelica.Blocks.Sources.RealExpression GFLMode(y = if OmegaFlag == 0 then -1 else 1)  annotation(
    Placement(visible = true, transformation(origin = {40, -60}, extent = {{-20, -10}, {20, 10}}, rotation = 0)));
...
equation
  ...
  connect(phi, multiSwitch.u[1]) annotation(
    Line(points = {{-280, 0}, {-151, 0}, {-151, -100}, {-18, -100}}, color = {0, 0, 127}));
  ...
  connect(changeOfBaseq.y, product.u1) annotation(
    Line(points = {{82, -20}, {87, -20}, {87, -14}, {92, -14}}, color = {0, 0, 127}));
  connect(product.y, iqConvPu) annotation(
    Line(points = {{116, -20}, {270, -20}}, color = {0, 0, 127}));
  connect(GFLMode.y, product.u2) annotation(
    Line(points = {{62, -60}, {86, -60}, {86, -26}, {92, -26}}, color = {0, 0, 127}));
  connect(product.y, iqSquare.u1) annotation(
    Line(points = {{116, -20}, {120, -20}, {120, -124}, {138, -124}}, color = {0, 0, 127}));
  connect(product.y, iqSquare.u2) annotation(
    Line(points = {{116, -20}, {120, -20}, {120, -136}, {138, -136}}, color = {0, 0, 127}));
