...
model OneGFM_InfiniteBus_VoltagePhase
  ...
  Modelica.Blocks.Sources.Step Pref(height = 0, offset = 0.93, startTime = 15) annotation(
...
equation
 ...
 // loadPQ.deltaP = pre(loadPQ.deltaP) + 0 ;//(base SnRef)
   loadPQ.deltaP = 1.4 ;//(base SnRef)
   lastVariation =time;
  end when;
  IgridModule=Modelica.ComplexMath.'abs'(line.terminal2.i);  


    if time > 5+SamplingTime  then
// PSteadyState = pre(PSteadyState)+ GFM.measurementPcc.idPccPu;
      PSteadyState=0.93;
      Ptheoric=DeltaPtheoric*exp((time-5-SamplingTime)*Thau)+PSteadyState;    
   // DeltaPtheoric=0;
   else
     PSteadyState=0.93;
  ...
  DeltaPtheoric=(sin(0.95)-sin(0.88))/(Xeff);  // PSteadyState= pre(PSteadyState);
  ...
  connect(Uref.y, GFM.UFilterRefPu) annotation(
    Line(points = {{-119, 38}, {-100, 38}, {-100, 60}, {-60, 60}}, color = {0, 0, 127}));
  ...
  connect(infiniteBus.terminal, line.terminal1) annotation(
    Line(points = {{16, -42}, {84, -42}}, color = {0, 0, 255}));
