...
model BridgeInverterRLC
 import Modelica;
  ...
  Modelica.Blocks.Interfaces.RealInput udConvRefPu(start = udConvRef0Pu) "d-axis voltage reference at the inverter bridge output in pu (base UNom, SNom)" annotation(
    Placement(visible = true, transformation(origin = {-208, 60}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {-110, 120}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealInput uqConvRefPu(start = uqConvRef0Pu) "q-axis voltage reference at the inverter bridge output in pu (base UNom, SNom)" annotation(
    Placement(visible = true, transformation(origin = {-202, -10}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {-110, 60}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  Modelica.Blocks.Interfaces.RealInput idPccPu(start = idPcc0Pu) "d-axis current at the grid connection point in pu (base UNom, SNom) (generator convention)" annotation(
    Placement(visible = true, transformation(origin = {-204, 28}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {-110, -20}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealInput iqPccPu(start = iqPcc0Pu) "q-axis current at the grid connection point in pu (base UNom, SNom) (generator convention)" annotation(
    Placement(visible = true, transformation(origin = {-200, -46}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {-110, -76}, extent = {{-10, 10}, {10, -10}}, rotation = 0)));
  ...
  Modelica.Blocks.Interfaces.RealInput omegaPu(start = omega0Pu) "angular frequency (omega) for the VSC system" annotation(
  ...
  parameter Types.PerUnit omega0Pu = SystemBase.omegaRef0Pu "Start value of the angular reference frequency for the VSC system (omega) ";
 ...
 // Types.Angle AngleUconv(start=0) "Angle of the Uinv source voltage";

    Types.PerUnit PConvPu "active power at the converter output";
    Types.PerUnit QConvPu "reactive power at the converter output";

    Types.PerUnit uConvRefModule "Module of the voltage reference";
    Types.PerUnit uConvRefPhase "Phase of the voltage reference";
equation
// VSC behavior
...
/* Power Calculation in base SNom, UNom (generator convetion)*/
 ...
 //Calcul of the Angle Uinv
  //AngleUconv = Modelica.ComplexMath.arg(Complex(udConvRefPu,uqConvRefPu));
  //Calcul of the magnitude and phase of the uConvRef
 uConvRefModule = sqrt(uqConvRefPu^2+udConvRefPu^2);
 uConvRefPhase = atan2(uqConvRefPu,udConvRefPu)*180/3.14;
