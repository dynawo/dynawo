...
model GFM_VSM_cc_v3Prope_ForRLFilterWithWashoutFilter "Grid Forming converters test case"
  /*
  ...
  parameter Types.PerUnit DampingPowerFactor = 25"DampingPower constant";
  parameter Types.PerUnit DampingAngleFactor =26 "DampingAngle constant";
  parameter Types.Time TimeHighPassFilter =1 "Time order in sec of the high order pass filter";
...
/*VoltageFilterReference*/
 ...
 //Droop Controls
  ...
  Modelica.Blocks.Interfaces.RealInput OmegaRefPu annotation(
    Placement(visible = true, transformation(origin = {-1926.5, 4050.5}, extent = {{-328.5, -328.5}, {328.5, 328.5}}, rotation = 0), iconTransformation(origin = {-114, -50}, extent = {{15, -15}, {-15, 15}}, rotation = 180)));
 ...
 Dynawo.Electrical.Controls.Converters.OuterControls.Synchronization.VirtualSynchroMachineFeedForward virtualSynchroMachine(DampingAngleFactor = DampingAngleFactor, DampingPowerFactor = DampingPowerFactor,H = H, PMesure0Pu = PMesure0Pu, PRef0Pu = PRef0Pu, TimeHighPassFilter = TimeHighPassFilter,   omega0Pu = omega0Pu,  omegaRef0Pu = omegaRef0Pu,  theta0 = theta0)  annotation(
    Placement(visible = true, transformation(origin = {2589.5, 3751.5}, extent = {{-956.5, -956.5}, {956.5, 956.5}}, rotation = 0)));
...
equation
 connect(virtualSynchroMachine.theta, measurementPcc.theta) annotation(
    Line(points = {{3642, 4172}, {29987, 4172}, {29987, 1222}}, color = {0, 0, 127}));
 ...
 connect(virtualSynchroMachine.omegaPu, omegaPu) annotation(
    Line(points = {{3642, 3407}, {9647, 3407}, {9647, 4619}, {20853, 4619}}, color = {0, 0, 127}));
 ...
 connect(virtualSynchroMachine.omegaPu, VSC.omegaPu) annotation(
    Line(points = {{3642, 3407}, {19627, 3407}, {19627, 1257}}, color = {0, 0, 127}));
 ...
 connect(virtualSynchroMachine.omegaPu, VoltageFilterControl.omegaPu) annotation(
    Line(points = {{3642, 3407}, {3675, 3407}, {3675, 2177}, {-2120, 2177}, {-2120, 1219}}, color = {0, 0, 127}));
 ...
 connect(switch1.y, virtualSynchroMachine.PRefPu) annotation(
    Line(points = {{46751, 8024}, {47368, 8024}, {47368, 9484}, {-514, 9484}, {-514, 3637}, {1537, 3637}}, color = {0, 0, 127}));
 ...
 connect(virtualSynchroMachine.theta, theta) annotation(
    Line(points = {{3642, 4172}, {6979, 4172}, {6979, 7071}, {17946, 7071}}, color = {0, 0, 127}));
 ...
 connect(OmegaRefPu, virtualSynchroMachine.omegaRefPu) annotation(
    Line(points = {{-1926, 4051}, {1537, 4051}, {1537, 4249}}, color = {0, 0, 127}));
 connect(virtualSynchroMachine.PMesurePu, measurementPcc.PGenPu) annotation(
    Line(points = {{1537, 3139}, {65, 3139}, {65, -1646}, {32281, -1646}, {32281, -416}, {31179, -416}}, color = {0, 0, 127}));
protected
  annotation(
    preferredView = "diagram",
    experiment(StartTime = 0, StopTime = 30, Tolerance = 1e-06, Interval = 0.001),
    __OpenModelica_simulationFlags(initialStepSize = "0.001", lv = "LOG_STATS", nls = "kinsol", s = "ida", nlsLS = "klu", maxIntegrationOrder = "2", maxStepSize = "10", emit_protected = "()"),
    Documentation(info = "<html><head></head><body><div><div><font face=\"Bitstream Charter\">Control structure taken from: ASSESSING COMPLIANCE OF POWER PARK &nbsp;MODULES WITH FUTURE GRID-FORMING &nbsp;CAPABILITY REQUIREMENTS paper. In summary there are the following blocs in cascade:</font></div></div><div><font face=\"Bitstream Charter\"><br></font></div><div><font face=\"Bitstream Charter\">Outer Loop (Send U reference filter)--&gt; QSEM (send i reference) --&gt;Current saturation --&gt; Current control --&gt; Delay (to model the converter) --&gt; Frame rotation --&gt; Voltage re,im of the VSC</font></div><div><font face=\"Bitstream Charter\"><br></font></div><div><font face=\"Bitstream Charter\">In case of ccControl Voltage re,im is given at the filter level</font></div><div><font face=\"Bitstream Charter\">In case of vc Control Voltage re,im is given before the filter &nbsp;</font></div><div><font face=\"Bitstream Charter\"><br></font></div><div><font face=\"Bitstream Charter\">Power Synchronisation, calculee frame rotation reference of the converter (Angle of the ocnverter)&nbsp;</font></div><div><font face=\"Bitstream Charter\"><br></font></div><div><font face=\"Bitstream Charter\"><b>CurrentSaturation and Virtual Impedance</b> blocs are well detailed in the report:&nbsp;WP3 - Control and Operation of a Grid with 100% Converter-Based Devices, Chapter III</font></div><div><font face=\"Bitstream Charter\"><b>Current-control&nbsp;</b>GFM is explained in the paper&nbsp;A Comparative Study of Grid-Forming Controls and &nbsp;their Effects on Small-Signal Stability&nbsp;</font><span style=\"font-family: 'Bitstream Charter';\">II.C</span></div><div><font face=\"Bitstream Charter\"><b>AVR, </b>to track Q* at PCC, it's a slow control (10s), it was added only to study the coflict between this control and the inner voltage control (V-Q droop) that is faster (qqs ms). It should be deactivated normally</font></div><div><span style=\"font-family: 'Bitstream Charter';\"><b>Outer Loop,</b>&nbsp;</span><font face=\"Bitstream Charter\"><b>V-Q(droop), </b>(Qref - Q at pcc)Mq+Ufilter_ref=Ufilter,&nbsp;</font></div><div><font face=\"Bitstream Charter\"><b>QSEM, </b>is between the modulated voltage (Voltage after the filter) and the PCC voltage. It is used to determine the references of the currents, therefore the impedance configured is Ltransformer and Rtransformer</font></div><div><font face=\"Bitstream Charter\"><b>Frame rotation</b>, is just a rotation of angle from dq to re,im or vicerversa</font></div><div><font face=\"Bitstream Charter\"><b>Keep Stability in case of Faults</b>, for this Pref is modified in case Upcc is lower than 0.95pu. then :</font></div><div><font face=\"Bitstream Charter\">Pref (updated dynamicaly)=Upcc*Pref (initial one)</font></div></body></html>"),
    Diagram(coordinateSystem(grid = {1, 1}, extent = {{-35410, 10050}, {47950, -8400}}), graphics = {Text(extent = {{2355, -975}, {2355, -975}}, textString = "text"), Text(origin = {-25812, 9885.5}, extent = {{-9603, -167}, {9605, 168}}, textString = "Ki=1, Kp=0 (Settling time esta a 1 sec)"), Text(origin = {-22786, 8863}, extent = {{-11237, 164}, {11237, -164}}, textString = "Ki=0.1, Kp=0 (control mas lento) - Settling time esta à 5s"), Text(origin = {-11310, -4048.5}, lineColor = {198, 70, 0}, extent = {{-4277, 750}, {4277, -749}}, textString = "Modif Qmesure=Qpcc and not longer Qfilter")}),
    Icon(coordinateSystem(extent = {{-100, -100}, {100, 100}}), graphics = {Text(origin = {-173, 99}, extent = {{-33, 19}, {33, -19}}, textString = "QrefPu"), Text(origin = {-175, 51}, extent = {{-33, 19}, {33, -19}}, textString = "UrefPu"), Text(origin = {-173, 11}, extent = {{-33, 19}, {33, -19}}, textString = "PrefPu"), Text(origin = {-179, -39}, extent = {{-45, 25}, {45, -25}}, textString = "OmegaRefPu"), Text(origin = {173, 29}, extent = {{-33, 19}, {33, -19}}, textString = "PCC"), Rectangle(extent = {{-100, 100}, {100, -100}}), Text(origin = {-5, 3}, extent = {{-69, 43}, {69, -43}}, textString = "GFM_VSM_CCQSEM"), Text(origin = {173, 93}, extent = {{-33, 19}, {33, -19}}, textString = "OmegaPu"), Text(origin = {190, -62}, extent = {{-50, 28}, {50, -28}}, textString = "Theta(rad)")}));
end GFM_VSM_cc_v3Prope_ForRLFilterWithWashoutFilter;
