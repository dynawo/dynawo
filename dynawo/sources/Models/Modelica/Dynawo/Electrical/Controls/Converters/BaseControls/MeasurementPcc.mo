...
model MeasurementPcc
 ...
 parameter Types.ApparentPowerModule SNom "Nominal apparent power module for the converter";
  ...
  Modelica.Blocks.Interfaces.RealInput theta(start = theta0) "phase shift between the converter's rotating frame and the grid rotating frame in rad" annotation(
    Placement(visible = true, transformation(origin = {-120, 60}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {0, 192}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  Modelica.Blocks.Interfaces.RealInput idPccPu(start = idPcc0Pu) "d-axis current at the grid connection point in pu (base UNom, SNom) (generator convention)" annotation(
    Placement(visible = true, transformation(origin = {-120, 2}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {-110, 10}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Interfaces.RealInput iqPccPu(start = iqPcc0Pu) "q-axis current at the grid connection point in pu (base UNom, SNom) (generator convention)" annotation(
    Placement(visible = true, transformation(origin = {-120, -46}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {-110, -44}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  Modelica.Blocks.Interfaces.RealOutput UModule annotation(
    Placement(visible = true, transformation(origin = {110, 94}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {110, 162}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
...
equation
/*Firdt order filter to mesure the voltage and the current*/
//der(yd) * 1/10 + yd = idPccPu;
//der(yq) * 1/10 + yq = iqPccPu;
...
//der(udPccPu)*1/W_Measures+udPccPu=udPccPuFilter;
//der(uqPccPu)*1/W_Measures+uqPccPu=uqPccPuFilter;
//der(idPccPu)*1/W_Measures+udPccPu=idPccPuFilter;
//der(iqPccPu)*1/W_Measures+udPccPu=iqPccPuFilter;
