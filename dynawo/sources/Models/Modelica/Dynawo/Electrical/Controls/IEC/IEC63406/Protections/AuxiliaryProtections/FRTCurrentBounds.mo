...
model FRTCurrentBounds "Current limitation during FRT (IEC63406)"
  ...
  //Input variables
  Modelica.Blocks.Interfaces.RealInput ipLVRTPrimPu(start = -P0Pu * SystemBase.SnRef / (SNom * U0Pu)) "Active current calculated by the LVRT module in pu (base UNom, SNom) and before saturation (generator convention)" annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput iqLVRTPrimPu(start = Q0Pu * SystemBase.SnRef / (SNom * U0Pu)) "Reactive current calculated by the LVRT module in pu (base UNom, SNom) and before saturation (receptor convention)" annotation(
    Placement(visible = true, transformation(origin = {-120, -40}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {-120, -40}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
...
equation
  ipMaxPu = if pqFRTFlag then min(sqrt(max((IMaxPu ^ 2 - iqLVRTPrimPu ^ 2), 0)), IPMaxPu) else min(IMaxPu, IPMaxPu);
  ipMinPu = if StorageFlag then (if pqFRTFlag then max(-sqrt(max((IMaxPu ^ 2 - iqLVRTPrimPu ^ 2), 0)), IPMinPu) else max(-IMaxPu, IPMinPu)) else 0;
