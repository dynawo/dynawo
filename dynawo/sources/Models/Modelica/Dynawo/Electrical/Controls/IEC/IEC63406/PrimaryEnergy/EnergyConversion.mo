...
or flywheel energy storage systems" annotation(
  ...
  //Input variables
  Modelica.Blocks.Interfaces.RealInput pPrimPu(start = -P0Pu) "Power from the primary energy, which should be specified by model users and can be time-varying to represent the variations of primary energy" annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput pMeasPu(start = -P0Pu) annotation(
  ...
  //Output variables
  Modelica.Blocks.Interfaces.RealOutput pAvailInPu(start = P0Pu) annotation(
  ...
  Modelica.Blocks.Interfaces.RealOutput pAvailOutPu(start = -P0Pu) annotation(
    Placement(visible = true, transformation(origin = {110, 40}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {110, -34}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));

  Dynawo.Electrical.Controls.IEC.IEC63406.PrimaryEnergy.Auxiliary.StorageSys storageSys(PMaxPu = PMaxPu, SNom = SNom, SOCFlag = SOCFlag, SOCInit = SOCInit, SOCMax = SOCMax, SOCMin = SOCMin, Tess = Tess)  annotation(
  ...
  Dynawo.NonElectrical.Blocks.NonLinear.LimitedFirstOrder limitedFirstOrder(Y0 = -P0Pu, YMax = PMaxPu, YMin = 0, tFilter = Tconv) annotation(
    Placement(visible = true, transformation(origin = {-20, 40}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
...
equation
  ...
  connect(pMeasPu, storageSys.pMeasPu) annotation(
    Line(points = {{-122, -40}, {-82, -40}, {-82, -28}, {-44, -28}}, color = {0, 0, 127}));
