...
model PControl
  ...
  //Input variables
  Modelica.Blocks.Interfaces.RealInput pFFRPu(start=PFFR0Pu) "Measured frequency outputted by the phase-locked loop" annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput pAvailInPu(start=PAvailIn0Pu) "Minimum output electrical power available to the active power control module" annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput pRefPu(start=-P0Pu) "Active power reference provided by the plant controller" annotation(
  ...
  //Output variables
  Modelica.Blocks.Interfaces.RealOutput iPcmdPu(start = -P0Pu * SystemBase.SnRef / (SNom * U0Pu)) "Active current command" annotation(
  ...
  Modelica.Blocks.Continuous.FirstOrder firstOrder(T = TpRef, y_start = -P0Pu) annotation(
