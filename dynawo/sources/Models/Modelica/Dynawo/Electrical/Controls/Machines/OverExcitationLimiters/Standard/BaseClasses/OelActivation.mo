...
model OelActivation
  ...
  //Input variables
  Modelica.Blocks.Interfaces.RealInput IAct(start = 0) "Input signal" annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput IRefPu(start = IRef0Pu) "Input signal" annotation(
  ...
  //Output variable
  Modelica.Blocks.Interfaces.RealOutput IBias(start = 0) "Output signal" annotation(
...
equation
  when tErr <= 0 or timer.y >= tEn then
    IBias = 0;
  elsewhen IRefPu >= IInst and timer1.y > tOff then
    IBias = if tEn == 0 then 0 else IReset;
  end when;

  timer.u = IAct > IRefPu;
  timer1.u = IRefPu > IAct + IThOff;
