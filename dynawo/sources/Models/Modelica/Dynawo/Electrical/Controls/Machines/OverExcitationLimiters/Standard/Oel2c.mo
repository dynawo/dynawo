...
model Oel2c "IEEE overexcitation limiter type OEL2C model"
  ...
  parameter Types.PerUnit ILim "OEL thermal field current limit";
  parameter Types.PerUnit IReset "OEL reset-reference, if OEL is inactive";
  parameter Types.PerUnit ITfPu "OEL reference for inverse time calculations";
  ...
  parameter Types.VoltageModulePu VInvMaxPu "OEL maximum inverse time output";
  parameter Types.VoltageModulePu VInvMinPu "OEL minimum inverse time output";
  ...
  parameter Types.VoltageModulePu VOel2MaxPu "Maximum OEL first lead-lag output limit";
  parameter Types.VoltageModulePu VOel2MinPu "Minimum OEL first lead-lag output limit";
  ...
  Modelica.Blocks.Continuous.FirstOrder firstOrder(T = tROel, k = KScale, y_start = KScale * Input0Pu) annotation(
  ...
  final parameter Types.CurrentModulePu IRef0Pu = KAct * KScale * Input0Pu "Initial reference current";
  final parameter Types.CurrentModulePu IScaled0Pu = KScale * Input0Pu / ITfPu "Initial scaled current";
  final parameter Types.Time tErr0 = tFcl - (K2 * (IScaled0Pu ^ C2 - 1) + (if IScaled0Pu <= 1 then FixedRu else FixedRd)) / KFb;
...
equation
  ...
  connect(gain4.y, oelActivation.IAct) annotation(
    Line(points = {{-79, 80}, {-60, 80}, {-60, 16}, {-44, 16}}, color = {0, 0, 127}));
  connect(oelActivation.IBias, add3.u2) annotation(
