...
equation
  ...
  when omegaMonitoredPu <= OmegaMinPu and not
                                             (pre(switchOffSignal)) then
    tThresholdReachedMin = time;
  elsewhen omegaMonitoredPu > OmegaMinPu and pre(tThresholdReachedMin) <> Constants.inf and not
                                                                                               (pre(switchOffSignal)) then
  ...
  when omegaMonitoredPu >= OmegaMaxPu and not
                                             (pre(switchOffSignal)) then
    tThresholdReachedMax = time;
  elsewhen omegaMonitoredPu < OmegaMaxPu and pre(tThresholdReachedMax) <> Constants.inf and not
                                                                                               (pre(switchOffSignal)) then
