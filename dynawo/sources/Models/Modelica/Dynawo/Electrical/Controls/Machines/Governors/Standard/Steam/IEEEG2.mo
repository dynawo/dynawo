...
model IEEEG2 "IEEE governor type IEEEG2"
  ...
  parameter Types.Time t4 "Governor lead time constant in s";
  parameter Types.Time t5 "Governor lag time constant in s";

  //Input variables
  Modelica.Blocks.Interfaces.RealInput omegaPu(start = SystemBase.omega0Pu) "Angular frequency in pu (base omegaNom)" annotation(
    Placement(visible = true, transformation(origin = {-180, -40}, extent = {{-20, -20}, {20, 20}}, rotation = 0), iconTransformation(origin = {-120, -1.77636e-15}, extent = {{-20, -20}, {20, 20}}, rotation = 0)));
  ...
  Modelica.Blocks.Continuous.TransferFunction transferFunction(a = {t5, 1}, b = {t4, 1}, x_scaled(start = {Pm0Pu}), x_start = {Pm0Pu}, y(start = Pm0Pu)) annotation(
