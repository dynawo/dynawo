...
model Scl2c "IEEE stator current limiter type SCL2C model"

  //Regulation parameters
  parameter Types.PerUnit C1 "SCL exponent for calculation of I ERRinv1";
  parameter Types.PerUnit C2 "SCL exponent for calculation of I ERRinv2";
  ...
  parameter Types.CurrentModulePu IInst "SCL instantaneous stator current limit";
  parameter Types.CurrentModulePu IInstUel "Underexcited region instantaneous stator current limit";
  parameter Types.CurrentModulePu ILim "SCL thermal stator current limit";
  parameter Types.CurrentModulePu IqOelMinPu "SCL OEL minimum reactive current reference value";
  parameter Types.CurrentModulePu IqUelMaxPu "SCL UEL maximum reactive current reference value";
  parameter Types.CurrentModulePu IReset "SCL reset-reference, if inactive";
  parameter Types.CurrentModulePu ITfPu "SCL thermal reference for inverse time calculations";
  parameter Types.CurrentModulePu IThOff "SCL reset threshold value";
  parameter Types.PerUnit K1 "SCL gain for calculation of I ERRinv1";
  parameter Types.PerUnit K2 "SCL gain for calculation of I ERRinv2";
  ...
  parameter Types.PerUnit Krd "SCL reference ramp-down rate";
  parameter Types.PerUnit Kru "SCL reference ramp-up rate";
  ...
  parameter Types.VoltageModulePu VInvMaxPu "SCL maximum inverse time output";
  parameter Types.VoltageModulePu VInvMinPu "SCL minimum inverse time output";
  parameter Types.VoltageModulePu VOel1MaxPu "Maximum OEL output limit";
  parameter Types.VoltageModulePu VOel1MinPu "Minimum OEL output limit";
  parameter Types.VoltageModulePu VOel2MaxPu "Maximum OEL lead-lag 1 output limit";
  parameter Types.VoltageModulePu VOel2MinPu "Minimum OEL lead-lag 1 output limit";
  parameter Types.VoltageModulePu VOel3MaxPu "Maximum OEL PID output limit";
  parameter Types.VoltageModulePu VOel3MinPu "Minimum OEL PID output limit";
  ...
  parameter Types.VoltageModulePu VUel1MaxPu "Maximum UEL output limit";
  parameter Types.VoltageModulePu VUel1MinPu "Minimum UEL output limit";
  parameter Types.VoltageModulePu VUel2MaxPu "Maximum UEL lead-lag 1 output limit";
  parameter Types.VoltageModulePu VUel2MinPu "Minimum UEL lead-lag 1 output limit";
  parameter Types.VoltageModulePu VUel3MaxPu "Maximum UEL PID output limit";
  parameter Types.VoltageModulePu VUel3MinPu "Minimum UEL PID output limit";
  ...
  Dynawo.NonElectrical.Blocks.NonLinear.LimitedLeadLag limitedLeadLag2(YMax = VUel2MaxPu, YMin = VUel2MinPu, t1 = tC2Oel, t2 = tB2Oel) annotation(
    Placement(visible = true, transformation(origin = {310, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Dynawo.NonElectrical.Blocks.NonLinear.LimitedLeadLag limitedLeadLag3(YMax = VUel1MaxPu, YMin = VUel1MinPu, t1 = tC1Oel, t2 = tB1Oel) annotation(
  ...
  Modelica.Blocks.Nonlinear.Limiter limiter(uMax = IInstUel, uMin = -999) annotation(
  ...
  final parameter Types.CurrentModulePu IRef0Pu = sqrt((PGen0Pu ^ 2 + QGen0Pu ^ 2) / (Modelica.ComplexMath.'abs'(ut0Pu) ^ 2)) "Initial reference current";
  final parameter Types.CurrentModulePu IScaled0Pu = Modelica.ComplexMath.'abs'(it0Pu) / ITfPu "Initial scaled current";
  final parameter Types.Time tErr0 = tScl - (K2 * (IScaled0Pu ^ C2 - 1) + (if IScaled0Pu <= 1 then FixedRu else FixedRd)) / KFb;
...
equation
  ...
  connect(sclUelActivation.IUelBias, add3.u1) annotation(
    Line(points = {{122, 160}, {140, 160}, {140, 128}, {158, 128}}, color = {0, 0, 127}));
  connect(limiter.y, sclUelActivation.IUelRef) annotation(
    Line(points = {{41, 100}, {60, 100}, {60, 144}, {76, 144}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(pythagoras.y, sclUelActivation.IUelAct) annotation(
  ...
  connect(sclReferenceCurrent.tErr, sclOelActivation.tErr) annotation(
    Line(points = {{-78, -128}, {-60, -128}, {-60, -60}, {60, -60}, {60, -80}, {76, -80}}, color = {0, 0, 127}, pattern = LinePattern.Dot));
  ...
  connect(pythagoras1.y, sclOelActivation.IOelAct) annotation(
    Line(points = {{-159, -80}, {20, -80}, {20, -64}, {75, -64}}, color = {0, 0, 127}));
  connect(firstOrder7.y, sclOelActivation.IOelRef) annotation(
    Line(points = {{-19, -140}, {60, -140}, {60, -96}, {76, -96}}, color = {0, 0, 127}, pattern = LinePattern.Dash));
  connect(sclOelActivation.IOelBias, add31.u1) annotation(
