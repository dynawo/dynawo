...
model GovHydro4 "Governor type GovHydro4"
  ...
  parameter Types.Frequency DeltaOmegaDbPu "Frequency Intentional deadband width in pu, typical: 0" annotation(
    Dialog(tab = "General parameters"));
  parameter Types.Frequency DeltaOmegaEpsPu "Frequency Intentional db hysteresis in pu, typical: 0" annotation(
    Dialog(tab = "General parameters"));
  parameter Types.PerUnit DeltaPDbPu "ActivePower Unintentional dead-band in pu, typical: 0" annotation(
    Dialog(tab = "General parameters"));
  parameter Types.PerUnit DTurb "Turbine damping factor in pu, typical: 0.5 (simple turbine) or 1.1 (real turbine) (if ModelInt == 0 then 0.5 else 1.1)" annotation(
    Dialog(tab = "General parameters"));
  parameter Types.PerUnit GMax "Maximum gate opening in pu of MWbase, typical: 1" annotation(
    Dialog(tab = "General parameters"));
  parameter Types.PerUnit GMin "Minimum gate opening in pu of MWbase, typical: 0" annotation(
    Dialog(tab = "General parameters"));
  ...
  // inputs, outputs
  Modelica.Blocks.Interfaces.RealInput OmegaPu "Rotor speed in pu (base: OmegaNom)" annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput OmegaRefPu "Rotor speed reference in pu (base: OmegaNom)" annotation(
  ...
  Modelica.Blocks.Interfaces.RealOutput PmPu "Mechanical power in pu (base: PNomTurb)" annotation(
  ...
  Modelica.Blocks.Continuous.TransferFunction tfRTemp(a = {tR, 1}, b = {RTemp * tR, 0}, initType = Modelica.Blocks.Types.Init.InitialState, x_start = {InitSet}, y_start = 0) annotation(
...
equation
  ...
  connect(tfRTemp.y, addFeedback.u1) annotation(
    Line(points = {{3, 56}, {-152, 56}, {-152, 98}}, color = {0, 0, 127}));
  ...
  connect(dbPower.y, tfRTemp.u) annotation(
    Line(points = {{90, 140}, {112, 140}, {112, 56}, {26, 56}}, color = {0, 0, 127}));
