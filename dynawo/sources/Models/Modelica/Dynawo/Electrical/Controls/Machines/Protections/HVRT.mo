...
model HVRT "High-voltage ride-through protection"
  ...
  parameter Types.VoltageModulePu U0Pu "Initial voltage in pu (base UNom)";
...
protected
  Types.Time tThresholdReached(start = Modelica.Constants.inf) "Time when the threshold was reached";

equation
  when firstOrder.y >= UOverPu and not (pre(switchOffSignal.value)) then
    Timeline.logEvent1(TimelineKeys.HVRTArming);
    tThresholdReached = time;
  elsewhen (not (firstOrder.y >= UOverPu)) and pre(tThresholdReached) <> Modelica.Constants.inf and not (pre(switchOffSignal.value)) then
  ...
  when firstOrder.y >= UOverPu then
    timer.u = true;
  end when;
