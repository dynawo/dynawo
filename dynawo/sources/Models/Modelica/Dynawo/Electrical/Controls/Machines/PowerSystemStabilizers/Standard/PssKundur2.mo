...
model PssKundur2 "Power system stabilizer based on Kundur's book"
  ...
  parameter Types.Time t3 "Phase compensation lead time constant in s";
  parameter Types.Time t4 "Phase compensation lag time constant in s";
  ...
  Dynawo.NonElectrical.Blocks.Continuous.Washout washout(U0 = 0, tW = tW) annotation(
  ...
  Modelica.Blocks.Continuous.TransferFunction transferFunction(a = {t4, 1}, b = {t3, 1}, y_start = 0) annotation(
...
equation
  ...
  connect(phaseCompensation.y, transferFunction.u) annotation(
    Line(points = {{62, 0}, {76, 0}}, color = {0, 0, 127}));
  connect(transferFunction.y, UPssPu) annotation(
    Line(points = {{100, 0}, {130, 0}}, color = {0, 0, 127}));
