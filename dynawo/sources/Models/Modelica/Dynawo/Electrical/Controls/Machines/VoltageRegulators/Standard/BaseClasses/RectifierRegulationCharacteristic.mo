...
equation
  ...
  annotation(
    ...
    Documentation(info = "<html><head></head><body>This characteristic function is defined by five different modes :<div><ul><li>Two constant modes (below 0 and above 1)</li><li>Two linear modes (between 0 and ULow, between UHigh and 1)</li><li>One non-linear mode (between ULow and UHigh)</li></ul>The function is continuous thanks to the adjusted coefficients A1 and A2.</div><div><br></div><div>With the default values of UHigh and ULow :</div><div><ul><li>A1 = sqrt(3) / 3</li><li>A2 = sqrt(3)</li><li>The characteristic is a symmetry : for any value of u, y(y(u)) = u</li></ul></div></body></html>"));
