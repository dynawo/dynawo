...
model DynCurrentLoopAntiWindUp "Current loop control for grid forming and grid following converters"
  /*
  ...
  */
  ...
  Modelica.Blocks.Interfaces.RealInput idConvRefPu(start = IdConvRef0Pu) "d-axis current reference in the converter in pu (base UNom, SNom) (generator convention)" annotation(
  ...
  Modelica.Blocks.Interfaces.RealInput iqConvRefPu(start = IqConvRef0Pu) "q-axis current reference in the converter in pu (base UNom, SNom) (generator convention)" annotation(
    Placement(visible = true, transformation(origin = {-150, -80}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-110, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  NonElectrical.Blocks.Continuous.PIAntiWindup pIAntiWindupd(Ki = Kic, Kp = Kpc, Y0 = UdConv0Pu - Kfd*UdFilter0Pu + LFilter*Omega0Pu*IqConv0Pu, YMax = YMax, YMin = YMin)  annotation(
    Placement(transformation(origin = {-49, 81}, extent = {{-10, -10}, {10, 10}})));
  NonElectrical.Blocks.Continuous.PIAntiWindup pIAntiWindupq(Ki = Kic, Kp = Kpc, Y0 = UqConv0Pu - Kfq*UqFilter0Pu - LFilter*Omega0Pu*IdConv0Pu, YMax = YMax, YMin = YMin)  annotation(
    Placement(transformation(origin = {-56, -80}, extent = {{-10, -10}, {10, 10}})));
equation
  ...
  connect(feedbackd.y, pIAntiWindupd.u) annotation(
    Line(points = {{-111, 80}, {-61, 80}, {-61, 81}}, color = {0, 0, 127}));
  connect(pIAntiWindupd.y, feedbackLwq.u1) annotation(
    Line(points = {{-38, 81}, {37, 81}, {37, 86}, {38, 86}}, color = {0, 0, 127}));
  connect(feedbackq.y, pIAntiWindupq.u) annotation(
    Line(points = {{-111, -80}, {-68, -80}}, color = {0, 0, 127}));
  connect(pIAntiWindupq.y, feedbackLwd.u2) annotation(
    Line(points = {{-45, -80}, {38, -80}, {38, -86}}, color = {0, 0, 127}));
  annotation(
