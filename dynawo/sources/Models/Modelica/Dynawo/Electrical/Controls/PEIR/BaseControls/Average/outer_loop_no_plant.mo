...
model outer_loop_no_plant
  ...
  //   - Direct mode (orange):        P_ref/V or U_ref/V → current ref directly
  //
  ...
  // k_direct_control_q = false  →  bypass Q PI, use i_q_ref = U_ref / V_meas
  // =============================================================================
  // ── Inputs ───────────────────────────────────────────────────
  ...
  Modelica.Blocks.Interfaces.RealInput U_ref(start = U_filter0) annotation(
  ...
  // ── Sum nodes (P_ref - P_meas, U_ref - Q_meas) ───────────────
  ...
  parameter Boolean k_direct_control_q = false "true: bypass Q PI, use i_q_ref = U_ref/V_meas | false: use PI regulator (default)";
...
// ── Current limiter → i_q_ref ────────────────────────────────
  ...
  connect(division1.u1, P_ref) annotation(
    Line(points = {{-78, 48}, {-178, 48}, {-178, 82}, {-220, 82}}, color = {0, 0, 127}));
  connect(pi_controller_antiwind1.y, current_limiter_outer_loop.iq_raw) annotation(
    Line(points = {{-2, 20}, {42, 20}, {42, 44}, {86, 44}, {86, 44}}, color = {0, 0, 127}));
  connect(U_ref, sum_node_q.u1) annotation(
    Line(points = {{-220, 18}, {-84, 18}, {-84, 20}, {-82, 20}}, color = {0, 0, 127}));
  connect(sum_node_q.u2, V_meas) annotation(
    Line(points = {{-74, 12}, {-70, 12}, {-70, -60}, {-220, -60}}, color = {0, 0, 127}));
  annotation(
...
Orange path: direct control via P_ref/V and U_ref/V.
...
k_direct_control_p/q = true  → direct (bypass PI)")}),
    Documentation(info = "<html>
      ...
      <ul>
        ...
        <li><b>Orange path:</b> stateless direct control — i_d = P_ref/V, i_q = U_ref/V.</li>
      </ul>
