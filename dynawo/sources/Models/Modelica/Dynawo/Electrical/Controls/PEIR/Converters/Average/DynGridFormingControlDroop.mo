...
model DynGridFormingControlDroop
  ...
  parameter Types.PerUnit WfDroop "Cutoff pulsation of the active filter (in rad/s)";

  // Voltage reference control parameters
  parameter Types.PerUnit Mq "Reactive power droop control coefficient";
  parameter Types.PerUnit Wf "Cutoff pulsation of the reactive filter (in rad/s)";
  ...
  Modelica.Blocks.Interfaces.RealInput UFilterRefPu(start = URef0Pu) "Voltage module reference at the converter's capacitor in pu (base UNom)" annotation(
  ...
  Dynawo.Electrical.Controls.PEIR.BaseControls.GFM.PowerAngleControls.DroopControl droopControl(Mp = Mp, Omega0Pu = Omega0Pu, PFilter0Pu = PFilter0Pu, Theta0 = Theta0, Wf = WfDroop)  annotation(
    Placement(visible = true, transformation(origin = {-48, 78}, extent = {{-16, -16}, {16, 16}}, rotation = 0)));
...
equation
  ...
  connect(voltageReferenceControl.uqRefPu, QSEM.uqFilterRefPu) annotation(
    Line(points = {{-20, 17}, {-20, 18.44}, {-2.4, 18.44}}, color = {85, 170, 0}, thickness = 0.5));
  connect(UFilterRefPu, voltageReferenceControl.URefPu) annotation(
  ...
  connect(droopControl.theta, theta) annotation(
    Line(points = {{-30, 84}, {106, 84}}, color = {0, 0, 127}));
  connect(droopControl.omegaPu, omegaPu) annotation(
    Line(points = {{-30, 72}, {106, 72}}, color = {0, 0, 127}));
  connect(droopControl.omegaPu, QSEM.omegaPu) annotation(
    Line(points = {{-30, 72}, {16, 72}, {16, 42}}, color = {0, 0, 127}));
  connect(droopControl.omegaPu, currentLoop.omegaPu) annotation(
    Line(points = {{-30, 72}, {76, 72}, {76, 42}}, color = {0, 0, 127}));
  connect(omegaRefPu, droopControl.omegaRefPu) annotation(
    Line(points = {{-108, 96}, {-74, 96}, {-74, 90}, {-66, 90}}, color = {0, 0, 127}, thickness = 0.5));
  connect(omegaSetPu, droopControl.omegaSetPu) annotation(
    Line(points = {{-108, 60}, {-74, 60}, {-74, 66}, {-66, 66}}, color = {0, 0, 127}));
  connect(droopControl.PFilterRefPu, PFilterRefPu) annotation(
    Line(points = {{-66, 84}, {-108, 84}}, color = {85, 170, 0}, thickness = 0.5));
  connect(PFilterPu, droopControl.PFilterPu) annotation(
    Line(points = {{-108, 72}, {-66, 72}}, color = {85, 170, 0}));
