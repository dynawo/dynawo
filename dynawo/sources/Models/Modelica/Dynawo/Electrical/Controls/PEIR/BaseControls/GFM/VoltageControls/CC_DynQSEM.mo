...
model CC_DynQSEM "Quasi-Static Electrical Model"
  /*
  ...
  */
  ...
  parameter Types.PerUnit Wqsem "Bandwidth of the PCC voltage filter in the QSEM";
  ...
  Modelica.Blocks.Interfaces.RealOutput iqConvRefPu(start = IqConv0Pu) "q-axis current reference in pu (base UNom, SNom)" annotation(
    Placement(visible = true, transformation(origin = {110, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {110, -40}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Interfaces.BooleanInput BlocCurrentSaturation_Enable "True when CurrentSaturation is clamping the current reference: freezes the PCC voltage filter below" annotation(
    Placement(transformation(origin = {0, -120}, extent = {{-10, -10}, {10, 10}}), iconTransformation(origin = {70, 110}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  ...
  Types.PerUnit udPccFiltPu(start = UdPcc0Pu) "Filtered d-axis PCC voltage used internally by QSEM";
  Types.PerUnit uqPccFiltPu(start = UqPcc0Pu) "Filtered q-axis PCC voltage used internally by QSEM";
equation
  der(udPccFiltPu) = if BlocCurrentSaturation_Enable then 0 else Wqsem*(udFilteredPCCPu - udPccFiltPu);
  der(uqPccFiltPu) = if BlocCurrentSaturation_Enable then 0 else Wqsem*(uqFilteredPCCPu - uqPccFiltPu);
  [idConvRefPu; iqConvRefPu] = 1/(RFilter^2 + (LFilter*omegaPu + XVI)^2)*[RFilter, LFilter*omegaPu + XVI; -(LFilter*omegaPu + XVI), RFilter]*[udFilterRefPu - udPccFiltPu; uqFilterRefPu - uqPccFiltPu];
  annotation(
    preferredView = "text",
