...
model DynGridFormingControlVSM_CSA_VC
  /*
  ...
  Modelica.Blocks.Interfaces.RealInput PFilterPu(start = PFilter0Pu) "Active power generated at the converter's capacitor in pu (base SNom) (generator convention)" annotation(
    Placement(transformation(origin = {-108, 72}, extent = {{-8, -8}, {8, 8}}), iconTransformation(origin = {-109, -73}, extent = {{-9, -9}, {9, 9}})));
  ...
  Modelica.Blocks.Interfaces.RealInput URefPu(start = URef0Pu) "Voltage module reference in pu (base UNom)" annotation(
    Placement(transformation(origin = {-128, 30}, extent = {{-8, -8}, {8, 8}}), iconTransformation(origin = {-109, 3}, extent = {{-9, -9}, {9, 9}})));
  Modelica.Blocks.Interfaces.RealInput QFilterRefPu "Reactive power reference at the converter's capacitor in pu (base SNom) (generator convention)" annotation(
  ...
  Modelica.Blocks.Math.Gain gain(k = -XVI) annotation(
    Placement(transformation(origin = {-82, -6}, extent = {{-4, -4}, {4, 4}})));
...
equation
  ...
  connect(inverseCurrentLoop.idConvRefPu, CSA.idConvRefPu) annotation(
    Line(points = {{22, 33}, {22, 28}, {30, 28}}, color = {0, 0, 127}));
  ...
  connect(idConvPu, gain.u) annotation(
    Line(points = {{-108, -16}, {-118, -16}, {-118, -6}, {-87, -6}}, color = {0, 0, 127}));
  connect(idConvPu, inverseCurrentLoop.idQSEM) annotation(
    Line(points = {{-108, -16}, {-32, -16}, {-32, 30}, {-16, 30}}, color = {0, 0, 127}));
  connect(iqConvPu, inverseCurrentLoop.iqQSEM) annotation(
    Line(points = {{-108, -34}, {-22, -34}, {-22, 20}, {-16, 20}}, color = {0, 0, 127}));
  connect(PFilterPu, VSM.PFilterPu) annotation(
    Line(points = {{-108, 72}, {-64, 72}}, color = {0, 0, 127}));
  connect(URefPu, add.u1) annotation(
    Line(points = {{-128, 30}, {-94, 30}, {-94, 38}}, color = {0, 0, 127}));
  connect(gain1.y, add.u2) annotation(
    Line(points = {{-112, 4}, {-94, 4}, {-94, 26}}, color = {0, 0, 127}));
  connect(add.y, inverseCurrentLoop.udConvRefPu) annotation(
    Line(points = {{-70, 32}, {-54, 32}, {-54, 42}, {-10, 42}}, color = {0, 0, 127}));
  connect(gain.y, inverseCurrentLoop.uqConvRefPu) annotation(
    Line(points = {{-78, -6}, {-46, -6}, {-46, 46}, {-6, 46}, {-6, 42}}, color = {0, 0, 127}));
  connect(currentLoop.DeltaUidConvRefPu, inverseCurrentLoop.DeltaUidConvRefPu) annotation(
    Line(points = {{62, 42}, {62, 52}, {4, 52}, {4, 42}}, color = {0, 0, 127}));
  connect(currentLoop.DeltaUiqConvRefPu, inverseCurrentLoop.DeltaUiqConvRefPu) annotation(
    Line(points = {{60, 4}, {60, -4}, {4, -4}, {4, 4}}, color = {0, 0, 127}));
equation
...
end DynGridFormingControlVSM_CSA_VC;