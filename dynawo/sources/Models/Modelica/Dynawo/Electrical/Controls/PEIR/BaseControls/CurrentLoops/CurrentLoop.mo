...
model CurrentLoop "Current loop control for grid forming and grid following converters"
  ...
  parameter Types.PerUnit XFilter "Filter impedance in pu (base UNom, SNom)";
  ...
  Modelica.Blocks.Continuous.Integrator integratord(k = Kic, y_start = UdConv0Pu - Kfd * UdFilter0Pu + XFilter * IqConv0Pu) annotation(
  ...
  Modelica.Blocks.Math.Gain gainq(k = Kpc) annotation(
    Placement(visible = true, transformation(origin = {-60, -80}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Continuous.Integrator integratorq(k = Kic, y_start = UqConv0Pu - Kfq * UqFilter0Pu - XFilter * IdConv0Pu) annotation(
    Placement(visible = true, transformation(origin = {-60, -110}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Math.Gain GainLfd(k = XFilter) annotation(
    Placement(visible = true, transformation(origin = {-10, 25}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Math.Gain GainLfq(k = XFilter) annotation(
    Placement(visible = true, transformation(origin = {-10, -25}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
...
equation
  ...
  connect(GainLfd.u, idConvPu) annotation(
    Line(points = {{-22, 25}, {-120, 25}, {-120, 50}, {-150, 50}}, color = {0, 0, 127}));
  connect(GainLfq.u, iqConvPu) annotation(
    Line(points = {{-22, -25}, {-120, -25}, {-120, -50}, {-150, -50}}, color = {0, 0, 127}));
