...
equation
  // ── Step 1: compute raw boost ────────────────────────────────
  // If voltage falls below UboostStart, inject additional reactive current
  // proportional to the voltage deviation — WECC-like undervoltage support
  if noEvent(U_meas_pu < UboostStart) then
    iq_boost_raw = Kqv * (UboostStart - U_meas_pu);
  else
    iq_boost_raw = 0;
  end if;
  ...
  /*
     If PQFlag = true  (P priority):
       - d-axis has priority
       - if |id_raw| >= Imax: all current on d-axis, iq_lim = 0
       - else: id_lim = id_raw, iq limited by circle constraint

     If PQFlag = false (Q priority):
       - q-axis (including boost) has priority
       - if |iq_eff| >= Imax: all current on q-axis, id_lim = 0
       - else: iq_lim = iq_eff, id limited by circle constraint
  */
  if PQFlag then
    // P priority (d-axis)
