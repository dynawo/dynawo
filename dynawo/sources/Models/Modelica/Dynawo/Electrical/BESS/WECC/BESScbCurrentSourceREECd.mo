...
model BESScbCurrentSourceREECd "WECC BESS with electrical control model type C, generator/converter model type B and plant control type A "
  ...
  // Initial parameters
  final parameter Types.PerUnit URef0Pu = if VCompFlag == true then UInj0Pu else (U0Pu + Kc*Q0Pu*SystemBase.SnRef/SNom) "Start value of voltage setpoint for plant level control, calculated depending on VcompFlag, in pu (base UNom)" annotation(
