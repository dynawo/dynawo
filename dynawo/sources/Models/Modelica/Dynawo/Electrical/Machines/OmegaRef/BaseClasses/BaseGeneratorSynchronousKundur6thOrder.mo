...
partial model BaseGeneratorSynchronousKundur6thOrder "6th order synchronous machine based on the Kundur book - Base dynamic model"
  extends GeneratorSynchronousParametersKundur6thOrder;
...
equation
  ...
  if running.value then
    ...
    udPu = (RaPPu + RTfoPu) * idPu - lambdaqPu;
    uqPu = (RaPPu + RTfoPu) * iqPu + lambdadPu;
    ...
    MdSatPPu = MdPPu * lambdaAirGapPu/(lambdaAirGapPu + ASat*exp(BSat*(lambdaAirGapPu - lambdaT1)));
    MqSatPPu = MqPPu * lambdaAirGapPu/(lambdaAirGapPu + ASat*exp(BSat*(lambdaAirGapPu - lambdaT1)));
  else
    udPu = 0;
    uqPu = 0;
...
end BaseGeneratorSynchronousKundur6thOrder;
