...
model GeneratorPTanPhi "Generator PV when linked to reactive power control loop"
  ...
  Modelica.Blocks.Interfaces.RealInput deltaPGenPu(start = 0) "Percentage variation of active power reference in pu (base PNom)";
...
equation
  ...
  if running.value then
    PGenRawPu = PGen0Pu * (1 + deltaPGenPu) + deltaPRefPu * PNom / SystemBase.SnRef + AlphaPu * (1 - omegaRefPu);
