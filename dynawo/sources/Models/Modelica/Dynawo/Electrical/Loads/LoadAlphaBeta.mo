...
equation
  if (running.value) then
    PPu = PRefPu * (1 + deltaP) * ((sqrt(terminal.V.re^2 + terminal.V.im^2 + Modelica.Constants.eps) / ComplexMath.'abs'(u0Pu)) ^ alpha);
    QPu = QRefPu * (1 + deltaQ) * ((complexToPolar.len / ComplexMath.'abs'(u0Pu)) ^ beta);
  else
