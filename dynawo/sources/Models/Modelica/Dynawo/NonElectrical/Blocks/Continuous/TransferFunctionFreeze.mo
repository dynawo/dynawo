...
block TransferFunctionFreeze "Linear transfer function"
  extends Modelica.Blocks.Interfaces.SISO(y(start = y_start));
   
    //Input Parameter
  Modelica.Blocks.Interfaces.BooleanInput freeze if UseFreeze annotation(
    Placement(transformation(origin = {-70, 120}, extent = {{-20, -20}, {20, 20}}, rotation = -90), iconTransformation(origin = {-56, 120}, extent = {{-20, -20}, {20, 20}}, rotation = -90)));
...
IntegratorSetFreeze integrator(Y0=y_start,UseFreeze=UseFreeze);

equation
  assert(size(b, 1) <= size(a, 1), "Transfer function is not proper");
    if nx == 0 then
    integrator.u = der(d*u);
  else
    der(x_scaled[1])    = (-a[2:na]*x_scaled + a_end*u) / a[1];
    der(x_scaled[2:nx]) = x_scaled[1:nx-1];
    integrator.u = der(((bb[2:na] - d*a[2:na])*x_scaled) / a_end + d*u);
    x = x_scaled / a_end;
  end if;
  if UseFreeze then
  connect(freeze,integrator.freeze);
  end if;
...
<pre>      2*s + 4
 y = --------- * u
    ...
    Icon(coordinateSystem(preserveAspectRatio = true, extent = {{-100.0, -100.0}, {100.0, 100.0}}), graphics = {Line(points = {{-80.0, 0.0}, {80.0, 0.0}}, color = {0, 0, 127}), Text(lineColor = {0, 0, 127}, extent = {{-90.0, 10.0}, {90.0, 90.0}}, textString = "b(s)"), Text(lineColor = {0, 0, 127}, extent = {{-90.0, -90.0}, {90.0, -10.0}}, textString = "a(s)")}),
    Diagram(coordinateSystem(preserveAspectRatio = true, extent = {{-100, -100}, {100, 100}}), graphics = {Line(points = {{40, 0}, {-40, 0}}), Text(extent = {{-55, 55}, {55, 5}}, textString = "b(s)"), Text(extent = {{-55, -5}, {55, -55}}, textString = "a(s)"), Rectangle(extent = {{-60, 60}, {60, -60}}, lineColor = {0, 0, 255}), Line(points = {{-100, 0}, {-60, 0}}, color = {0, 0, 255}), Line(points = {{60, 0}, {100, 0}}, color = {0, 0, 255})}));
end TransferFunctionFreeze;
