...
block TransferFunction "Linear transfer function"
  ...
  parameter Boolean ZeroA1 = false "If true, highest-order coefficient of denominator can be zero";
...
equation
  ...
  if ZeroA1 and a[1] <= 100 * Modelica.Constants.eps then
    y = u;
...
Initial values of the states <strong>x</strong> can be set via parameter <strong>x_start</strong>.</p><p>The denominator a has to be at least of order 1 i.e. of size 2.</p><p>If the highest-order component of the denominator is zero, the block returns the input as output.</p>
