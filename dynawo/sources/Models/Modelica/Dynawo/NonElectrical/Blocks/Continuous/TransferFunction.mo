...
block TransferFunction "Linear transfer function"
  ...
  output Real x[nx](start = x_start)
...
protected
  ...
  parameter Real d = bb[1] / a_one;
  parameter Real a_one = if a[1] > 100 * Modelica.Constants.eps * sqrt(a*a) then a[1] else 1;
  parameter Real a_end = if a[end] > 100 * Modelica.Constants.eps * sqrt(a*a) then a[end] else 1;
...
Initial values of the states <strong>x</strong> can be set via parameter <strong>x_start</strong>.</p><p>If the highest-order component of the denominator is zero, the block returns the input as output.</p>
