...
model GovSteam1 "Steam turbine governor, based on the GovSteamIEEE1 (with optional deadband and nonlinear valve gain added)"
  ...
  parameter Types.ApparentPowerModule SNom "Rated apparent power of the generator";
  parameter Types.ActivePower PNomTurb "Rated power of the generator";
  ...
  Modelica.Blocks.Continuous.FirstOrder firstOrder(T = t4, initType = Modelica.Blocks.Types.Init.InitialState, y_start = at) annotation(
    Placement(visible = true, transformation(origin = {210, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Continuous.FirstOrder firstOrder1(T = t5, initType = Modelica.Blocks.Types.Init.InitialState, y_start = at) annotation(
    Placement(visible = true, transformation(origin = {270, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Continuous.FirstOrder firstOrder2(T = t6, initType = Modelica.Blocks.Types.Init.InitialState, y_start = at) annotation(
    Placement(visible = true, transformation(origin = {330, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  Modelica.Blocks.Math.Gain gain(k = if t3 > 0 then 1 / t3 else 0) annotation(
  ...
  Modelica.Blocks.Math.Gain gain8(k = K8) annotation(
    Placement(visible = true, transformation(origin = {420, -50}, extent = {{-10, -10}, {10, 10}}, rotation = -90)));
  Modelica.Blocks.Continuous.LimIntegrator limitedIntegrator(outMax = PMaxPu, outMin = PMinPu, y_start = at) annotation(
    Placement(visible = true, transformation(origin = {-30, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  parameter Types.ActivePowerPu PmRef0Pu(fixed=false) "Initial reference mechanical power in pu (base PNom)" annotation(
    Dialog(group = "Initialization"));
...
initial algorithm
  ...
  PmRef0Pu := at + K * (SystemBase.omega0Pu - SystemBase.omegaRef0Pu);
...
equation
  ...
  connect(leadLagBlockPass.y, add3.u2) annotation(
    Line(points = {{-248, 0}, {-222, 0}}, color = {0, 0, 127}));
  connect(switch.y, gain9.u) annotation(
    Line(points = {{-318, -50}, {-316, -50}, {-316, 0}, {-312, 0}}, color = {0, 0, 127}));
  connect(gain9.y, leadLagBlockPass.u) annotation(
    Line(points = {{-288, 0}, {-272, 0}}, color = {0, 0, 127}));
  connect(gain.y, limiter.u) annotation(
    Line(points = {{-158, 0}, {-102, 0}}, color = {0, 0, 127}));
  connect(gain10.y, Pm1Pu) annotation(
    Line(points = {{491, 100}, {510, 100}}, color = {0, 0, 127}));
  connect(add6.y, gain10.u) annotation(
    Line(points = {{462, 100}, {468, 100}}, color = {0, 0, 127}));
  connect(limitedIntegrator.y, firstOrder.u) annotation(
    Line(points = {{-18, 0}, {0, 0}, {0, 26}, {140, 26}, {140, 0}, {198, 0}}, color = {0, 0, 127}));
  connect(firstOrder3.y, gain5.u) annotation(
    Line(points = {{404, 0}, {420, 0}, {420, 38}}, color = {0, 0, 127}));
  connect(firstOrder3.y, gain8.u) annotation(
    Line(points = {{404, 0}, {420, 0}, {420, -38}}, color = {0, 0, 127}));
  connect(firstOrder2.y, firstOrder3.u) annotation(
    Line(points = {{342, 0}, {380, 0}}, color = {0, 0, 127}));
  connect(switch1.y, add3.u3) annotation(
    Line(points = {{-80, -50}, {-240, -50}, {-240, -8}, {-222, -8}}, color = {0, 0, 127}));
