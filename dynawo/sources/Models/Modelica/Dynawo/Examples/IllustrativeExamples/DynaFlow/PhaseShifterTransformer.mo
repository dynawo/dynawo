...
model PhaseShifterTransformer "Elementary system with one infinite bus, one load, two lines and a Phase Tap Changer Transformer"
  ...
  Dynawo.Electrical.Transformers.TransformersVariableTapControlled.TransformerPhaseTapChangerControlled tfo01(AlphaTfoMax = 0.19634954084936207, AlphaTfoMin = -0.19634954084936207, BPu = 0.0*ZBASE69_0, GPu = 0.0*ZBASE69_0, i10Pu = Complex(0.50081, 0.0984), i20Pu = Complex(0.50081, 0.0984), increaseTapToIncreaseValue = true, NbTap = 19, P0 = -0.5, P10Pu = 0.50081, PDeadBand = 0.05, PTarget = -0.3, Q10Pu = 0.0984, RPu = 0.0/ZBASE69_0, RatioTfo0Pu = 1, regulating0 = false, state0 = Dynawo.Electrical.Controls.Transformers.BaseClasses.TapChangerPhaseShifterParams.State.Standard, t1st = 25, tap0 = 9, tapMax = 18, tapMin = 0, tNext = 15, U10Pu = 1, U20Pu = 1, u10Pu = Complex(1, 0), u20Pu = Complex(1, 0), XPu = 0.0/ZBASE69_0) annotation(
...
equation
  ...
  connect(tfo01.terminal2, bus01.terminal) annotation(
    Line(points = {{20, 32}, {20, 40}, {0, 40}, {0, 60}}, color = {0, 0, 255}));
  ...
  connect(tfo01.terminal1, bus03.terminal) annotation(
  ...
  annotation(
    ...
    Documentation(info = "<html><head></head><body><div>This test case is composed one infinite bus, three regular buses, one load, two lines a phase tap changer transformer and the associated phase shifter controller.&nbsp;</div><div>This is inspired by the <a href=\"https://en.wikipedia.org/wiki/Quadrature_booster\">quadrature_booster Wikipedia page</a>.</div><div><br></div><div>At t = 0 s, the phase tap changer transformer is at its centre tap position of 9 and has a phase angle of 0°. It thus does not affect the power flow through the circuit and both lines are equally loaded at 0.5 pu.</div><div>At t = 50 s, the phase shifter controller is turned on with a target PMonitored = 0.3 pu (+/- tolerance) through line02.</div><div>The controller then passes several taps up. The resulting positive phase angle (7.5°) has diverted portion of loading onto the parallel circuit, while the total load supplied is unchanged.</div><div><br></div><div>N.B : In the example found in the Wikipedia page, the resulting phase angle is negative (-7.5°) and final tap position is 4 instead of 15. However, we have the same power flow. This is explained by the convention that we used in the transformer model where the monitored power is at terminal 1 and is in receptor convention, we should increase the tap to increase the negative value. We can replicate the example by using the transformer model separated from the tap changer and we can monitor for example -line01.P2Pu in generator convention.</div><div><br><div>
    <figure>
    <img width=\"350\" src=\"modelica://Dynawo/Examples/IllustrativeExamples/DynaFlow/Resources/PhaseShifterPMonitored.png\">
    </figure>
</div></div></body></html>"));
