...
equation
  ...
  annotation(
    ...
    Documentation(info = "<html><head></head><body>The event simulated is a bolted fault of 150ms at t=2s and close to the installation. In this simulation, the fault is eliminated by switching off the line concerned. The production unit should inject reactive current during the fault and recover its active power in less than 2s if it follows the grid code's requirements. The test is described thorougly in fiche I5 from the french grid code.</body></html>"));
end SheetI5;
