...
model SheetI2Xccb
  ...
  extends Dynawo.Examples.GridCodeSimulations.BaseUnitModel(URef(height = 0.02), XccPu = XccPu_To_Be_Defined);

  parameter Types.PerUnit XccPu_To_Be_Defined = XbPu "Reactance of equivalent branch connection to the grid in pu (base SnRef, UNom)";
...
equation
  ...
  annotation(
    ...
    Documentation(info = "<html><head></head><body>The event simulated is a 2% step order at t=2s on the plant voltage droop regulation required from PEIRs connected to the grid. Here, the short circuit impedance is big, emulating a weak grid situation. The test is described thorougly in fiche I2 from the french grid code.</body></html>"));
end SheetI2Xccb;
