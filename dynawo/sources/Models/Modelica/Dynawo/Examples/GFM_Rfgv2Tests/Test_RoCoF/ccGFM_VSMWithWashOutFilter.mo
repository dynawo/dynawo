...
model ccGFM_VSMWithWashOutFilter
...
//Angle is calculated in Degrees
  ...
  connect(Pref_.y, gFM_VSM_cc.PrefPu) annotation(
    Line(points = {{-122, 4}, {-100, 4}, {-100, 32}, {-42, 32}}, color = {0, 0, 127}));
  connect(const.y, acGrid.OmegaRef) annotation(
    Line(points = {{-116, -36}, {-64, -36}, {-64, -34}}, color = {0, 0, 127}));
  connect(gFM_VSM_cc.OmegaRefPu, const.y) annotation(
    Line(points = {{-42, 14}, {-92, 14}, {-92, -36}, {-116, -36}}, color = {0, 0, 127}));
  annotation(
  ...
  Documentation(info = "<html><head></head><body>VSM control structure from Valentin with a Gain damping Power and a Gain Damping Angle. Values given by Valentin; GainDampingAngle=26, GainDampingPower=25 and high Pass Filter TS/(1+Ts) with T=0.5. This kind of control structure avoid FCR (Frequency Containment Reserve), without the use of a PLL, in steady state. It was developed to compare the behavior of this kind of converter against the envelopes proposed for the GFM (obtained frol generic model where Wref = Wpll and VSM is the one taken from migrate)<div><br><div><br></div></div></body></html>"));
end ccGFM_VSMWithWashOutFilter;
