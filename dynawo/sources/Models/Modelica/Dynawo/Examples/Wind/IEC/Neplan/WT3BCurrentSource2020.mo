...
equation
  ...
  annotation(
    ...
    experiment(StartTime = 0, StopTime = 45, Tolerance = 1e-05, Interval = 0.001),
...
The following image shows the reference step&nbsp;</div><div>and the model's P response as well as the&nbsp;</div><div>mechanical module's rotation speed responses. 

<b>Active power and rotation speed responses:</b></div><div> <img width=\"450\" src=\"modelica://Dynawo/Examples/Wind/IEC/Resources/Images/type3bPrefstepP.png\"><br></div><div><b><u>Bug desription:</u></b></div><div>This model does not behave as exptected.&nbsp;</div><div><b>Desired behavior:</b> The output power obtains the new P setpoint stably</div><div>after a transient period.</div><div><b>Actual behavior:</b> The output power drops, almost reaching the new setpoint.&nbsp;</div><div>After some time, it reaches the setpoint and when it does&nbsp;</div><div>the model starts oscillating (the simulaion is NOT running stably&nbsp;</div><div>after the oscillations decay).</div><div><b>Note:</b> This behaviour was also seen&nbsp;</div><div>in the reference model (DIgSILENT PowerFactory)</div><div><br></div><div>&nbsp;
