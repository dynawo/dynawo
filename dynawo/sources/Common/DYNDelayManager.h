...
class DelayManager {
 public:
  ...
  std::pair<double, double> getLastDelay(size_t value_id) const;

  size_t size(size_t value_id) const;
 ...
 private:
   ...
   *
   * Uses the last index in the array
   *
   * @returns the new id
   */
  size_t generateId() const {
    return delays_.size() - 1;
  }
