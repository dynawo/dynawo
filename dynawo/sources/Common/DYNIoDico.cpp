...
void IoDicos::addDico(const string& name, const string& baseName, const string& locale) {
  ...
  if (hasIoDico(name)) {
    const boost::shared_ptr<IoDico>& dico = getIoDico(name);
    dico->readFile(file);  // new key/sentence added to the existing dico
  } else {
    const auto dico = boost::make_shared<IoDico>(name);
    dico->readFile(file);
  ...
  if (!files.empty()) {
    const auto dico = boost::make_shared<OppositeEventDico>(name);
...
void IoDicos::addDicos(const string& dictionariesMappingFile, const string& locale) {
  ...
  const auto dico = boost::make_shared<IoDico>("MAPPING");
