...
void launchSimu(const std::string& jobsFileName) {
  ...
  const boost::shared_ptr<job::JobsCollection>& jobsCollection = importer.importFromFile(jobsFileName);
  ...
  for (const auto& job : jobsCollection->getJobs()) {
    ...
    const auto context = boost::make_shared<SimulationContext>();
    ...
    try {
      simulation = boost::make_shared<Simulation>(job, context);
      simulation->init();
