...
void
SimulationRT::configureSimulationInputs() {
  //---- dydFile ----
...
void
SimulationRT::configureCriteria() {
...
void
SimulationRT::configureSimulationOutputs() {
...
void
SimulationRT::configureConstraintsOutputs() {
  ...
  if (jobEntry_->getOutputsEntry()->getConstraintsEntry()) {
    ...
    SimulationRT::exportConstraintsMode_t exportModeFlag = SimulationRT::EXPORT_CONSTRAINTS_NONE;
    string outputFile = "";
    if (exportMode == "TXT") {
      exportModeFlag = SimulationRT::EXPORT_CONSTRAINTS_TXT;
      outputFile = createAbsolutePath("constraints.log", constraintsDir);
    } else if (exportMode == "XML") {
      exportModeFlag = SimulationRT::EXPORT_CONSTRAINTS_XML;
    ...
    setConstraintsExportMode(SimulationRT::EXPORT_CONSTRAINTS_NONE);
  }
}

void
SimulationRT::configureTimelineOutputs() {
  // Timeline settings
  if (jobEntry_->getOutputsEntry()->getTimelineEntry()) {
    ...
    SimulationRT::exportTimelineMode_t exportModeFlag = SimulationRT::EXPORT_TIMELINE_NONE;
    string outputFile = "";
    if (exportMode == "TXT") {
      exportModeFlag = SimulationRT::EXPORT_TIMELINE_TXT;
      outputFile = createAbsolutePath("timeline.log", timeLineDir);
    } else if (exportMode == "CSV") {
      exportModeFlag = SimulationRT::EXPORT_TIMELINE_CSV;
      outputFile = createAbsolutePath("timeline.csv", timeLineDir);
    } else if (exportMode == "XML") {
      exportModeFlag = SimulationRT::EXPORT_TIMELINE_XML;
    ...
    setTimelineExportMode(SimulationRT::EXPORT_TIMELINE_NONE);
  }
}

void
SimulationRT::configureTimetableOutputs() {
  // Timetable settings
...
void
SimulationRT::configureCurveOutputs() {
  // Curves settings
  if (jobEntry_->getOutputsEntry()->getCurvesEntry()) {
    ...
    SimulationRT::exportCurvesMode_t exportModeFlag = SimulationRT::EXPORT_CURVES_NONE;
    string outputFile = "";
    if (exportMode == "CSV") {
      exportModeFlag = SimulationRT::EXPORT_CURVES_CSV;
      outputFile = createAbsolutePath("curves.csv", curvesDir);
    } else if (exportMode == "XML") {
      exportModeFlag = SimulationRT::EXPORT_CURVES_XML;
    ...
    setCurvesExportMode(SimulationRT::EXPORT_CURVES_NONE);
  }
}

void
SimulationRT::configureFinalStateValueOutputs() {
  ...
  if (jobEntry_->getOutputsEntry()->getFinalStateValuesEntry()) {
    ...
    setFinalStateValuesExportMode(SimulationRT::EXPORT_FINAL_STATE_VALUES_NONE);
  }
}

void
SimulationRT::configureFinalStateOutputs() {
  ...
  for (std::map<double, DYN::SimulationRT::ExportStateDefinition>::const_iterator it = dumpStateDefinitionsMap.begin();
...
void
SimulationRT::configureLostEquipmentsOutputs() {
  ...
  if (jobEntry_->getOutputsEntry()->getLostEquipmentsEntry()
    ...
    setLostEquipmentsExportMode(SimulationRT::EXPORT_LOSTEQUIPMENTS_XML);
    setLostEquipmentsOutputFile(createAbsolutePath("lostEquipments.xml", lostEquipmentsDir));
  }
}

void
SimulationRT::compileModels() {
...
void
SimulationRT::loadDynamicData() {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
  Timer timer("SimulationRT::loadDynamicData()");
#endif
  // Load model
...
void
SimulationRT::importCurvesRequest() {
...
void
SimulationRT::importFinalStateValuesRequest() {
...
void
SimulationRT::initFromData(const shared_ptr<DataInterface>& data, const shared_ptr<DynamicData>& dyd) {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
  Timer timer("SimulationRT::initFromData()");
#endif
  Modeler modeler;
...
SimulationRT::init() {
  ...
  // SimulationRT::initFromData==>Modeler::initSystem==>Modeler::initModelDescription(dyd,data)
...
bool
SimulationRT::hasIntermediateStateToDump() const {
...
void
SimulationRT::endSimulationWithError(bool criteria, bool isSimulationDiverging) {
  if (timetableOutputFile_ != "")
    remove(timetableOutputFile_);
...
bool
SimulationRT::checkCriteria(double t, bool finalStep) {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
  Timer timer("SimulationRT::checkCriteria()");
#endif
...
void
SimulationRT::getFailingCriteria(std::vector<std::pair<double, std::string> >& failingCriteria) const {
  data_->getFailingCriteria(failingCriteria);
}

void
SimulationRT::updateParametersValues() {
  if (exportCurvesMode_ == EXPORT_CURVES_NONE)
    return;
...
void
SimulationRT::printSolverHeader() {
  Trace::info() << "-----------------------------------------------------------------------" << Trace::endline;
...
void
SimulationRT::addEvent(const MessageTimeline& messageTimeline) {
  if (timeline_) {
...
void
SimulationRT::printHighestDerivativesValues() {
...
void
SimulationRT::printEnd() {
  solver_->printEnd();
}

void
SimulationRT::setCriteriaStep(const int step) {
  if (step <= 0)
...
void
SimulationRT::openFileStream(ofstream& stream, const std::string& path) {
...
void
SimulationRT::printCurves(std::ostream& stream) const {
...
void SimulationRT::printFinalStateValues(std::ostream& stream) const {
...
void
SimulationRT::printTimeline(std::ostream& stream) const {
  if (filterTimeline_) {
...
void
SimulationRT::printConstraints(std::ostream& stream) const {
...
void
SimulationRT::printLostEquipments(std::ostream& stream) const {
...
void
SimulationRT::dumpIIDMFile(const boost::filesystem::path& iidmFile) {
  if (data_)
    data_->dumpToFile(iidmFile.generic_string());
}

void
SimulationRT::dumpIIDMFile() {
  if (finalState_.iidmFile_) {
...
void
SimulationRT::dumpState(const boost::filesystem::path& dumpFile) {
  if (!model_) return;
  stringstream state;
...
double
SimulationRT::loadState(const string& fileName) {
...
void
SimulationRT::printCurrentTime(const string& fileName) {
...
SimulationRT::ExportStateDefinition::ExportStateDefinition(double timestamp,
