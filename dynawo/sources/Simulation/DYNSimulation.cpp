...
Simulation::init() {
  ...
  if (initialStateFile_ != "") {
    ...
    model_->printToFile(outputsDirectory_, "loadState", true, true, true);
...
Simulation::calculateIC() {
  ...
  model_->printToFile(outputsDirectory_, "loadState", true, true, false);
  ...
  model_->printToFile(outputsDirectory_, "init", true, true, true);

  solver_->calculateIC();
  model_->getCurrentZ(zCurrent_);

  model_->printToFile(outputsDirectory_, "afterCalculateIC", true, true, true);
  ...
  std::vector<double> gg(model_->sizeG());
  for (int i = 0 ; i < model_->sizeG() ; ++i)
    gg[i] = model_->getG()[i];
  model_->printVectorToFile(createAbsolutePath("tmpGafterCalculateIC", outputsDirectory_), "solG", &gg[0], model_->sizeY(), 0);
...
Simulation::simulate() {
  ...
  try {
    ...
    model_->printVectorToFile(createAbsolutePath("tmpFEnd", outputsDirectory_), "solF", &ff[0], model_->sizeG(), nbPrint);

    std::vector<double> gg(model_->sizeG());
    for (int i = 0 ; i < model_->sizeG() ; ++i)
      gg[i] = model_->getG()[i];
    model_->printVectorToFile(createAbsolutePath("tmpGEnd", outputsDirectory_), "solG", &gg[0], model_->sizeY(), nbPrint);

    model_->printToFile(outputsDirectory_, "End", true, true, true);
