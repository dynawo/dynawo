...
Simulation::Simulation(const shared_ptr<job::JobEntry>& jobEntry, const shared_ptr<SimulationContext>& context, shared_ptr<DataInterface> data) :
context_(context),
jobEntry_(jobEntry),
data_(data),
timeline_(),
...
Simulation::importFinalStateValuesRequest() {
  ...
  for (const auto& finalStateValue : finalStateValuesCollection->getFinalStateValues()) {
    CurvesMap::const_iterator entry = curvesMap.find(std::make_pair(finalStateValue->getModelName(), finalStateValue->getVariable()));
...
Simulation::initStructure() const {
  model_->initBuffers();
  const shared_ptr<ModelMulti> model = dynamic_pointer_cast<ModelMulti>(model_);
...
bool
Simulation::checkCriteria(const double t, const bool finalStep) {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
...
void
Simulation::updateCurves(const bool updateCalculateVariable) {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
