...
RatioTapChangerInterfaceIIDM::sanityCheck(const std::string& parentName) const {
  if (tapChangerIIDM_.has_regulating() && tapChangerIIDM_.regulating()) {
    if (!tapChangerIIDM_.has_targetV())
      DYNErrorQueue::instance().push(DYNError(DYN::Error::STATIC_DATA, MissingTargetVInRatioTapChanger, parentName));
    if (!tapChangerIIDM_.has_terminalReference())
      DYNErrorQueue::instance().push(DYNError(DYN::Error::STATIC_DATA, MissingTerminalRefInRatioTapChanger, parentName));
    if (tapChangerIIDM_.has_terminalReference() && tapChangerIIDM_.terminalReference().side == IIDM::side_end) {
      DYNErrorQueue::instance().push(DYNError(DYN::Error::STATIC_DATA, MissingTerminalRefSideInRatioTapChanger, parentName));
    }
  }
}
