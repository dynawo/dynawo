...
QuadripoleCriteria<T>::checkCriteria(double t, bool finalStep, const boost::shared_ptr<timeline::Timeline>& timeline) {
  ...
  for (const auto& quadripole : quadripoles_) {
    ...
    if (params_->hasI1Max() && i1 > params_->getI1Max()) {
      std::unique_ptr<FailingCriteria> quadripoleFailingCriteria = std::unique_ptr<FailingCriteria>(new QuadripoleFailingCriteria(Side::ONE,
                                                                                                  quadripole->getID(),
                                                                                                  i1,
                                                                                                  params_->getI1Max(),
                                                                                                  params_->getId()));
    ...
    if (params_->hasI2Max() && i2 > params_->getI2Max()) {
      std::unique_ptr<FailingCriteria> quadripoleFailingCriteria = std::unique_ptr<FailingCriteria>(new QuadripoleFailingCriteria(Side::TWO,
                                                                                                  quadripole->getID(),
                                                                                                  i2,
                                                                                                  params_->getI2Max(),
                                                                                                  params_->getId()));
