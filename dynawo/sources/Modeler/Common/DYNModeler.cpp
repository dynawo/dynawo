...
Modeler::initNetwork() {
  ...
  const shared_ptr<SubModel> modelNetwork = SubModelFactory::createSubModelFromLib(DDBDir + "/DYNModelNetwork" + sharedLibraryExtension());
  modelNetwork->initFromData(data_);
  data_->setModelNetwork(modelNetwork);
  modelNetwork->name("NETWORK");
  const shared_ptr<ParametersSet>& networkParams = dyd_->getNetworkParameters();
...
void
Modeler::initParamDescription(const shared_ptr<ModelDescription>& modelDescription) const {
  const shared_ptr<ParametersSet>& params = modelDescription->getParametersSet();
...
void
Modeler::initStaticRefs(const shared_ptr<SubModel>& model, const shared_ptr<ModelDescription>& modelDescription) const {
...
void
Modeler::collectAllInternalConnections(const shared_ptr<dynamicdata::ModelicaModel>& model,
...
Modeler::sanityCheckFlowConnection() const {
  ...
  for (const auto& modelDescription : dyd_->getModelDescriptions()) {
    ...
    const shared_ptr<ModelDescription>& modelDesc = modelDescription.second;
    ...
    for (const auto& variableConnectedInternally : variablesConnectedInternally) {
      const shared_ptr<Variable>& var = subModel->getVariable(variableConnectedInternally.first);
      if (var->getType() == FLOW) {
