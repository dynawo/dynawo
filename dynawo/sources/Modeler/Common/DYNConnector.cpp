...
int
ConnectorContainer::getConnectorVarNum(const shared_ptr<SubModel>& subModel, const shared_ptr<Variable>& variable, const bool flowConnector) {
  int numVar;
...
ConnectorContainer::evalFConnector(const double /*t*/) {
  ...
  if (index_.empty()) {
    ...
    for (const auto& flowConnector : flowConnectors_) {
      ...
      const unsigned int connectedSubModelsSize = connectedSubModels.size();
