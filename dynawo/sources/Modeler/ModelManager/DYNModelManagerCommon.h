...
#define logConstraintFromModelica(key, begin) \
  getModelManager()->addConstraint(getModelManager()->name(), begin, Message(Message::CONSTRAINT_KEY, KeyConstraint_t::names(KeyConstraint_t::value(key))))

#define logConstraintFromModelicaWithData(key, begin, kind, limit, varIndex, ...) \
  getModelManager()->addConstraintWithSource((Message(Message::CONSTRAINT_KEY, KeyConstraint_t::names(KeyConstraint_t::value(key))), ##__VA_ARGS__), \
                                    begin, kind, limit, varIndex)
