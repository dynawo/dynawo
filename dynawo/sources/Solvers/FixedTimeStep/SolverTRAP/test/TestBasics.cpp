...
/*
TEST(SimulationTest, testSolverTRAPTestBetaUnstableRoot) {
  const double tStart = 0.;
  const double tStop = 5.;
  // Here the maximum root restart is artificially set at zero to test the maximum root restart detection in the trapezoidal solver strategy.
  std::pair<boost::shared_ptr<Solver>, boost::shared_ptr<Model> > p = initSolverAndModelWithDyd("jobs/solverTestBeta.dyd", tStart, tStop);
  boost::shared_ptr<Solver> solver = p.first;
  boost::shared_ptr<Model> model = p.second;
...
TEST(SimulationTest, testSolverTRAPAlgebraicMode) {
  const double tStart = 0.;
...
TEST(SimulationTest, testSolverTRAPSilentZ) {
  const double tStart = 0.;
  const double tStop = 10.;
...
TEST(SimulationTest, testSolverTRAPSilentZDisabled) {
  const double tStart = 0.;
  const double tStop = 10.;
  std::pair<boost::shared_ptr<Solver>, boost::shared_ptr<Model> > p = initSolverAndModelWithDyd("jobs/solverTestSilentZ.dyd",
...
TEST(SimulationTest, testSolverTRAPSilentZNotUsedInContinuous) {
  const double tStart = 0.;
  const double tStop = 10.;
...
TEST(SimulationTest, testSolverTRAPSilentZNotUsedInContinuousDisabled) {
  const double tStart = 0.;
  const double tStop = 10.;
  std::pair<boost::shared_ptr<Solver>, boost::shared_ptr<Model> > p = initSolverAndModelWithDyd("jobs/solverTestSilentZNotUsedInContinuous.dyd",
                                                                                                tStart, tStop, true, true, false);
...
TEST(SimulationTest, testSolverTRAPSilentZNotUsedInContinuous2) {
  const double tStart = 0.;
  const double tStop = 10.;
