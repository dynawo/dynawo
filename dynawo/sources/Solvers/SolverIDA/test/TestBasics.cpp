...
boost::shared_ptr<Model> initModel(Modeler modeler, const double& tStart = 0, bool enableSilentZ = true) {
  ...
  model->initBuffers();
  model->setEnableSilentZ(enableSilentZ);
...
TEST(SimulationTest, testSolverIDAAlgebraicMode) {
  ...
  ASSERT_DOUBLE_EQUALS_DYNAWO(y[11], 0);
  ASSERT_DOUBLE_EQUALS_DYNAWO(y[12], 1);
  ASSERT_DOUBLE_EQUALS_DYNAWO(yp[8], 0.95214618832553821193);
  ...
  ASSERT_DOUBLE_EQUALS_DYNAWO(yp[8], 0.93468597466729808065);
  ASSERT_DOUBLE_EQUALS_DYNAWO(tCurrent, 1.0000002);
  for (size_t i = 0; i < z.size(); ++i) {
    if (i == 12 || i == 16) {
      ASSERT_DOUBLE_EQUALS_DYNAWO(z[i], 1);  // bus state == OPEN
  ...
  ASSERT_DOUBLE_EQUALS_DYNAWO(y[11], 0);
  ASSERT_DOUBLE_EQUALS_DYNAWO(y[12], 1);
  ASSERT_DOUBLE_EQUALS_DYNAWO(yp[8], 0.93468597860101021446);
