...
/**
 * @file  DYNSolverIDASPM.h
 *
 * @brief Solver based on sundials/IDA solver : variable step, order one or two
 *
 */
#ifndef SOLVERS_VARIABLETIMESTEP_SOLVERIDASPM_DYNSOLVERIDASPM_H_
#define SOLVERS_VARIABLETIMESTEP_SOLVERIDASPM_DYNSOLVERIDASPM_H_
...
/**
 * @brief SolverIDASPM factory class
 *
 * Class for IDA solver creation
 */
class SolverIDASPMFactory : public SolverFactory {
 public:
  /**
 ...
 */
class SolverIDASPM : public Solver::Impl {
 public:
  /**
 ...
 private:
  ...
  /**
   * @brief build the oscillating-variable boolean vector from the solver
   *        parameters. A state variable is flagged oscillating if its name is in
   *        the explicit list spmOscillatingVariablesList OR matches the regular
   *        expression spmOscillatingVariables (the two selectors are unioned;
   *        either may be left empty).
   * @return the number of oscillating variables selected
   */
  int buildOscillatingVarsVector();

  /**
   * @brief split a separator-delimited string (',' ';' or whitespace) into tokens
   * @param str the string to split
   * @return the non-empty tokens
   */
  static std::vector<std::string> splitVariableList(const std::string& str);
 ...
 private:
  ...
  // Sinusoidal Predictor Method (SPM) parameters and work data
  double spmPulsation_;  ///< reference angular frequency wbar of the oscillating components (e.g. 100*pi for 50 Hz)
  std::string spmOscillatingVariables_;  ///< regular expression selecting the oscillating variables by name (optional)
  std::string spmOscillatingVariablesList_;  ///< explicit list of oscillating variable names, separated by ',' ';' or whitespace (optional)
  N_Vector sundialsVectorIsAC_;  ///< boolean vector flagging the oscillating (1.0) / non-oscillating (0.0) variables
  SUNMatrix spmJacX_;  ///< state Jacobian dF/dX (CSR of J) pushed to the SPM estimator on each IDA Jacobian update
  SUNMatrix spmJacXdot_;  ///< derivative Jacobian dF/dXdot (CSR of J) pushed to the SPM estimator on each IDA Jacobian update
};

}  // end of namespace DYN

#endif  // SOLVERS_VARIABLETIMESTEP_SOLVERIDASPM_DYNSOLVERIDASPM_H_
