...
/**
 * @brief SolverIDASPMFactory getter
 * @return A pointer to a new instance of SolverIDASPMFactory
 */
extern "C" DYN::SolverFactory * getFactory() {
  return (new DYN::SolverIDASPMFactory());
}

/**
 * @brief SolverIDASPMFactory destroy method
 */
...
/**
 * @brief SolverIDASPM getter
 * @return A pointer to a new instance of SolverIDASPM
 */
extern "C" DYN::Solver* DYN::SolverIDASPMFactory::create() const {
  DYN::Solver* solver(new DYN::SolverIDASPM());
  return solver;
}

/**
 * @brief SolverIDASPM destroy method
 */
extern "C" void DYN::SolverIDASPMFactory::destroy(DYN::Solver* solver) const {
  delete solver;
}

namespace DYN {

SolverIDASPM::SolverIDASPM() :
IDAMem_(NULL),
linearSolver_(NULL),
...
lastRowVals_(NULL),
spmPulsation_(0.),
spmOscillatingVariables_(""),
spmOscillatingVariablesList_(""),
sundialsVectorIsAC_(NULL),
spmJacX_(NULL),
spmJacXdot_(NULL) {
}

void
SolverIDASPM::cleanIDA() {
  if (sundialsMatrix_ != NULL) {
    SUNMatDestroy(sundialsMatrix_);
    sundialsMatrix_ = NULL;
  }
  if (spmJacX_ != NULL) {
    SUNMatDestroy(spmJacX_);
    spmJacX_ = NULL;
  }
  if (spmJacXdot_ != NULL) {
    SUNMatDestroy(spmJacXdot_);
    spmJacXdot_ = NULL;
  }
  ...
  if (sundialsVectorIsAC_ != NULL) {
    N_VDestroy_Serial(sundialsVectorIsAC_);
    sundialsVectorIsAC_ = NULL;
  }
}
...
void
SolverIDASPM::defineSpecificParameters() {
  ...
  // Sinusoidal Predictor Method parameters
  // spmPulsation: reference angular frequency of the oscillating components (rad/s), e.g. 314.159265 for 50 Hz
  parameters_.insert(make_pair("spmPulsation", ParameterSolver("spmPulsation", VAR_TYPE_DOUBLE, !mandatory)));
  // spmOscillatingVariables: ECMAScript regular expression matching the names of the oscillating variables (optional)
  parameters_.insert(make_pair("spmOscillatingVariables", ParameterSolver("spmOscillatingVariables", VAR_TYPE_STRING, !mandatory)));
  // spmOscillatingVariablesList: explicit list of oscillating variable names, separated by ',' ';' or whitespace (optional)
  parameters_.insert(make_pair("spmOscillatingVariablesList", ParameterSolver("spmOscillatingVariablesList", VAR_TYPE_STRING, !mandatory)));
}

void
SolverIDASPM::setSolverSpecificParameters() {
  ...
  // SPM parameters
  const ParameterSolver& pulsation = findParameter("spmPulsation");
  spmPulsation_ = pulsation.hasValue() ? pulsation.getValue<double>() : 2. * std::acos(-1.) * 50.;
  const ParameterSolver& oscRegex = findParameter("spmOscillatingVariables");
  spmOscillatingVariables_ = oscRegex.hasValue() ? oscRegex.getValue<std::string>() : "";
  const ParameterSolver& oscList = findParameter("spmOscillatingVariablesList");
  spmOscillatingVariablesList_ = oscList.hasValue() ? oscList.getValue<std::string>() : "";
}

const std::string&
SolverIDASPM::solverType() const {
  static std::string solverType = "IDASPMSolver";
  return solverType;
}

void
SolverIDASPM::init(const std::shared_ptr<Model>& model, const double t0, const double tEnd) {
  ...
  // SPM estimator Jacobians dF/dX and dF/dXdot (filled and pushed in evalJ); the
  // capacity grows automatically through SolverCommon::copySparseToKINSOL.
  spmJacX_ = SUNSparseMatrix(model->sizeY(), model->sizeY(), 10., CSR_MAT, sundialsContext_);
  spmJacXdot_ = SUNSparseMatrix(model->sizeY(), model->sizeY(), 10., CSR_MAT, sundialsContext_);
  if (spmJacX_ == NULL || spmJacXdot_ == NULL)
    throw DYNError(Error::SUNDIALS_ERROR, SolverFuncErrorIDA, "SUNSparseMatrix");
  ...
  // (9bis) Sinusoidal Predictor Method
  // ----------------------------------
  // Build the oscillating-variable boolean vector (sundialsVectorIsAC_) from the
  // spmOscillatingVariables regular expression and enable the SPM. The state and
  // derivative Jacobians dF/dX and dF/dXdot are pushed to the Fourier-coefficient
  // estimator from evalJ (IDA's Jacobian-update callback) via IDASPMSetJacobians;
  // the estimator reuses them and its factorization across the intervening steps.
  // sundialsVectorYType_ provides the differential indicator.
  const int nbOscillating = buildOscillatingVarsVector();
  Trace::info(Trace::solver()) << "SolverIDASPM: " << nbOscillating << "/" << model->sizeY()
                               << " oscillating variables, pulsation = " << spmPulsation_ << " rad/s"
                               << Trace::endline;
  flag = IDASetSPM(IDAMem_, sundialsVectorIsAC_, spmPulsation_,
                   sundialsVectorYType_, 10 * model->sizeY());
  if (flag != 0)
    throw DYNError(Error::SUNDIALS_ERROR, SolverFuncErrorIDA, "IDASetSPM");
...
void
SolverIDASPM::calculateIC(const double /*tEnd*/) {
#ifdef _DEBUG_
  vector<double> y0;
...
void
SolverIDASPM::analyseFlag(const int & flag) {
  stringstream msg;
  switch (flag) {
    case IDA_SUCCESS:
...
int
SolverIDASPM::evalF(realtype tres, N_Vector yy, N_Vector yp,
        N_Vector rr, void* data) {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
  Timer timer("SolverIDASPM::evalF");
#endif
  SolverIDASPM* solver = reinterpret_cast<SolverIDASPM*> (data);
...
void
SolverIDASPM::printWeightedErrors() const {
...
int
SolverIDASPM::evalG(realtype tres, N_Vector yy, N_Vector yp, realtype* gout,
        void* data) {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
  Timer timer("SolverIDASPM::evalG");
#endif
  SolverIDASPM* solver = reinterpret_cast<SolverIDASPM*> (data);
  Model& model = solver->getModel();
  realtype* iyy = NV_DATA_S(yy);
...
int
SolverIDASPM::evalJ(realtype tt, realtype cj,
        N_Vector yy, N_Vector yp, N_Vector /*rr*/,
        SUNMatrix JJ, void* data,
        N_Vector /*tmp1*/, N_Vector /*tmp2*/, N_Vector /*tmp3*/) {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
  Timer timer("SolverIDASPM::evalJ");
#endif
  SolverIDASPM* solver = reinterpret_cast<SolverIDASPM*> (data);
  Model& model = solver->getModel();
  ...
  // Sinusoidal Predictor Method: push the state and derivative Jacobians into the
  // Fourier-coefficient estimator. evalJ is IDA's Jacobian-update callback, so the
  // estimator receives a fresh dF/dX and dF/dXdot exactly when IDA refreshes its
  // Newton Jacobian, and reuses them (and the factorization of its normal-equations
  // matrix) on the intervening steps. dF/dX is evalJt with cj = 0; dF/dXdot is
  // evalJtPrim. The model state was set by copyContinuousVariables above.
  SparseMatrix smjX;
  smjX.init(size, size);
  model.evalJt(tt, 0., smjX);
  SolverCommon::copySparseToKINSOL(smjX, solver->spmJacX_, size, NULL);

  SparseMatrix smjXdot;
  smjXdot.init(size, size);
  model.evalJtPrim(tt, 1., smjXdot);
  SolverCommon::copySparseToKINSOL(smjXdot, solver->spmJacXdot_, size, NULL);

  IDASPMSetJacobians(solver->IDAMem_, solver->spmJacX_, solver->spmJacXdot_);

  return 0;
}

std::vector<std::string>
SolverIDASPM::splitVariableList(const std::string& str) {
...
int
SolverIDASPM::buildOscillatingVarsVector() {
  const int size = model_->sizeY();
  sundialsVectorIsAC_ = N_VNew_Serial(size, sundialsContext_);
  if (sundialsVectorIsAC_ == NULL)
    throw DYNError(Error::SUNDIALS_ERROR, SolverFuncErrorIDA, "N_VNew_Serial");

  // Explicit name list (faithful to the original per-variable selection): the
  // remaining set tracks which listed names are not matched, to warn on typos.
  const std::vector<std::string> listVec = splitVariableList(spmOscillatingVariablesList_);
  std::set<std::string> listSet(listVec.begin(), listVec.end());
  std::set<std::string> unmatched(listVec.begin(), listVec.end());

  // Regex selector (optional): an empty pattern disables regex matching.
  const bool useRegex = !spmOscillatingVariables_.empty();
  const std::regex re = useRegex ? std::regex(spmOscillatingVariables_) : std::regex();

  realtype* isAC = NV_DATA_S(sundialsVectorIsAC_);
  int count = 0;
  for (int i = 0; i < size; ++i) {
    const std::string name = model_->getVariableName(i);
    bool oscillating = false;
    if (!listSet.empty() && listSet.count(name) > 0) {
      oscillating = true;
      unmatched.erase(name);
    }
    if (!oscillating && useRegex && std::regex_match(name, re))
      oscillating = true;
    isAC[i] = oscillating ? 1. : 0.;
    if (oscillating)
      ++count;
  }

  // Warn for any listed name that does not exist in the model (typo / stale name).
  for (const std::string& name : unmatched)
    Trace::warn(Trace::solver()) << "SolverIDASPM: oscillating variable '" << name
                                 << "' from spmOscillatingVariablesList is not a state variable of the model"
                                 << Trace::endline;

  if (count == 0)
    Trace::warn(Trace::solver()) << "SolverIDASPM: neither spmOscillatingVariablesList nor the regular expression '"
                                 << spmOscillatingVariables_ << "' selected any variable;"
                                 << " the SPM degrades to a standard IDA integration"
                                 << Trace::endline;
  return count;
}

void
SolverIDASPM::solveStep(double tAim, double& tNxt) {
    ...
    if (flag == IDA_ROOT_RETURN) {
      ...
      for (unsigned int i = 0; i < rootsFound.size(); i++) {
        if (rootsFound[i] != NO_ROOT) {
          Trace::debug() << "SolverIDASPM: rootsfound ->  g[" << i << "]=" << rootsFound[i] << Trace::endline;
...
bool SolverIDASPM::setupNewAlgRestoration(modeChangeType_t modeChangeType) {
...
void SolverIDASPM::updateAlgebraicRestorationStatistics() {
  long int nNewt = 0;
  long int nre = 0;
  long int nje = 0;
...
vector<state_g>
SolverIDASPM::getRootsFound() const {
#if defined(_DEBUG_) || defined(PRINT_TIMERS)
  Timer timer("SolverIDASPM::getRootsFound()");
#endif
...
double SolverIDASPM::getTimeStep() const {
  double hused = 0.;
...
void
SolverIDASPM::printHeaderSpecific(std::stringstream& ss) const {
  ss << "¦ iter num   order (k)      time step (h)";
}

void
SolverIDASPM::getLastConf(long int& nst, int& kused, double& hused) const {
...
void
SolverIDASPM::printSolveSpecific(std::stringstream& msg) const {
  long int nst = 0;
  int kused = 0;
  double hused = 0.;
...
void
SolverIDASPM::errHandlerFn(int error_code, const char* module, const char* function,
...
void
SolverIDASPM::setDifferentialVariablesIndices() {
  const std::vector<propertyContinuousVar_t>& modelYType = model_->getYType();
