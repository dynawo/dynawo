...
SolverIDA::defineSpecificParameters() {
  ...
  parameters_.insert(make_pair("optimizeReinit", ParameterSolver("optimizeReinit", VAR_TYPE_BOOL, notMandatory)));
  ...
  if (!solver->flagInit()) {
    ...
    if (!yErr.empty()) {
      ...
      for (vector<std::pair<double, int> >::iterator it = yErr.begin(); it != yErr.end() && i < nbErr; ++it, ++i)
        Trace::debug() << DYNLog(SolverIDAErrorValue, thresholdErr, it->second, model.getVariableName(it->second), it->first) << Trace::endline;
    }
