...
def replace_dynamic_indexing(body):
    ...
    for line in body:
        if ("calc_base_index_dims_subs" not in line and "&omc_Modelica_Math_BooleanVectors_index(" not in line):
        ...
        if "calc_base_index_dims_subs" in line:
            ...
            if "real_array_create" in line:
                ...
                for var, var_name, size, expr in match:
                    ...
                    index_tmp+=1
        elif "&omc_Modelica_Math_BooleanVectors_index(" in line:
            ptrn_var_index = re.compile(r'&omc_Modelica_Math_BooleanVectors_index\(threadData, (?P<var>[\w 0-9]+)\)')
            match = ptrn_var_index.findall(line)
            for var in match:
                body_to_return.append("  integer_array " + var +"_table = omc_Modelica_Math_BooleanVectors_index(threadData," + var+");\n")
                body_to_return.append(re.sub(ptrn_var_index,"&"+ var + "_table", line))
