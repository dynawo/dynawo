...
/**
 * @file activePowerControl1.cpp
 * @brief sample program creating a network with the activePowerControl extension
 */
...
using namespace IIDM::extensions::activepowercontrol;
...
int main(int argc, char** argv) {
  ...
  BatteryBuilder battery_builder = BatteryBuilder()
    .minMaxReactiveLimits( MinMaxReactiveLimits(0, 10) )
    .pmin(0).pmax(10).p0(5).q0(6);
  ...
  Battery& battery = network.substations().get("station").voltageLevels().get("VL").batteries().get("battery1");

  battery.setExtension(ActivePowerControlBuilder().participate(true).droop(2.5).build());
