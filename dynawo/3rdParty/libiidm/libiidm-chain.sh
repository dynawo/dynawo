...
install_libiidm() {
  ...
  if [ "$DYNAWO_LIBRARY_TYPE" = "SHARED" ]; then
    IIDM_LIBRARY_TYPE=ON
  else
    IIDM_LIBRARY_TYPE=OFF
  fi
  cmake $SOURCE_DIR -DIIDM_BUILD_SHARED=$IIDM_LIBRARY_TYPE -DBUILD_XML=ON -DCMAKE_INSTALL_PREFIX=$INSTALL_DIR -DCMAKE_PREFIX_PATH=$LIBXML_INSTALL_DIR -DCMAKE_BUILD_TYPE=$BUILD_TYPE -DCMAKE_C_COMPILER=$C_COMPILER -DCMAKE_CXX_COMPILER=$CXX_COMPILER -DCXX11_ENABLED=$CXX11_ENABLED $CMAKE_OPTIONNAL
