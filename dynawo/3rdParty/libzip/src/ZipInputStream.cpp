...
__LA_SSIZE_T bis_read(struct archive* /*archive*/, void* client_data, const void** buffer) {
...
boost::shared_ptr<ZipFile> ZipInputStream::read(const std::string& filename) {
    std::ifstream stream(filename.c_str(), std::ios::binary);
    if (!stream.good())
        throw ZipException::Impl(Error::FILE_NOT_FOUND, filename);
...
boost::shared_ptr<ZipFile> ZipInputStream::read(std::istream& stream) {
    boost::shared_ptr<ZipFile> zip(new ZipFile::Impl());
    ...
    if (status != ARCHIVE_OK) {
        ...
        throw ZipException::Impl(Error::LIBARCHIVE_INTERNAL_ERROR, message);
    }
    ...
    while (archive_read_next_header2(archive, info) == ARCHIVE_OK) {
        ...
        boost::shared_ptr<ZipEntry> entry(new ZipEntry::Impl(info, data));
