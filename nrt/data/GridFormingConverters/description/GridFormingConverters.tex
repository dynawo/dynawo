...
\section*{Test case description}
This test case consists of a current-controlled grid-forming converter based on a Virtual Synchronous Machine Control (VSM), with current limitation enforced through a proportional virtual impedance (activated above IMaxVI) combined with a Quasi-Static Electrical Model to translate the resulting voltage reference into a current reference.
The GFM model is connected to an infinite bus through a RL line (R=0.0001667 p.u., X=0.005 p.u.) and the following events are simulated :
\begin{itemize}
...
measured at the RLC Filter of the Converter block.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[scale = 0.8]
    \begin{axis}[
      ...
      \legend{$P_{\text{FilterRef}}$,$P_{\text{Filter}}$}
    \end{axis}
  \end{tikzpicture}
  \caption{Active power in p.u. at the GFM filter and its reference value}
  \label{comp20}
\end{figure}

Results show that the converter is able to track the changes in reference active power and to react to the sudden changes in frequency and voltage at the bus.
As a note, the whole converter block is computed in the Real-Imaginary frame i.e. same as the grid's. A previous version of this model existed with the Converter block in the GFM rotating frame (DQ) and produced identical results.
\end{document}
