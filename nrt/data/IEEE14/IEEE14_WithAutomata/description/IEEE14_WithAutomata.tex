...
Transformer 5-6 is equipped with a ratio tap-changer that controls the voltage at its higher terminal (node 5). The role of the tap-changer is to maintain the high terminal voltage within a certain range, independently of what happens on the other terminal. \\
...
\begin{figure}[H]
\subfigure[Load 2 and 3 terminal voltages (p.u)]
{%
  \begin{tikzpicture}
...
\subfigure[Network voltage at bus 2 and 3 (p.u)]
{%
  \begin{tikzpicture}
    \begin{axis}[height = 2.3in]
        \addplot[color=blue!50]
...
{%
  \begin{tikzpicture}
    \begin{axis}[height = 1.5in, legend pos=north west]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{TAP_CHANGER_LOCK_tapChangerLock_locked}]
        {../IEEE14_TapChangerLock/reference/outputs/curves/curves.csv};
        \addplot[color=green!50]
        table[x=time, y expr=\thisrow{TAP_CHANGER_LOCK_tapChangerLock_lockedT}]
        {../IEEE14_TapChangerLock/reference/outputs/curves/curves.csv};
        \legend{$Locked$, $LockedT$}
    \end{axis}
  \end{tikzpicture}
}
