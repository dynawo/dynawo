...
\par At t=0, the power plant is injecting 0.7 pu of active power and 0.2 pu of reactive power into the grid. The voltage module at its terminal is 1 pu and the voltage angle is equal to 0 rad.
...
\begin{tikzpicture}
    \begin{axis}[height = 2.5in]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{WECCPVVSource_photovoltaics_wecc_reec_PInjPu}]
...
\begin{tikzpicture}
    \begin{axis}[height =2.5in]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{WECCPVVSource_photovoltaics_wecc_reec_QInjPu}]
        {../WECC/PV/WECCPVVoltageSource2/reference/outputs/curves/curves.csv};
        \addplot[color=red!50]
        table[x=time, y expr=\thisrow{WECCPVVSource_photovoltaics_wecc_reec_QInjPu}]
        {../WECC/PV/WECCPVVoltageSource4/reference/outputs/curves/curves.csv};
...
\item A fault at t=1 s and cleared at t=1.1 s. This test case is simulated in two separates folder to test busBreaker and nodeBreaker topologies, Fault and BBSFault folders.
\end{itemize}
...
The considered network consist in a 1000~MW generator that feeds a 600~MW + 200~Mvar load though a step-up transformer 24/400 kV, two parallel lines, and a step-down transformer 400/158 kV.
