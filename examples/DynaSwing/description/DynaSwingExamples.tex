...
\begin{itemize}
\item at $t=1s$: a 0.07 s three-phase fault at the transformer high voltage terminal;
...
\item A PV plant with the injector represented as a voltage source. Two REGC models enables the voltage source connection REGC-B : WECCPVPlantVoltageSource1, WECCPVPlantVoltageSource2 and REGC-C : WECCPVPlantVoltageSource3, WECCPVPlantVoltageSource4.
\item A Wind plant of type 4A with the injector represented as a current source (WECCWPP4ACurrentSource1 and WECCWPP4ACurrent
Source2).
The type 4A is referred to the wind with representation of drive train control. There are two test cases : WECCWPP4ACurrentSource1 where WTGTA model is used to represent the drive train control with constant mechanical power and  WECCWPP4ACurrentSource2 with WTGTB model with the mechanical power derived from the electrical power.
\item A Wind plant of type 4B with the injector represented as a current source (WECCWPP4BCurrentSource). The type 4B doesn't have a drive train control representation.
\item A Wind plant of type 3  with the injector represented as a current source (WECCWPP3CurrentSource1 and WECCWPP3CurrentSource2). The type 3 wind model has a representation of the mechanical part. There are two test cases :  WECCWPP3CurrentSource1 with the combination of WTGQA, WTGPA, WTGAA and WTGTA for the mechanical part, and WECCWPP3CurrentSource2 with the combination of WTGQA, WTGPB, WTGAA and WTGTA.
...
The implementation of the models WECCPVCurrentSource, WECCWPP4ACurrentSource1 and WECCWPP4BCurrentSource in Modelica has led to the following publication in the 14th Modelica conference:\\
...
\par At t = 0 s, the power plant is injecting 0.7 pu of active power and 0.2 pu of reactive power into the grid. The voltage module at its terminal is 1 pu and the voltage angle is equal to 0 rad.
...
\par The same scenario is simulated in the three cases. First the infinite bus voltage is reduced from 1 pu to 0.5 pu from t = 1 s to t = 2 s. Then its frequency is increased to 1.01 pu (from 1 pu) from t = 6 s to t = 6.5 s.
...
Four test cases are available for the PV power plants connected with voltage source : WECCPVPlantVoltageSource1, WECCPVPlantVoltageSource2, WECCPVPlantVoltageSource3 et WECCPVPlantVoltageSource4.

N.B :WECCPVPlantVoltageSource1 and WECCPVPlantVoltageSource3 could also be used to represent wind turbines.

\subsection{WECCPVPlantVoltageSource2 and WECCPVPlantVoltageSource4}
Here we are going to compare results from WECCPVPlantVoltageSource2 (REPC-A + REEC-B + REGC-B) against WECCPVPlantVoltageSource4 (REPC-A + REEC-B + REGC-C) :
...
\par At t = 0 s, the power plant is injecting 0.7 pu of active power and 0.2 pu of reactive power into the grid. The voltage module at its terminal is 1.03713742957 pu and the voltage angle is equal to 0.203890124 rad.
...
\par The same scenario is simulated in the two cases. First the infinite bus voltage is reduced from 1 pu to 0.5 pu from t = 4 s to t = 8 s. Then its frequency is increased to 1.01 pu (from 1 pu) from t = 16 s to t = 16.5 s.
...
\begin{tikzpicture}
    \begin{axis}[height = 2.5in]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{WECCPVVSource_photovoltaics_injector_PInjPu}]
        {../WECC/PV/WECCPVPlantVoltageSource2/reference/outputs/curves/curves.csv};
        \addplot[color=red!50]
        table[x=time, y expr=\thisrow{WECCPVVSource_photovoltaics_injector_PInjPu}]
        {../WECC/PV/WECCPVPlantVoltageSource4/reference/outputs/curves/curves.csv};
        \legend{$WECCPVPlantVoltageSource2$, $WECCPVPlantVoltageSource4$}
    \end{axis}
  \end{tikzpicture}
}
...
\begin{tikzpicture}
    \begin{axis}[height =2.5in]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{WECCPVVSource_photovoltaics_injector_QInjPu}]
        {../WECC/PV/WECCPVPlantVoltageSource2/reference/outputs/curves/curves.csv};
        \addplot[color=red!50]
        table[x=time, y expr=\thisrow{WECCPVVSource_photovoltaics_injector_QInjPu}]
        {../WECC/PV/WECCPVPlantVoltageSource4/reference/outputs/curves/curves.csv};
...
\subsection{WECCPVPlantVoltageSource1 and WECCPVPlantVoltageSource3}
Here we are going to compare results from WECCPVPlantVoltageSource1 (REPC-A + REEC-A + REGC-B) against WECCPVPlantVoltageSource3 (REPC-A + REEC-A + REGC-C) :

\subsubsection{Test case description}
In both test cases, the PV power plants are connected to an infinite bus through a line.
The line's parameters are $R = 0$ and $X = 0.0000020661$ to represent a strong grid.

Also, in order to compare both test cases, the parameter $tG = 0.02$ of REGC-B is chosen to match the response time of REGC-C with $Kii = 30$ and $Kip = 5$.


\subsubsection{Initial conditions}

\par At t = 0 s, the power plant is injecting 0.7 pu of active power and 0.2 pu of reactive power into the grid. The voltage module at its terminal is 1 pu and the voltage angle is equal to 0.00000144621 rad.
...
\begin{tikzpicture}
    \begin{axis}[height = 2.5in]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{WECCPVVSource_photovoltaics_HvMeasurements_PPu}]
        {../WECC/PV/WECCPVPlantVoltageSource1/reference/outputs/curves/curves.csv};
        \addplot[color=red!50]
        table[x=time, y expr=\thisrow{WECCPVVSource_photovoltaics_HvMeasurements_PPu}]
        {../WECC/PV/WECCPVPlantVoltageSource3/reference/outputs/curves/curves.csv};
        \legend{$WECCPVPlantVoltageSource1$, $WECCPVPlantVoltageSource3$}
    \end{axis}
  \end{tikzpicture}
}
\subfigure[Reactive power injected in pu (base SNom)]
...
\section{Type 3 Wind}
\subsection{Test case description}
The WPP3 is connected to an infinite bus through a line in both test cases.

\begin{figure}[H]
\center
  \includegraphics[width=0.55\textwidth]{WPP3.png}
  \caption{Test case diagram}
\end{figure}
...
\par The same scenario is simulated in both cases. First the infinite bus voltage is reduced from 1 pu to 0.6 pu from t = 1 s to t = 2 s. Then its frequency is increased to 1.01 pu (from 1 pu) from t = 6 s to t = 6.5 s.

\subsection{Results}
Here we plot active and reactive power for both test cases.
\begin{figure}[H]
\subfigure[Active power injected in pu (base SNom)]
{%
\begin{tikzpicture}
    \begin{axis}[height = 1.5in]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{WPP3_WPP3_injector_PInjPu}]
        {../WECC/Wind/WECCWPP3CurrentSource1/reference/outputs/curves/curves.csv};
        \addplot[color=red!50]
        table[x=time, y expr=\thisrow{WPP3_WPP3_injector_PInjPu}]
        {../WECC/Wind/WECCWPP3CurrentSource2/reference/outputs/curves/curves.csv};
        \legend{$WECCWPP3CurrentSource1$, $WECCWPP3CurrentSource2$}
    \end{axis}
  \end{tikzpicture}
}
\subfigure[Reactive power injected in pu (base SNom)]
...
\begin{tikzpicture}
    \begin{axis}[height = 1.5in]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{WPP3_WPP3_injector_QInjPu}]
        {../WECC/Wind/WECCWPP3CurrentSource1/reference/outputs/curves/curves.csv};
        \addplot[color=red!50]
        table[x=time, y expr=\thisrow{WPP3_WPP3_injector_QInjPu}]
        {../WECC/Wind/WECCWPP3CurrentSource2/reference/outputs/curves/curves.csv};
...
At initial time, the active power demanded by the battery is 0.5 pu (base SNom = 6 MVA) and the reactive power is 0 pu (base SNom = 6 MVA).
...
{%
  \begin{tikzpicture}
    \begin{axis}[height = 1.5in, yticklabel style={/pgf/number format/.cd,fixed,precision=3}]
        \addplot[color=blue!50]
        table[x=time,y expr=\thisrow{BESS_BESS_injector_UPu}
        ]
        {../WECC/BESS/WECCBESSPlantCurrentSource/reference/outputs/curves/curves.csv};
    \end{axis}
  \end{tikzpicture}
  }
\subfigure[Active power injected in pu (base SNom)]
...
\begin{tikzpicture}
    \begin{axis}[height = 1.5in]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{BESS_BESS_injector_PInjPuSn}]
        {../WECC/BESS/WECCBESSPlantCurrentSource/reference/outputs/curves/curves.csv};
    \end{axis}
  \end{tikzpicture}
}
...
\begin{tikzpicture}
    \begin{axis}[height = 1.5in]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{BESS_BESS_injector_QInjPuSn}]
        {../WECC/BESS/WECCBESSPlantCurrentSource/reference/outputs/curves/curves.csv};
    \end{axis}
  \end{tikzpicture}
}
...
\begin{tikzpicture}
    \begin{axis}[height = 1.5in]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{BESS_BESS_reecC_idCmdPu}]
        {../WECC/BESS/WECCBESSPlantCurrentSource/reference/outputs/curves/curves.csv};
    \end{axis}
  \end{tikzpicture}
}
...
\begin{tikzpicture}
    \begin{axis}[height = 1.5in]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{BESS_BESS_reecC_iqCmdPu}]
        {../WECC/BESS/WECCBESSPlantCurrentSource/reference/outputs/curves/curves.csv};
    \end{axis}
  \end{tikzpicture}
}
...
At t = 0.5 s the grid frequency is decreased by 0.005 pu and brought back to normal at 4 s. At t = 6 s, a short-circuit occurs between the lines. It is cleared after 150ms.

The following figures show the evolution of active and reactive power and frequency for the converter during simulation.

\begin{figure}[H]
\subfigure[Frequency in pu (base OmegaNom)]
{%
  \begin{tikzpicture}
    \begin{axis}[height = 1.5in, yticklabel style={/pgf/number format/.cd,fixed,precision=3}]
        \addplot[color=blue!50]
        table[x=time,y expr=\thisrow{GfmEPRI_gfm_pll_omegaPLLPu}
        ]
        {../GridForming/EPRI/GFM_Fault/reference/outputs/curves/curves.csv};
    \end{axis}
  \end{tikzpicture}
  }
\subfigure[Active power of the converter in pu (base SNom)]
{%
\begin{tikzpicture}
    \begin{axis}[height = 1.5in]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{GfmEPRI_gfm_epriVoltageControl_PPu}]
        {../GridForming/EPRI/GFM_Fault/reference/outputs/curves/curves.csv};
    \end{axis}
  \end{tikzpicture}
}
\subfigure[Reactive power of the converter in pu (base SNom)]
{%
\begin{tikzpicture}
    \begin{axis}[height = 1.5in]
        \addplot[color=blue!50]
        table[x=time, y expr=\thisrow{GfmEPRI_gfm_epriVoltageControl_QPu}]
        {../GridForming/EPRI/GFM_Fault/reference/outputs/curves/curves.csv};
    \end{axis}
  \end{tikzpicture}
}
...
Two converters are grid following and use the WECC WPP4A model presented above and three converters are grid forming converters using a droop control.

\section{Test cases description}

\par The test case is presented in the following figure. INF, HVDC1 and HVDC2 are grid forming converters while WP1 and WP2 are wind plants using the grid following WECC WPP4A model.
...
\begin{itemize}
\item An active power double step at WP2 at t = 1 s and t = 2 s and an active power double step at HVDC1 at t = 4 s and t = 5 s.
\item A line disconnection at t = 1 s.
\item A fault at t = 1 s and cleared at t = 1.1 s. This test case is simulated in two separate folders to test busBreaker and nodeBreaker topologies, Fault and BBSFault folders.
\end{itemize}
...
The considered network consists in a 1000~MW generator that feeds a 600~MW + 200~Mvar load though a step-up transformer 24/400 kV, two parallel lines, and a step-down transformer 400/158 kV.
...
The simulated scenario is :
\begin{itemize}
\item at $t=1s$: a 0.1 s three-phase fault at the load bus;
\end{itemize}
...
\chapter{DTR Fiche I8}

\section{Test case description}

This test is performed accordingly with DTR Fiche I8. The aim of this fiche is to check the behavior of a synchronous machine in case of a frequency variation. The objective is to verify that the voltage at the terminals of the production unit remains properly regulated during a significant variation in
frequency. The study of voltage stability under frequency variation is carried out using a simplified network diagram in which the generation unit is connected to a very powerful network with variable frequency and voltage via a connection reactance of
$X_{cc}$.

\begin{figure}[H]
  \includegraphics[width=\textwidth]{DTRI8.png}
  \caption{DTR Fiche I8 setup}
\end{figure}

\subsection{Scenario}
The simulated scenario is :
\begin{itemize}
\item at $t=20s$: a 7000MW load is disconnected;
\end{itemize}
...
\item $Order$=2;
\item $Accuracy_{Rel}$=$10^{-5}$;
\item $Accuracy_{Abs}$=$10^{-5}$;
\end{itemize}

\newpage
\section{Results}

The two figures below show the evolution of the voltage at the PDR bus and the frequency of the network.

\begin{figure}[H]
\subfigure[Voltage at PDR bus]
{%
  \begin{tikzpicture}
    \begin{axis}[height = 2in]
        \addplot[color=blue!50]
        table[x=time,y=BusPDR_bus_terminal_V_re]
        {../SingleMachineSystem/SynchronousMachineI8/reference/outputs/curves/curves.csv};
    \end{axis}
  \end{tikzpicture}
}
\subfigure[Frequency]
{%
  \begin{tikzpicture}
    \begin{axis}[height = 2in]
        \addplot[color=blue!50]
        table[x=time, y=Synch_Gen_generator_omegaPu]
        {../SingleMachineSystem/SynchronousMachineI8/reference/outputs/curves/curves.csv};
    \end{axis}
  \end{tikzpicture}
}
\caption{DTR Fiche I8 (voltage and frequency)}
\end{figure}
