...
However, when the system is facing important discontinuities or large disturbances, the \ac{NR} algorithm can fail. In this case, the strategy is to \textbf{decrease the time step temporarily}. The time step is then increased when the system comes back to a more stable situation to reach its normal fixed value. By default, the time step is set to 1 s in \Dynawo. \\
...
\item If the time step is not equal to its maximum values and the \ac{NR} algorithm converges, the time step is increased if the \ac{DAE} system doesn't need a reinitialization (see ~\ref{sec:AERestoration}). In this case, the increase is done by dividing the time step by a factor $kReduceStep$.
