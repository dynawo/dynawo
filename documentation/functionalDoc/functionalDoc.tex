...
Each component type accepts a set of \textit{generic} parameters (e.g. \texttt{bus\_uMax}), applied to all components of that type, and \textit{component-specific} parameters whose name is prefixed by the component identifier (e.g. \texttt{BUS1\_uMax}). When both are provided, the component-specific value takes precedence. Parameters are provided in a \textit{.par} file associated to the network model in the \textit{.dyd} file. \\

The following two parameters apply to the network model as a whole:
\begin{table}[H]
\center
...
\midrule
\rowcolor{white}
\pname{startingPointMode} & \small{STRING} & \small{Starting point mode: \texttt{"flat"} (flat start) or \texttt{"warm"} (warm start)} & \small{warm} \\
\rowcolor{gray!10}
\pname{deactivate\_zero\_crossing\_functions} & \small{BOOL} & \small{If true, deactivates zero-crossing detection in the network model} & \small{false} \\
\bottomrule
\end{tabular}
\caption{Global network parameters}
\end{table}

\textbf{Note on \texttt{deactivate\_zero\_crossing\_functions}:} when set to \texttt{true}, this parameter disables the current limit automata and other protection mechanisms embedded in the network model (overload tripping for lines and transformers, transformer tap changer actions, shunt reclosing delays, etc.). This is intended for use cases such as DynaSwing simulations, where the studied phenomena (transient stability, inter-area oscillations) take place on a timescale much shorter than the operating time of protections: activating protections in this context would be physically irrelevant and would only slow down the simulation. \\
...
\begin{table}[H]
\center
\begin{tabular}{ p{5cm} | c | p{6cm} | c }
\toprule
\textbf{Name} & \textbf{Type} & \textbf{Description} & \textbf{Default value} \\
\midrule
\rowcolor{white}
\pname{bus\_uMax} & \small{DOUBLE} & \small{Upper voltage constraint limit (p.u.); a constraint is logged if violated} & \small{from IIDM} \\
\rowcolor{gray!10}
\pname{bus\_uMin} & \small{DOUBLE} & \small{Lower voltage constraint limit (p.u.); a constraint is logged if violated} & \small{from IIDM} \\
\rowcolor{white}
\pname{bus\_hasShortCircuitCapabilities} & \small{BOOL} & \small{Indicates if the bus has short-circuit capabilities} & \small{false} \\
\bottomrule
\end{tabular}
...
\midrule
\rowcolor{white}
\pname{line\_currentLimit\_maxTimeOperation} & \small{DOUBLE} & \small{Maximum overload duration (s) before the line is tripped} & \small{90} \\
\rowcolor{gray!10}
\pname{line\_isDynamic} & \small{BOOL} & \small{If true, enables the dynamic line model (includes $L \frac{di}{dt}$ and $C \frac{dv}{dt}$ terms)} & \small{false} \\
\bottomrule
\end{tabular}
...
\midrule
\rowcolor{white}
\pname{transformer\_currentLimit\_maxTimeOperation} & \small{DOUBLE} & \small{Maximum overload duration (s) before the transformer is tripped} & \small{90} \\
\rowcolor{gray!10}
\pname{transformer\_t1st\_THT} & \small{DOUBLE} & \small{First tap changer action delay (s) for very high voltage ($\geq$ 130 kV) transformers} & \small{30} \\
\rowcolor{white}
\pname{transformer\_tNext\_THT} & \small{DOUBLE} & \small{Subsequent tap changer action delay (s) for very high voltage transformers} & \small{10} \\
\rowcolor{gray!10}
\pname{transformer\_t1st\_HT} & \small{DOUBLE} & \small{First tap changer action delay (s) for high voltage (50–130 kV) transformers} & \small{60} \\
\rowcolor{white}
\pname{transformer\_tNext\_HT} & \small{DOUBLE} & \small{Subsequent tap changer action delay (s) for high voltage transformers} & \small{10} \\
\rowcolor{gray!10}
\pname{transformer\_tolV} & \small{DOUBLE} & \small{Voltage tolerance (kV) used for the tap changer automaton set point} & \small{0.015} \\
\bottomrule
\end{tabular}
...
\midrule
\rowcolor{white}
\pname{generator\_isVoltageDependent} & \small{BOOL} & \small{If true, active and reactive power injection depends on voltage} & \small{false} \\
\rowcolor{gray!10}
\pname{generator\_alpha} & \small{DOUBLE} & \small{Voltage exponent for active power; required when voltage-dependent} & \small{--} \\
\rowcolor{white}
\pname{generator\_beta} & \small{DOUBLE} & \small{Voltage exponent for reactive power; required when voltage-dependent} & \small{--} \\
\bottomrule
\end{tabular}
...
\midrule
\rowcolor{white}
\pname{load\_alpha} & \small{DOUBLE} & \small{Voltage exponent for active power ($P \propto (U/U_0)^\alpha$)} & \small{0} \\
\rowcolor{gray!10}
\pname{load\_beta} & \small{DOUBLE} & \small{Voltage exponent for reactive power ($Q \propto (U/U_0)^\beta$)} & \small{0} \\
\rowcolor{white}
\pname{load\_isRestorative} & \small{BOOL} & \small{If true, uses the first-order restorative load model} & \small{false} \\
\rowcolor{gray!10}
\pname{load\_isControllable} & \small{BOOL} & \small{If true, both P and Q can be varied by an external model} & \small{false} \\
\rowcolor{white}
\pname{load\_isPControllable} & \small{BOOL} & \small{If true, only P can be varied by an external model} & \small{false} \\
\rowcolor{gray!10}
\pname{load\_isQControllable} & \small{BOOL} & \small{If true, only Q can be varied by an external model} & \small{false} \\
\rowcolor{white}
\pname{load\_isSettable} & \small{BOOL} & \small{If true, P0 and Q0 set points can be modified during the simulation} & \small{false} \\
\rowcolor{gray!10}
\pname{load\_Tp} & \small{DOUBLE} & \small{Active power restoration time constant (s); used when \texttt{load\_isRestorative=true}} & \small{90} \\
\rowcolor{white}
\pname{load\_Tq} & \small{DOUBLE} & \small{Reactive power restoration time constant (s); used when \texttt{load\_isRestorative=true}} & \small{90} \\
\rowcolor{gray!10}
\pname{load\_zPMax} & \small{DOUBLE} & \small{Maximum value of the restorative $z_P$ variable} & \small{100} \\
\rowcolor{white}
\pname{load\_zQMax} & \small{DOUBLE} & \small{Maximum value of the restorative $z_Q$ variable} & \small{100} \\
\rowcolor{gray!10}
\pname{load\_alphaLong} & \small{DOUBLE} & \small{Long-term voltage exponent for active power in the restorative model} & \small{0} \\
\rowcolor{white}
\pname{load\_betaLong} & \small{DOUBLE} & \small{Long-term voltage exponent for reactive power in the restorative model} & \small{0} \\
\bottomrule
\end{tabular}
...
\midrule
\rowcolor{white}
\pname{capacitor\_no\_reclosing\_delay} & \small{DOUBLE} & \small{Minimum waiting time (s) between two successive actions on a capacitor shunt} & \small{300} \\
\rowcolor{gray!10}
\pname{reactance\_no\_reclosing\_delay} & \small{DOUBLE} & \small{Minimum waiting time (s) between two successive actions on a reactance shunt} & \small{0} \\
\bottomrule
\end{tabular}
\caption{Shunt parameters}
\end{table}

\paragraph{Dangling line}
~~\\

A dangling line represents a line that is connected to the network at one end only. Current maximum limits can be applied: if the dangling line is overloaded for longer than the allowed time, it will be tripped. \\
...
\midrule
\rowcolor{white}
\pname{dangling\_line\_currentLimit\_maxTimeOperation} & \small{DOUBLE} & \small{Maximum overload duration (s) before the dangling line is tripped} & \small{90} \\
\bottomrule
\end{tabular}
\caption{Dangling line parameters}
\end{table}
...
\subsubsection[Initialization models in Dynawo]{Initialization models in \Dynawo}

Initialization models in \Dynawo are fully independent algebraic models designed to compute an initial value
for any variable of a given dynamic model (i.e. the model used during the simulation) with a non straighforward initialization.
These initial values are computed by solving an independant specific problem, relying on input static network and model parameters.

The initialization models are used during the local initialization phase, where each model is initialized independently,
without enforcing consistency between interconnected models.

By convention, the suffix \_INIT is used in the Modelica library to associate a dynamic model with its corresponding initialization model.

The transfer of values from the initialization model to the dynamic model relies on a naming convention:
a variable computed in the initialization model must have exactly the same name as a parameter in the dynamic model.

The following example shows these conventions.

\lstinputlisting[language=Modelica,title=Test\_INIT.mo]{../resources/exampleExecutables/TestInitModel/Test_INIT.mo}
\lstinputlisting[language=Modelica,title=Test.mo]{../resources/exampleExecutables/TestInitModel/Test.mo}

In this example the \lstinline[language=Modelica]{u} variable will start with the value of \lstinline[language=Modelica]{u0} that is computed in the \_INIT model.
...
\item the initial tolerance multiplier used at the first Newton iteration (tol = \textit{initialaddtol} * \textit{fnormtol}) \textit{initialaddtol}
...
initialaddtol & 0.1 & initialaddtolAlg & 0.1 & initialaddtolAlgJ & 0.1 \\
...
In addition, a few other parameters are available for all solvers (Simplified, Trapezoidal and \ac{IDA}) to fine-tune the algebraic equations restoration and the solver behavior [see below]. We advise not to modify these parameters except if you encounter an unexpected simulation problem ; in this case, you can try to deactivate some of these parameters. Nevertheless, they have been thoroughly tested before being integrated.
\label{CommonBehaviorParameters}
\begin{itemize}
\item \textit{optimizeReinitAlgebraicResidualsEvaluations} (boolean, true by default) discards some algorithmic optimizations realized in the solver implementation (for example, we currently avoid recalculating algebraic residuals at the first iteration of a time-step because their value is the same due to the 0-order prediction scheme use) ;
\item \textit{enableSilentZ} (boolean, true by default) allows the discrete variable propagation loop to stop early when only "silent" discrete variables (i.e. without effect on the continuous equations) are still being modified ;
\item \textit{minimumModeChangeTypeForAlgebraicRestoration} (string, \textit{ALGEBRAIC} by default, other possible value \textit{ALGEBRAIC\_J\_UPDATE}) sets the minimum mode change severity that triggers an algebraic equations restoration ; \textit{minimumModeChangeTypeForAlgebraicRestorationInit} sets the same threshold but only during the solver initialization (no restoration is triggered by default during initialization) ;
\item \textit{multipleStrategiesForAlgebraicRestoration} (boolean, false by default) allows the algebraic equations restoration to retry with an alternative Newton-Raphson strategy (line search) if the default strategy fails to converge ;
\item \textit{printResiduals}, \textit{printReinitResiduals} and \textit{printUnstableRoot} (booleans, false by default) respectively print in the logs the residuals during the Newton-Raphson resolution, the residuals during an algebraic equations restoration, and the unstable roots detected by the solver.
\end{itemize}
...
In addition to these parameters, parameters defining the Newton-Raphson solver behavior during algebraic equations restoration are also used. They have a similar default value than for the simplified solver but can be overwritten by the user [see Table \ref{OptionalParameters}]. The common solver behavior parameters described in [\ref{CommonBehaviorParameters}] also apply to \ac{IDA}.

The optional boolean parameter \textit{activateAlternativeStrategiesOnDivergence} (false by default) activates a dedicated recovery strategy used when \ac{IDA} fails to converge or fails an error test on a time step. On a first failure, a reinitialization forcing the recalculation of the algebraic restoration Jacobian is triggered. If the step still fails, the minimum time step and the unit roundoff precision (see below) are both divided by a factor 100 before the step is retried, so that \ac{IDA} can resolve the transient with a finer resolution. These relaxed values are progressively restored to their original ones once the simulation has run long enough without further failure.

Two optional parameters enable to activate a time-scaled recomputation of \ac{IDA}'s internal unit roundoff (\textit{URound}), used by \ac{IDA} to compute the perturbations of finite-difference approximations and the root-finding tolerance (see the \href{https://sundials.readthedocs.io/en/latest/ida/Mathematics_link.html\#rootfinding}{\underline{Sundials documentation}}):
\begin{itemize}
\item \textit{activateTimeScaledURound} (boolean, false by default) activates the mechanism ;
\item \textit{timeScaledURoundPrecision} (double) is the reference precision from which the unit roundoff is recomputed at each time step, scaled with the current simulation time. By default, it is set to the simulation precision (parameter \textit{precision} of the \textit{.jobs} file, 1e-6 by default).
\end{itemize}
These two parameters are used in conjunction with \textit{activateAlternativeStrategiesOnDivergence}: when the recovery strategy divides the minimum time step by 100, it also divides \textit{timeScaledURoundPrecision} by the same factor.
