...
\textbf{To achieve this goal, \Dynawo is based on two main principles: the use of a high-level modelling language \href{https://modelica.org/} {\underline{Modelica}} and a strict separation between modelling and solving parts}.\\
...
\textbf{DynaFlow is a new steady-state simulation tool that aims at calculating the steady-state point by using a simplified time-domain simulation, guaranteeing the correctness of the solution found.} The \textit{Illustrative Test Cases} example section as well as the \textit{IEEE14 and WSCC9 test cases} show the pros of this approach compared to a purely static simulation tool. Preliminary studies on the France-Spain border have proved the advantages of the approach over classical static simulation tool, allowing to have a higher maximum flow across the border in realistic situations.\\
...
The interactive mode is under test based on common DynaWaltz combination of models and solver meant for long-term stability studies. Future developments are foreseen, including improvements on the interaction mechanism, optimization of the solver performance under real-time constraint, and models extension for relevent training scenarios.
