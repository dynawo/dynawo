...
LOG="idaspm/DYNAMO_IDASPM_clean/DYNAMO_IDASPM_patch/tnr/data/GFf_Load_InfBus/outputs/logs/dynamoCompiler.log"
raw=open(LOG,encoding='utf-8',errors='replace').read().split('\n')
s=next(i for i,l in enumerate(raw) if l.lstrip().startswith('"class GFf_Load_InfBus_test'))
e=next(i for i,l in enumerate(raw) if 'end GFf_Load_InfBus_test;' in l)
block=raw[s:e+1]
...
inj=('GFf_Load_InfBus.converter_Grid_InstFunction.getRecordConstructorFunction'
     ' failed for OpenModelica.Scripting.translateModel\nForming1.p.v[3];')
fix='GFf_Load_InfBus.converter_Grid_Forming1.p.v[3];'
n=text.count(inj); text=text.replace(inj,fix)
...
for ln in lines:
    ...
    if re.match(r'\s*(protected\s+)?parameter\s+enumeration\(', ln):
        m=re.search(r'\)\s+(GFf_Load_InfBus\.[A-Za-z0-9_.]+)', ln)
...
text=re.sub(r'GFf_Load_InfBus(?:\.[A-Za-z_]\w*)+',
...
for conv, val in (('Grid_Forming1', '0.0023327515569160017'),
                  ('Grid_Feeding1', '0.004666652459686555')):
    text = re.sub(
        r'(GFf_Load_InfBus_converter_' + conv + r'_Ti_i(?:\([^)]*\))?\s*=\s*)[^;]*wn_i[^;]*',
        r'\g<1>' + val, text)

# header / wrapper
text=text.replace('class GFf_Load_InfBus_test','model GFf_Load_InfBus',1)
text=text.replace('end GFf_Load_InfBus_test;','  annotation(preferredView = "text");\nend GFf_Load_InfBus;')
...
'''
open("GFf_Load_InfBus.mo","w").write(HEADER+text)
