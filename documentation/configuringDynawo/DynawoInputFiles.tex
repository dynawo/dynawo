...
If filter=``LAST'', constraints cleared during simulation will be removed and the ones still existing at the end of the simulation are associated to the value seen at the last time step.
...
\begin{itemize}
  \item \textbf{INPUT/ZMQ}: ZeroMQ interface in 'reply' mode on specified endpoint (default = 'tcp://*:5555'). Needs the environnement variable DYNAWO\_USE\_ZMQPP=ON and zmqpp library installed.
  \item \textbf{OUTPUT/ZMQ}: ZeroMQ interface in 'pub' mode on specified endpoint  (default = 'tcp://*:5556'). Needs the environnement variable DYNAWO\_USE\_ZMQPP=ON and zmqpp library installed.
\end{itemize}
