...
\subsection[Init models in Dynawo]{Init models in \Dynawo}

In Dynawo we use what we call an init model to initialize some parts of the "real" dynamic model.
At the start of the simulation during what we call local initialization phase we can compute the values for this init model and then for the global initialization phase we use values computed in the local phase to initialize values for the dynamic model.
This init model needs to be a fully independent algebraic model.
By convention we name them \_INIT in the Modelica library.
Then the mechanism to pass values between the two phases is also based on a convention.
For example, if you have a parameter named u0 in the dynamic model, the parameter's value will come either from the par file or from the init model, where u0 is a variable.
The latter mechanism is purely based on this naming convention where init variables and dynamic parameters need to have matching names.
