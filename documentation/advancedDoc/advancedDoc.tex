...
In Dynawo we use what we call an init model to initialize some parts of the "real" dynamic model. At the start of the simulation during what we call local initialization phase we can compute the values for this init model and then for the global initialization phase we use values computed in the local phase to initialize values for the dynamic model. This init model needs to be a fully independent algebraic model. By convention we name them \_INIT in the Modelica library. Then the mechanism to pass values between the two phases is also based on a convention. In a init model if you have a variable, for example u0, then if you have a parameter named u0 in the dynamic model the parameter will be get the value of the u0 variable from its init model. This mechanism is purely based on this naming convention where init variables and dynamic parameters need to be matched by names.
