...
\item \href{https://www.hdfgroup.org/solutions/hdf5/}{\underline{HDF5}}, a data model, library and file format for storing and managing large amounts of numerical data, developed and maintained by The HDF Group and distributed under the HDF5 License (a BSD-style permissive open-source license). \Dynawo uses version 1.12.2 of the HDF5 library as an optional dependency to support the HDF5 curves export mode (\texttt{exportMode="HDF5"}). When this mode is not required, \Dynawo can be built without HDF5 support by setting the CMake option \texttt{DYNAWO\_ACTIVATE\_HDF5\_EXPORT} to \texttt{OFF}.
